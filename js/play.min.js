...
var defineItemModal=new DefineItemModal;defineItemModal.init();var genericComparisonRow={};function generic_example(){generic_addRow("row_1_","your-settings-menu","exampleList")}function generic_addRow(a,b,c){$("#"+c).append(generic_generateRow(a,b));initAttributeButton(a+"attButton1");initAttributeButton(a+"attbutton2")}
...
a+")"});$(".playpage").css({"-ms-transform":"scale("+a+")"});$(".playpage").css({"-webkit-transform":"scale("+a+")"})}$(window).resize(resizePlayPage);console.log("Enter main.js version 0.5.0");$(document).ready();$("select").material_select();var testgraph={};
function testJumpNodes(){var a=!0;cy.$(".jumpend").each(function(b,c){var d=c.predecessors("node").roots();for(b=0;b<d.size();b++)d[b].hasClass("jump")||(console.log("bad one found",d[b].data("name")),a=!1,d[b].flashClass("jumpenderror",3E3),c.flashClass("jumpenderror",3E3))});return a}
