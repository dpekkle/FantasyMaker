...
$(".deletebutton")[0].click()):13!=b.which&&32!=b.which||$(".openoverlay").each(function(a){"none"!=$(this).css("display")&&(console.log("Enter press on ",$(this)),$(this).trigger("click"))}))})});var jumpOverlay={};function populateJumpOverlay(a){a=a.data("triggerType");"attribute"==a?loadJumpConditions():"button"==a&&loadJumpButton();setJumpNodeType(a)}
function saveJump(){cy.$(":selected")[0].data("triggerType",$("#choosejumptrigger :checked").attr("value"));saveJumpConditions();saveJumpButton();setJumpNodeType(null);$("#jumpConditionList").html("");$("#jumpButtonList").html("")}
...
function addJumpCondition(){var a=parseInt($("#jumpConditionList li").size());generic_addRow("row_"+a+"_","jump-settings-menu","jumpConditionList")}function loadJumpButton(){}function saveJumpButton(){}function createNewJumpButton(){}$("#choosejumptrigger").on("change",function(a){a=$("#choosejumptrigger :checked").attr("value");setJumpNodeType(a)});
...
function initJumpNodes(){conditional_jump_nodes=cy.nodes('.jump[triggerType = "attribute"]');jump_context_stack=[]}function checkConditionalJumps(){var a=!1;conditional_jump_nodes.forEach(function(b){if(evaluateConditionalJump(b))return a=b,!1});return a}
