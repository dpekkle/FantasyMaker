...
function http_redirectHome(){window.location="http://localhost:3000/create.html"}function http_save(a,b){http_addTokenToHeader();return $.ajax({type:"POST",url:"/saveProject",data:{save:JSON.stringify(a)},success:function(a){http_handleAuth(a)?b.data=!0:(b.data=!1,console.log(a))},contenttype:"application/json"})}
...
states.NEWJUMP:$(a).hasClass("prebuilt")&&current_state!=states.NEWFIGHT&&current_state!=states.NEWSTORE?exitStates():($(a).hasClass("deletebutton")&&removeElement(),current_state=states.DEFAULT,$(a).removeClass("activebutton"))}function choosePrebuilt(a){$(".prebuilt").addClass("activebutton");"Fight"==a?current_state=states.NEWFIGHT:"Store"==a&&(current_state=states.NEWSTORE)}var project={};console.log("Entering project.js");
var host="http://localhost:3000/",project_project=initEmptyProject("none","none");defaultState();
...
function checkBounds(a,b,c){a=parseFloat(a)||0;a+b>c&&(b=c-a);return b}function dragMoveListener(a){var b=$("#pagecontainers")[0],c=b.getBoundingClientRect().width/b.offsetWidth,d=b.getBoundingClientRect().height/b.offsetHeight,b=a.target,c=(parseFloat(b.getAttribute("data-x"))||0)+a.dx/c;a=(parseFloat(b.getAttribute("data-y"))||0)+a.dy/d;b.style.webkitTransform=b.style.transform="translate("+c+"px, "+a+"px)";b.setAttribute("data-x",c);b.setAttribute("data-y",a)}window.dragMoveListener=dragMoveListener;
var pageOverlay={},show_handles=!0;function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}function toggleHandles(){show_handles=!show_handles;$(".handlemode").toggleClass("activebutton");show_handles?($(".handlecontainer").show(),$(".handlemode").html("Handles: Shown")):($(".handlecontainer").hide(),$(".handlemode").html("Handles: Hidden"))}
...
function chooseNodeTemplate(a){$(".pagemode").addClass("activebutton");current_state=states.NEWPAGE;selected_page_template=project_project.project_templates[a];$(".pagemode").html("Page Node ("+a+")");console.log("Select template: "+a)}var canvasEvents={};console.log("Enter canvasEvents.js");source_node=null;
cy.on("tapend",":orphan",function(a){var b=a.cyTarget,c=a.cyRenderedPosition;cy.$(":parent").each(function(a,e){if(e!==b){var f=e.renderedBoundingBox();console.log(f);c.x>f.x1&&c.x<f.x2&&c.y>f.y1&&c.y<f.y2&&(console.log("Move ",b.id()," into ",e.id()),b.move({parent:e.id()}))}})});cy.on("taphold",":parent",function(a){parent.children().animated()});
