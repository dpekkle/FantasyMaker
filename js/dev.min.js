...
function http_delete(a){console.log("Delete");$.ajax({type:"POST",url:"php/delete.php",data:JSON.stringify(a),success:function(a){},contentType:"application/json"})}function http_packageProject(a,b,c){a={username:a,graph:[],statTypes:[]};a.graph=b;a.statTypes=c;return a};var generalOverlay={};function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}
function openEditPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null===a&&overlayToolbar(b);if(b.hasClass("page")){var c=b.data("textcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div.text-container:last").prepend(retrieveHandleHTML("text",c[a].name));c=b.data("imgcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div.img-container:last").prepend(retrieveHandleHTML("img",c[a].name));outgoingEdges=
b.outgoers().edges();for(var c=b.data("decisioncontainers"),d=0;d<outgoingEdges.size();d++){var e=!1;for(a=0;a<c.length;a++)outgoingEdges[d].data("name")==c[a].name&&(e=!0);e||addDecisionContainer(b,d,outgoingEdges.eq(d).data("text"),outgoingEdges[d].data("name"))}for(a=0;a<c.length;a++){e=!1;for(d=0;d<outgoingEdges.size();d++)c[a].name==outgoingEdges[d].data("name")&&($("#pagecontainers").append(c[a].html),$("#pagecontainers div.decision-container:last").prepend(retrieveHandleHTML("decision",c[a].name)),
e=!0);e||c.splice(a,1)}}}function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}function openEditControlOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$("#controlcontainers #controltext").val(escapeHtml(b.data("text")));populateControlOverlay(b)}
...
function cleanup_edge_labels(a){var b=a.source();if(b.hasClass("page"))for(var c=0;c<b.data("decisioncontainers").length;c++)if(b.data("decisioncontainers")[c].name==a.data("name")){b.data("decisioncontainers").splice(c,1);break}cy.remove(a);c=0;a=b.edgesTo("*");for(c;c<a.size();c++){var d=String.fromCharCode("A".charCodeAt()+c);a[c].style("label",d);a[c].data("name",d);b.hasClass("page")&&(b.data("decisioncontainers")[c].name=d,updatePageStyle(b))}}
...
cy.on("unselect",function(a){a.cyTarget.isNode()&&a.cyTarget.outgoers().removeClass("parent-selected");console.log("Unselect event fired ",a.cyTarget.data("id"));0===cy.$(":selected").size()?($(".editbutton").hide(),$(".selectionbutton").hide()):showOverlayLinks(cy.$(":selected")[0])});var contextMenu={};function changeCSSinMenu(a,b,c,d){b.$trigger.parent().siblings(".editdiv").css(c,d)}
$(function(){$.contextMenu({selector:".textmenu",trigger:"left",items:{sep2:"---------",font:{name:"Font",items:{"font-family":{name:"Style",items:{"fold2-key1":{name:"Courier New",callback:function(a,b){changeCSSinMenu(a,b,"font-family",'"Courier New", Courier, monospace')}},"fold2-key2":{name:"Lucinda Console",callback:function(a,b){changeCSSinMenu(a,b,"font-family",'"Lucida Console", Monaco, monospace')}},"fold2-key3":{name:"Times New Roman",callback:function(a,b){changeCSSinMenu(a,b,"font-family",
'"Times New Roman", Times, serif')}},"fold2-key4":{name:"Arial",callback:function(a,b){changeCSSinMenu(a,b,"font-family",'"Arial", Helvetica, sans-serif')}},"fold2-key5":{name:"Tahoma",callback:function(a,b){changeCSSinMenu(a,b,"font-family","Tahoma, Geneva, sans-serif")}},"fold2-key6":{name:"Comic Sans",callback:function(a,b){changeCSSinMenu(a,b,"font-family",'"Comic Sans MS", cursive, sans-serif')}}}},"font-size":{name:"Size",callback:function(a,b){var c=prompt("Enter font size","14");changeCSSinMenu(a,
b,"font-size",c+"px")}},"font-color":{name:"Colour",items:{Red:{name:"Black",callback:function(a,b){changeCSSinMenu(a,b,"color","Black")}},Gray:{name:"Gray",callback:function(a,b){changeCSSinMenu(a,b,"color","Gray")}},White:{name:"White",callback:function(a,b){changeCSSinMenu(a,b,"color","White")}},Custom:{name:"Custom",callback:function(a,b){var c=prompt("Enter hex colour","#FFFFFF");changeCSSinMenu(a,b,"background-color",c)}}}}}},background:{name:"Background",items:{Colour:{name:"Background Colour",
items:{Red:{name:"Red",callback:function(a,b){changeCSSinMenu(a,b,"background-color","Red")}},Gray:{name:"Gray",callback:function(a,b){changeCSSinMenu(a,b,"background-color","Gray")}},White:{name:"White",callback:function(a,b){changeCSSinMenu(a,b,"background-color","White")}},Custom:{name:"Custom",callback:function(a,b){var c=prompt("Enter hex colour","#FFFFFF");changeCSSinMenu(a,b,"background-color",c)}}}},Opacity:{name:"Opacity",items:{}}}},border:{name:"Border",items:{Visible:{name:"Visible",callback:function(a,
b){}},Colour:{name:"Colour",callback:function(a,b){}},Corners:{name:"Corners",callback:function(a,b){}},Thickness:{name:"Thickness",callback:function(a,b){}},Style:{name:"Style",callback:function(a,b){}}}},"delete":{name:"Delete",icon:"delete",callback:function(a,b){alert("Are you sure you want to delete this container?");b.$trigger.parent().parent().remove()}}}})});var dragDrop={};grid_snapping_mode=!1;var grid_targets=interact.createSnapGrid({x:10,y:10,range:10,offset:{x:-2,y:2}});function initSnapOptions(){return grid_snapping_mode?{targets:[grid_targets],relativePoints:[{x:1,y:1}],endOnly:!1}:{targets:[],relativePoints:[{x:1,y:1}],endOnly:!1}}var snap_options=initSnapOptions();
...
parseInt(c.value)}}console.log(project_project.characters[0])}function progressStory(a){null===currentNode?(currentNode=cy.$(".start")[0],parseNode()):0<currentNode.outgoers().size()?(currentNode=outgoingEdges.eq(a).target(),executeOutcomes(outgoingEdges.eq(a)),console.log("Now on node ",currentNode.data("id")),parseNode()):(currentNode=null,$(".playpage").html(""))}function stripDraggable(a){return a.replace(/drag-element/g,"")};console.log("Enter main.js version 0.5.0");$("select").material_select();var pageOverlay={};$(document).ready(function(){$(".modal-trigger").leanModal({dismissible:!0,ready:function(){var a=cy.$(":selected")[0];null!=a&&(a.hasClass("page")?(openEditPageOverlay(a),console.log("Opening page overlay")):a.isEdge()?(openEditConnectionOverlay(a),console.log("Opening Edge Overlay")):a.hasClass("control")&&(openEditControlOverlay(a),console.log("Opening Control Overlay")))},complete:function(){closeOverlay(null)}})});
function retrieveHandleHTML(a,b){var c;if("img"==a)c="<div id = 'img"+b+"'class = 'handle'>Image Container "+b;else if("text"==a)c="<div id = 'text"+b+"'class = 'handle'>Text Container "+b+'<a style="float:right" class="textmenu btn-floating btn waves-effect waves-light red"><i class="material-icons">settings</i></a>',c+="</div>";else if("decision"==a)c="<div id = 'decision"+b+"'class = 'handle'>Link "+b+"</div>";else return console.log("Unknown container type when generating handle for HTML"),null;
return c}function removeContainer(a,b){alert("Are you sure you want to delete"+a+"container"+b+"?");$("#"+a+b).parent().remove()}function addDecisionContainer(a,b,c,d){b="<div class = 'decision-container drag-element' style='position:absolute;'>"+("<div class = 'editdec decisionbutton drag-element resize-element' contenteditable=true>"+escapeHtml(c)+"</div>");b+="</div>";c=a.data("decisioncontainers");c.push({name:d,html:b});a.data("decisioncontainers",c)}
function addTextContainer(){var a;a="<div class='text-container drag-element' style='position:absolute;'><div class='editdiv resize-element' contenteditable=true ></div></div>";var b=$(".text-container").length;a=htmlToElements(a);$("#pagecontainers").append(a);$("#pagecontainers div.text-container:last").prepend(retrieveHandleHTML("text",b+1))}
function addImageContainer(){var a,b=prompt("Enter image url","http://");if(null!=b){if(null!=b.match(/\.(jpeg|jpg|gif|png)$/))a="<div class='img-container drag-element' style='position:absolute;'>",a+="<img class='editdiv resize-element' src="+b+"></img>",a+="</div>";else if(null!=b.match(/\.(webm)$/))a="<div class='img-container drag-element' style='position:absolute;'>",a+="<video preload='auto' autoplay='autoplay' loop='loop' class='editdiv resize-element'>",a+='<source src= "'+b+"\"type='video/webm'></source>",
a+="</video></div>";else if(null!=b.match(/\.(gifv|mp4)$/)){var c=b.lastIndexOf(".gifv");-1!=c&&(b=b.substr(0,c)+".mp4",console.log("Regexed to: ",b));a="<div class='img-container drag-element' style='position:absolute;'>";a+="<video preload='auto' autoplay='autoplay' loop='loop' class='editdiv resize-element'>";a+='<source src= "'+b+"\"type='video/mp4'></source>";a+="</video></div>"}else{alert("Not a valid url");return}$.ajax({url:b,success:function(b){b=$(".img-container").length;var c=htmlToElements(a);
$("#pagecontainers").append(c);$("#pagecontainers div.img-container:last").prepend(retrieveHandleHTML("img",b+1))},error:function(a){alert("file does not exist")}})}};var testgraph={};function checkValidGraph(){var a="";cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
