...
function retrieveHandleHTML(a,b){if("img"==a)return"<div class='handle'>Image Container "+b+"</div>";if("text"==a)return"<div class='handle'>Text Container "+b+"<p style='float:right;'>sup</p></div>";if("decision"==a)return"<div class='handle'>Link "+b+"</div>";console.log("Unknown container type when generating handle for HTML");return null}
...
function addTextContainer(){var a;a="<div id = 'text-container' class='drag-element' style='position:absolute;'><div id = 'editdiv' class='resize-element' contenteditable=true ></div></div>";var b=cy.$(":selected")[0].data("textcontainers").length,c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div#text-container:last").prepend(retrieveHandleHTML("text",b+1));c=cy.$(":selected")[0].data("textcontainers");c.push({name:b+1,html:a});cy.$(":selected")[0].data("textcontainers",c)}
function addImageContainer(){var a,b=prompt("Enter image url","http://");if(null!=b){if(null!=b.match(/\.(jpeg|jpg|gif|png)$/))a="<div id = 'img-container' class='drag-element' style='position:absolute;'>"+("<img id = 'editdiv' class='resize-element' src="+b+"></img>")+"</div>";else if(null!=b.match(/\.(webm)$/))a="<div id = 'img-container' class='drag-element' style='position:absolute;'><video preload='auto' autoplay='autoplay' loop='loop' id = 'editdiv' class='resize-element'>",a+='<source src= "'+
...
c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div#img-container:last").prepend(retrieveHandleHTML("image",b+1));c=cy.$(":selected")[0].data("imgcontainers");c.push({name:b+1,html:a});cy.$(":selected")[0].data("imgcontainers",c)}}function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}
function openEditPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null===a&&overlayToolbar(b);if(b.hasClass("page")){var c=b.data("textcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div#text-container:last").prepend(retrieveHandleHTML("text",c[a].name));c=b.data("imgcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div#img-container:last").prepend(retrieveHandleHTML("image",c[a].name));outgoingEdges=
