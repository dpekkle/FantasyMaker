...
g)}return!1}}}},border:border_menu_entry(c),position:zIndex_menu_entry(),sep2:"---------",clone:{name:"Clone",callback:function(a,b){}},template:{name:"Templates",items:{save:{name:"Save As",callback:function(a,e){for(var f=0;void 0!==b["template"+template_ID]&&100>f;)template_ID++,f++,console.log("conflict with template IDs");100>f&&(f=prompt("Enter name for template"),b["template"+template_ID]={name:f,savedhtml:e.$trigger.parent().parent().children(c)[0].outerHTML,callback:function(a,d){if("Load"==
d.$selected.parent().siblings("span").html()){console.log("Loading: "+a);var e=d.$trigger.parent().parent().children(c),f=e.html();e[0].outerHTML=b[a].savedhtml;d.$trigger.parent().parent().children(c).html(f)}else"Delete"==d.$selected.parent().siblings("span").html()&&delete b[a]}},template_ID++)}},load:{name:"Load",items:b},"delete":{name:"Delete",items:b}}},"delete":{name:"Delete",icon:"delete",callback:function(b,c){if(confirm("Are you sure you want to delete this container?")){var f=c.$trigger.parent().attr("id").split(a)[1];
removeContainer(a,f)}}}}};if("img"==a)return{items:{border:border_menu_entry(c),position:zIndex_menu_entry(),sep3:"---------",URL:{name:"Change URL",callback:function(a,b){var f=prompt("Enter new URL for image/video","http://");(html_string=checkImageURL(f,""))&&$.ajax({url:f,success:function(a){a=$(c).parent();$(c).remove();a.append(html_string)},error:function(a){alert("URL: "+f+" does not exist")}})}}}}}
...
c)}}}}}
function border_menu_entry(a){return{name:"Border",items:{Style:{name:"Style",items:{None:{name:"None",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-style","none")}},Solid:{name:"Solid",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-style","solid")}},Double:{name:"Double",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-style","double")}},Dashed:{name:"Dashed",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-style","dashed")}},Dotted:{name:"Dotted",callback:function(b,
c){return changeCSSinMenu(a,b,c,"border-style","dotted")}},Button:{name:"Button",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-style","outset")}},Ridge:{name:"Ridge",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-style","ridge")}}}},Colour:{name:"Colour",items:{Red:{name:"Black",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-color","Black")}},Gray:{name:"Gray",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-color","Gray")}},Primary:{name:"Primary",
callback:function(b,c){return changeCSSinMenu(a,b,c,"border-color",primary_colour)}},Secondary:{name:"Secondary",callback:function(b,c){return changeCSSinMenu(a,b,c,"border-color",secondary_colour)}}}},Corners:{name:"Rounded Corners",callback:function(b,c){var d=prompt("Enter a number from 0 (square)  to 12 (fully rounded)","5");return 12>=d&&0<=d?changeCSSinMenu(a,b,c,"border-radius",d+"px"):!1}},Thickness:{name:"Border Width",callback:function(b,c){var d=prompt("Enter a number from 0 up","2");return 100>=
d&&0<=d?changeCSSinMenu(a,b,c,"border-width",d+"px"):!1}}}}}
...
function dragMoveListener(a){var b=a.target,c=(parseFloat(b.getAttribute("data-x"))||0)+a.dx;a=(parseFloat(b.getAttribute("data-y"))||0)+a.dy;b.style.webkitTransform=b.style.transform="translate("+c+"px, "+a+"px)";b.setAttribute("data-x",c);b.setAttribute("data-y",a)}window.dragMoveListener=dragMoveListener;function modalBounds(){$("#page-modal").hasClass("open")&&$(".drag-element").each(function(){$(this)})}var doit;window.onresize=function(){clearTimeout(doit);doit=setTimeout(modalBounds,300)};var pageOverlay={},show_handles=!0;function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}function toggleHandles(){show_handles=!show_handles;$(".handlemode").toggleClass("activebutton");show_handles?($(".handle").show(),$(".handlemode").html("Handles: Shown")):($(".handle").hide(),$(".handlemode").html("Handles: Hidden"))}
...
function chooseNodeTemplate(a){selected_page_template=page_templates[a];$(".pagemode").html("Page Node ("+a+")");console.log("Select template: "+a)};var canvasEvents={};console.log("Enter canvasEvents.js");source_node=null;cy.on("tap",":selected",function(a){console.log("Tapped on: ",cy.$(":selected").data("name"));current_state===states.CONNECTING&&createConnection(a.cyTarget)});
cy.on("tap",function(a){var b=a.cyTarget;b===cy&&cy.$(":selected").unselect();current_state===states.CONNECTING&&b===cy&&null!==source_node&&(source_node.removeClass("source_node"),source_node=null);current_state===states.NEWPAGE?(console.log("Add a node"),b===cy&&console.log("Really Add a node"+cy.add({data:{name:cy.nodes().size()+1,outputcontainer:selected_page_template.outputcontainer,imgcontainers:selected_page_template.imgcontainers,audio:selected_page_template.audio,textcontainers:selected_page_template.textcontainers,
decisioncontainers:[]},classes:"page",group:"nodes",renderedPosition:a.cyRenderedPosition}).data()),1===cy.elements(".page").size()&&cy.$(".page").first().addClass("start")):current_state===states.NEWCONTROL&&b===cy&&cy.add({data:{name:cy.nodes().size()+1,text:"control node text",priorityList:[],defaultFailEdge:"none"},classes:"control",group:"nodes",renderedPosition:a.cyRenderedPosition})});
...
cy.on("unselect",function(a){a.cyTarget.isNode()&&a.cyTarget.outgoers().removeClass("parent-selected");console.log("Unselect event fired ",a.cyTarget.data("id"));0===cy.$(":selected").size()?($(".editbutton").hide(),$(".selectionbutton").hide()):showOverlayLinks(cy.$(":selected")[0])});var hotkeys={},nodes_to_clone;
$(document).ready(function(){var a=!1;$("html").on("keyup",function(b){if(17==b.which||91==b.which)a=!1}).on("keydown",function(b){if(17==b.which||91==b.which)a=!0;console.log(b.which);if(a&&67==b.which)0!=cy.$(":selected").length&&(console.log("Copy"),nodes_to_clone=cy.$(":selected").nodes().jsons(),console.log(nodes_to_clone));else if(a&&86==b.which){if(console.log(nodes_to_clone),null!==nodes_to_clone){console.log("Paste");cy.$(":selected").unselect();for(b=0;b<nodes_to_clone.length;b++){var c={x:nodes_to_clone[b].position.x+
10,y:nodes_to_clone[b].position.y+=10};nodes_to_clone[b].data.name=parseInt(cy.nodes().size()+1);nodes_to_clone[b].classes=nodes_to_clone[b].classes.replace("start","");delete nodes_to_clone[b].data.id;cy.add({data:nodes_to_clone[b].data,classes:nodes_to_clone[b].classes,group:nodes_to_clone[b].group,position:c}).select()}nodes_to_clone=cy.$(":selected").nodes().jsons()}}else if(27==b.which)console.log("Press escape!"),$(".modal").hasClass("open")||(defaultState(),cy.$(":selected").unselect());else if(49<=
b.which&&58>=b.which)"none"==$("#pagetemplates").css("display")?49==b.which?$(".pagemode").trigger("click"):50==b.which?$(".controlmode").trigger("click"):51==b.which&&$(".connectionmode").trigger("click"):$("#pagetemplates li a").eq(b.which-49).trigger("click");else if(0!=cy.$(":selected").length)if(8==b.which||46==b.which)console.log("Press delete"),$(".deletebutton")[0].click();else if(13==b.which||32==b.which)$(".modal").hasClass("open")?console.log("A modal is open"):$(".editbutton").each(function(a){"none"!=
$(this).css("display")&&(console.log("Enter press on ",$(this)),$(this).trigger("click"))})})});var playGame={};outgoingEdges=currentNode=null;function prepareForGame(){outgoingEdges=currentNode=null;$(".playpage").html("");for(var a=cy.elements(),b=0;b<a.length;b++)updatePageStyle(a[b])}function wipeGame(){$(".playpage").html("")}function parseNode(){console.log("Parse node ",currentNode.data("id"));outgoingEdges=currentNode.outgoers().edges();currentNode.hasClass("page")?parsePage(outgoingEdges):currentNode.hasClass("control")&&parseControl(currentNode,outgoingEdges)}
