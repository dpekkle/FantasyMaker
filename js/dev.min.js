...
"youtube":"error"};this.addAudioEvent=function(a){this.assets[a].addEvent()}}function loadAudioObject(a){var b=new audioObj;b.selected_audio=a.selected_audio;b.unique_id=0;b.loaded=a.loaded;if(0==a.assets.length)b.changed=!1;else{b.changed=!0;for(var c=0;c<a.assets.length;c++)b.addAsset(a.assets[c].name,a.assets[c].link)}return b};var initCanvas={};console.log("Enter initCanvas.js");states={DEFAULT:0,CONNECTING:1,NEWPAGE:2,NEWCONTROL:3};current_state=states.DEFAULT;
...
function http_addTokenToHeader(){var a=window.localStorage.getItem("token");a&&(a=JSON.parse(a).token,$.ajaxSetup({headers:{"x-access-token":a}}))};var generalOverlay={};$(document).ready(function(){$(".modal-trigger").leanModal({dismissible:!0,ready:function(){var a=cy.$(":selected")[0];null!=a&&(a.hasClass("page")?(openEditPageOverlay(a),console.log("Opening page overlay")):a.isEdge()?(openEditConnectionOverlay(a),console.log("Opening Edge Overlay")):a.hasClass("control")&&(openEditControlOverlay(a),console.log("Opening Control Overlay")))},complete:function(){closeOverlay(null)}})});
...
states.NEWJUMP:$(a).hasClass("prebuilt")&&current_state!=states.NEWFIGHT&&current_state!=states.NEWSTORE?exitStates():($(a).hasClass("deletebutton")&&removeElement(),current_state=states.DEFAULT,$(a).removeClass("activebutton"))}function choosePrebuilt(a){$(".prebuilt").addClass("activebutton");"Fight"==a?current_state=states.NEWFIGHT:"Store"==a&&(current_state=states.NEWSTORE)};var project={};console.log("Entering project.js");var project_project=initEmptyProject("none","none");defaultState();function initEmptyProject(a,b){return{project_templates:{Default:{pagestyle:"width: 800px; height:600px; border: 3px solid black",outputcontainer:"",imgcontainers:[],vidcontainers:[],textcontainers:[],decisioncontainers:[]}},template_menus:new templateMenuObj,audio:new audioObj,projectOwner:a,projectName:b,graph:[],gameAttributes:{},attributesHTML:"",resolution:{x:800,y:600}}}
...
a[0]);var b=!0;""==a[0]&&(console.log("No Attribute name entered"),Materialize.toast("Please enter an attribute name",3E3,"rounded"),b=!1);if(a[1]<a[2]||a[1]>a[3]||a[2]>a[3]||""==(a[2]||a[3]))console.log("Illegal attribute values"),Materialize.toast("Conflicting Attribute Values",3E3,"rounded"),b=!1;return b})};function gameBrowser_addGame(a){$("#games-list").append('<div class="col s12 m4 l3"> <div class="card medium"> <div class="card-image"> <img src="http://materializecss.com/images/sample-1.jpg"><span class="card-title">Game Title Goes Here <br/><span style="font-size: 18px;"> Author: Jon Doe</span></span></div><div class="card-content"> <p>Game Description Will Go Here</p> </div> <div class="card-action"> <a href="#">Play Game</a> </div> </div> </div>')};var hotkeys={},nodes_to_clone;
...
a+")"});$(".playpage").css({"-webkit-transform":"scale("+a+")"})}$(window).resize(resizePlayPage);console.log("Enter main.js version 0.5.0");$(document).ready();$("select").material_select();$(document).ready();console.log($.cookie("FM_playUsername"));console.log($.cookie("FM_playProject"));getProject();function getProject(){var a=$.cookie("FM_playUsername"),b=$.cookie("FM_playProject");$.when(authenticate()).done(function(){load(a,b)})}
