...
0));var c=b.data("textcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div.text-container:last").prepend(genHandleHTML("text",c[a].name));c=b.data("imgcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",c[a].name));outgoingEdges=b.outgoers().edges();for(var c=b.data("decisioncontainers"),d=0;d<outgoingEdges.size();d++){var e=!1;for(a=0;a<c.length;a++)outgoingEdges[d].data("name")==
c[a].name&&(e=!0);e||addDecisionContainer(b,d,outgoingEdges.eq(d).data("text"),outgoingEdges[d].data("name"))}for(a=0;a<c.length;a++){e=!1;for(d=0;d<outgoingEdges.size();d++)c[a].name==outgoingEdges[d].data("name")&&($("#pagecontainers").append(c[a].html),$("#pagecontainers div.decision-container:last").prepend(genHandleHTML("decision",c[a].name)),e=!0);e||c.splice(a,1)}show_handles||$(".handle").hide();bindHandleSelection()}}
function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}function openEditControlOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$("#controlcontainers #controltext").val(escapeHtml(b.data("text")));populateControlOverlay(b)}
...
function closeOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null!=b&&(b.hasClass("page")&&($("#pagecontainers .handle").remove(),b.data("textcontainers",[]),b.data("imgcontainers",[]),b.data("outputcontainer",""),b.data("pagestyle",$("#pagecontainers").attr("style")),$("#pagecontainers").children("div[class^='text-container']").each(function(a){var d=this.outerHTML;console.log("Updating HTML for ",a);var e=b.data("textcontainers");e.push({name:a+1,html:d});b.data("textcontainers",e)}),$("#pagecontainers").children("div[class^='decision-container']").each(function(a){var d=
...
$("#ColourPicker2").spectrum({color:secondary_colour,change:function(a){secondary_colour=a.toRgbString();$("#ColourPicker2").css("background-color",secondary_colour)}});var template_ID=0,text_template_menu_list={},decision_template_menu_list={},image_template_menu_list={},page_template_menu_list={};
...
$(c).parent();$(c).remove();a.append(html_string)},error:function(a){alert("URL: "+f+" does not exist")}})}}},events:{show:function(a){$.contextMenu.setInputValues(a,this.data())},hide:function(a){$.contextMenu.getInputValues(a,this.data());changeCSSinMenu(c,a,"border-style",this.data().Style)}}};if("page"==a)return{items:{border:border_menu_entry(c),background:{type:"color",customName:"Background Colour",className:"background-color "+c}},events:{show:function(a){$.contextMenu.setInputValues(a,this.data())},
hide:function(a){$.contextMenu.getInputValues(a,this.data());console.log(this.data());$(c).css("border-style",this.data().Style)}}}}function bringEleToFront(a){var b=0;$("#pagecontainers").children("div").each(function(){var a=$(this).css("zIndex");a>b&&(b=a)});b++;console.log("Set zIndex to ",b);a.css("zIndex",b)}
...
image_template_menu_list)}});$.contextMenu({selector:".pagemenu",trigger:"left",build:function(a,b){return generateContextMenu("page")}})});
...
this.addClass("color").on("contextmenu:focus",function(a){}).on("contextmenu:blur",function(a){}).on("keydown",function(a){})};var dragDrop={};grid_snapping_mode=!1;var grid_targets=interact.createSnapGrid({x:10,y:10,range:10,offset:{x:-2,y:2}});function initSnapOptions(){return grid_snapping_mode?{targets:[grid_targets],relativePoints:[{x:1,y:1}],endOnly:!1}:{targets:[],relativePoints:[{x:1,y:1}],endOnly:!1}}var snap_options=initSnapOptions();
...
function gameAttributes_addChild(a,b,c,d){a=gameAttributes_find(a);var e=generateID();a[e]=new GameAttribute(a.path,a.level,b,e,c,d);d=a[e];a.children.push(e);console.log("New Nested attribute added under: "+a.path);console.log("New Attribute ID: "+e);var f="'"+d.path+"'";b=c?'<li class="'+e+' margin"><a onclick="gameAttributes_display('+f+')">'+b+"</a></li>":'<li class="'+e+' margin"><a onclick="gameAttributes_display('+f+')">'+b+'</a><ul id="'+d.path+'-inner_list"></ul></li>';$("#"+a.path+"-inner_list").append(b)}
function gameAttributes_find(a){path=a.split("_");a=project_project.gameAttributes;for(var b=0;b<path.length;b++)a=a[path[b]];return a}function gameAttributes_update(a,b,c){a=gameAttributes_find(a);a.name=b;a.value=c;console.log(a+" Updated")}function gameAttributes_delete(a){path=a.split("_");a=project_project.gameAttributes;for(var b=0;b<path.length-1;b++)a=a[path[b]];console.log(a);console.log("Deleting - "+path[path.length-1]);a[path[path.length-1]]=null;delete a[path[path.length-1]]}
function gameAttributes_display(a){a=gameAttributes_find(a);console.log("displaying attribute in panel: "+a.name);var b=$("#attribute-detail-title"),c=$("#attribute-detail-class-list"),d=$("#attribute-detail-value-list");c.empty();d.empty();b.text(" ");b.text(a.name);if(a.is_leaf)$("#attribute-detail").show();else{$("#attribute-detail").show();for(var e=a.children,b=0;b<e.length;b++)a[e[b]].is_leaf?d.append("<li>"+a[e[b]].name+": "+a[e[b]].value+"</li>"):c.append("<li>"+a[e[b]].name+"</li>")}}
function generateID(){for(var a="",b=0;4>b;b++)a+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(62*Math.random()));return a}function saveListHTML(){project_project.attributeHTML=$("#attributes-list").html()}function loadListHTML(){$("#attributes-list").html(project_project.attributeHTML)};var hotkeys={},nodes_to_clone;
