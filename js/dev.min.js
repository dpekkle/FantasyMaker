...
'" onclick="attributeOverlay_updateAttribute(this.id)">Update</button> </div> </div> </li>';$("#"+a).append(b);attributeCount++;attID++}function attributeOverlay_updateAttribute(a){var b=$("#"+a+"-name").val();$("#"+a+"-header").html(b)}function attributeOverlay_deleteAttribute(){};function audioAsset(a,b,c,d){this.name="Audio "+b+": "+a;this.id=b;this.type=d;this.link=c;this.loadHTML=function(){if("youtube"==this.type)return"HTML string!";if("mp3"==this.type)return"Some other HTML string!";alert("You shouldn't be here...")}}
function audioObj(){this.size=0;this.assets=[];this.getAsset=function(a){return void 0!==a?0<=a&&a<this.size?this.assets[a]:"No such asset":this.assets};this.addAsset=function(a,b){var c=this.parseType();"error"!=c&&(this.assets[this.size]=new audioAsset(a,this.size,b,c),this.size++)};this.parseType=function(a){return"youtube"}}var audio=new audioObj;
function testAudioObjects(){audio.addAsset("Test asset","http://www.youtube.com/somelink");audio.addAsset("Test asset2","http://www.youtube.com/somelink2");console.log(audio.getAsset(0));console.log(audio.getAsset(1));console.log(audio.getAsset());console.log(audio.getAsset("a"));console.log(audio.getAsset(-1));console.log(audio.getAsset(2))};var httpRequests={};console.log("Enter httpRequests.js");function http_save(){console.log("save");project_updateProject();$.ajax({type:"POST",url:"/saveProject",data:{save:JSON.stringify(project_project)},success:function(a){alert("Graph Saved")},contenttype:"application/json"})}
...
(console.log("Enter press on ",$(this)),$(this).trigger("click"))}))})});var playGame={};outgoingEdges=currentNode=null;function prepareForGame(){outgoingEdges=currentNode=null;$(".playpage").html("");for(var a=cy.elements(),b=0;b<a.length;b++)updatePageStyle(a[b])}function wipeGame(){$(".playpage").html("");$(".playpage").attr("style","")}
function parseNode(){console.log("Parse node ",currentNode.data("id"));outgoingEdges=currentNode.outgoers().edges();currentNode.hasClass("page")?parsePage(outgoingEdges):currentNode.hasClass("control")&&parseControl(currentNode,outgoingEdges)}function parsePage(a){stylePage()}
