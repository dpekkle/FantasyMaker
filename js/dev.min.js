...
removeElement(),current_state=states.DEFAULT,$(a).removeClass("activebutton"))};var pageOverlay={};function prepareForGame(){for(var a=cy.elements(),b=0;b<a.length;b++)showPageOverlay(a[b]),closeOverlay(a[b])}
function showPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$(".toolbar").hide();$("#modalcontainers").children().hide();overlayToolbar(b);if(b.hasClass("page")){$("#pagecontainers").show();$("#pagecontainers #pagetext").val(escapeHtml(b.data("text")));outgoingEdges=b.outgoers().edges();for(var c=0;c<outgoingEdges.size();c++)$("#drag-container").append("<button class = 'drag-element' id='decision"+c+"'>"+escapeHtml(outgoingEdges.eq(c).data("text"))+"</button>")}b.isEdge()&&($("#connectioncontainers").show(),
$("#connectioncontainers #decisiontext").val(escapeHtml(b.data("text"))));b.hasClass("control")&&($("#controlcontainers").show(),$("#controlcontainers #controltext").val(escapeHtml(b.data("text"))));null===a&&document.getElementById("Modal").showModal()}
...
function closeOverlay(a){null===a&&document.getElementById("Modal").close();var b=a;null===a&&(b=cy.$(":selected")[0]);b.hasClass("page")&&(b.data("styleHTML",$("#pagecontainers").html()),console.log(b.data("styleHTML")),$(".decisionbutton").remove())}function escapeHtml(a){var b=document.createElement("div");b.appendChild(document.createTextNode(a));return b.innerHTML};var ui={};$(".textarea").on("input",function(a){a=this.value;cy.$(":selected").data("text",a)});function removeElement(){element=cy.$(":selected");if(!element.empty()){for(var a=0;a<element.connectedEdges().jsons().length;a++)http_modStack.push(element.connectedEdges().jsons()[a]);for(a=0;a<element.jsons().length;a++)http_modStack.push(element.jsons()[a]);cy.remove(element);total_pages--;cleanup_titles();cy.$("node").first().addClass("start")}}
...
cy.on("tap",function(a){var b=a.cyTarget;b===cy&&cy.$(":selected").unselect();current_state!==states.CONNECTING||b!==source_node&&b!==cy||null===source_node||(source_node.removeClass("source_node"),source_node=null);current_state===states.NEWPAGE?(console.log("Add a node"),b===cy&&(console.log("Really Add a node"),cy.add({data:{name:++total_pages,text:"page text",img:"none",audio:"none",styleHTML:"none"},classes:"page",group:"nodes",renderedPosition:a.cyRenderedPosition})),1===cy.elements().size()&&
cy.$("node").first().addClass("start")):current_state===states.NEWCONTROL&&b===cy&&cy.add({data:{name:++total_pages,text:"control node text"},classes:"control",group:"nodes",renderedPosition:a.cyRenderedPosition})});
...
function conditions_createOutcome(a,b,c){if("+"!=b&&"-"!=b&&"/"!=b&&"*"!=b)console.log("jsonFactory_createOutcome(): invalid modifier parameter. Could not generate outcome object.");else if(1==isNaN(c))console.log("jsonFactory_createOutcome(): invalid value parameter(Not a number). Could not generate outcome object.");else return{stat:a,modifier:eval,value:c}};interact(".drag-element").draggable({inertia:!0,restrict:{restriction:"parent",endOnly:!0,elementRect:{top:0,left:0,bottom:1,right:1}},autoScroll:!0,onmove:dragMoveListener,onend:function(a){var b=a.target.querySelector("p");b&&(b.textContent="moved a distance of "+(Math.sqrt(a.dx*a.dx+a.dy*a.dy)|0)+"px")}});
function dragMoveListener(a){var b=a.target,c=(parseFloat(b.getAttribute("data-x"))||0)+a.dx;a=(parseFloat(b.getAttribute("data-y"))||0)+a.dy;b.style.webkitTransform=b.style.transform="translate("+c+"px, "+a+"px)";b.setAttribute("data-x",c);b.setAttribute("data-y",a)}window.dragMoveListener=dragMoveListener;document.getElementById("drag-container").style.display="block";var playGame={};outgoingEdges=currentNode=null;function parseNode(){console.log("Parse node ",currentNode.data("id"));outgoingEdges=currentNode.outgoers().edges();currentNode.hasClass("page")?parsePage(outgoingEdges):currentNode.hasClass("control")&&parseControl(outgoingEdges)}function parsePage(a){stylePage()}
function stylePage(){var a=currentNode.data("styleHTML");$(".playpage").html(a);a.includes("pagetext")&&($(".playpage #pagetext").html(escapeHtml(currentNode.data("text"))),console.log(currentNode.data("text")));a.includes("decisionbutton")&&(console.log("Let's bind buttons"),$(".playpage").children("button").each(function(a){console.log("Bound button, i = ",a);$(this).click(function(){progressStory(a)})}),console.log("Decisions total: ",i))}function parseControl(a){}
function progressStory(a){null===currentNode?(currentNode=cy.$(".start")[0],parseNode()):0<currentNode.outgoers().size()?(currentNode=outgoingEdges.eq(a).target(),console.log("Now on node ",currentNode.data("id")),parseNode()):(currentNode=null,$(".playpage").html(""))};console.log("Enter main.js version 0.5.0");var dialog=document.querySelector("dialog");dialogPolyfill.registerDialog(dialog);var testgraph={};function checkValidGraph(){var a="";cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
