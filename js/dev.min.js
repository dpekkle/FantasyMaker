...
function projectSettings_closeOverlay(){$("#projName").val("");$.when($("#newProject-modal").closeModal()).done(showMainContent())}function projectSettings_deleteProject(a,b){$.when(http_deleteProject(a,b)).done(projectSettings_populateProjectsList(a))};var states={DEFAULT:0,CONNECTING:1,NEWPAGE:2,NEWCONTROL:3};current_state=states.DEFAULT;function defaultState(){exitStates();current_state=states.DEFAULT}function exitStates(){current_state==states.CONNECTING&&(cy.boxSelectionEnabled(!0),null!==source_node&&(source_node.removeClass("source_node"),source_node=null));$("#sidebar .btn").removeClass("activebutton");$(".pagemode").html("Page Node")}
...
;var project={};console.log("Entering project.js");var project_project=project_createNewProject();defaultState();
function project_createNewProject(){var a;a=$("#projName").val()?$("#projName").val():"none";project_project=a={project_templates:{Default:{pagestyle:"width:800px; height:600px; \tborder: 3px solid #BBBBBB",outputcontainer:"",imgcontainers:[],vidcontainers:[],textcontainers:[],decisioncontainers:[]}},template_menus:new templateMenuObj,audio:new audioObj,projectOwner:"Admin",projectName:a,graph:[],gameAttributes:{},attributesHTML:"",resolution:{x:1E3,y:800}};cy.elements().remove();$.when(http_save(project_project)).done(projectSettings_closeOverlay());
return a}function project_addTopGameAttributeFolder(a){var b=generateID();project_project.gameAttributes[b]=new GameAttribute(null,null,a,b,!1);a='<li class="'+b+'-list-element margin pointer"><a onclick="gameAttributes_display(\''+b+"')\">"+a+'</a><ul id="'+b+'-inner_list"></ul></li>';$("#attributes-list").append(a);console.log("new top level attribute added: "+project_project.gameAttributes[b].path)}
...
show_handles||$(".handlecontainer").hide();bindHandleSelection()}
function addImageContainer(){var a,b=prompt("Enter image url","http://i.imgur.com/V7vuv85.png"),c=genPageCenterHTML(300,220);null!=b&&(a="<div class='img-container drag-element' style='position:absolute; "+c+"'>",c=checkImageURL(b,"img"))&&(a+=c,a+="</div>",$.ajax({url:b,success:function(b){b=$(".img-container").length;var c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",b+1));bringContainerToFront($("pagecontainers div.img-container:last"));
show_handles||$(".handlecontainer").hide();bindHandleSelection()},error:function(a){alert("URL: "+b+" does not exist")}}))}
function addVideoContainer(){var a,b=prompt("Enter video url","http://i.imgur.com/6aSJo9b.gifv"),c=genPageCenterHTML(300,220);null!=b&&(a="<div class='vid-container drag-element' style='position:absolute; "+c+"'>",c=checkImageURL(b,"vid"))&&(a+=c,a+="</div>",$.ajax({url:b,success:function(b){b=$(".vid-container").length;var c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div.vid-container:last").prepend(genHandleHTML("vid",b+1));bringContainerToFront($("pagecontainers div.vid-container:last"));
show_handles||$(".handlecontainer").hide();bindHandleSelection()},error:function(a){alert("URL: "+b+" does not exist")}}))}
...
$("#pagecontainers div.text-container:last").prepend(genHandleHTML("text",c[b].name));c=a.data("imgcontainers");for(b=0;b<c.length;b++)$("#pagecontainers").append(c[b].html),$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",c[b].name));c=a.data("vidcontainers");for(b=0;b<c.length;b++)$("#pagecontainers").append(c[b].html),$("#pagecontainers div.img-container:last").prepend(genHandleHTML("vid",c[b].name));$("#eventspane").append('<span class="eventspanetitle eventname" style="text-align:center; font-size: 16px;">Asset</span><span class="eventspanetitle eventtype" style="text-align:center; font-size: 16px;">Event</span><span class="eventspanetitle eventtrigger" style="text-align:center; font-size: 16px;">Time</span>');
...
function savePage(a){$("#pagecontainers .handlecontainer").remove();a.data("textcontainers",[]);a.data("imgcontainers",[]);a.data("vidcontainers",[]);a.data("events",[]);a.data("pagestyle",$("#pagecontainers").attr("style"));a.data("outputcontainer",$(".output-container").outerHTML);var b=[];$("#pagecontainers").children("div[class^='text-container']").each(function(a){var c=this.outerHTML;console.log("Updating HTML for ",a);b.push({name:a+1,html:c})});a.data("textcontainers",b);var c=[];$("#pagecontainers").children("div[class^='img-container']").each(function(a){var b=
this.outerHTML;console.log("Save HTML for img ",a);c.push({name:a+1,html:b})});a.data("imgcontainers",c);var d=[];$("#pagecontainers").children("div[class^='vid-container']").each(function(a){var b=this.outerHTML;console.log("Save HTML for vid ",a);d.push({name:a+1,html:b})});a.data("imgcontainers",c);$("#pagecontainers").children("div[class^='decision-container']").each(function(b){var c=this.outerHTML;a.data("decisioncontainers")[b].html=c;console.log("Save HTML for decision ",b)});var e=[];$("#eventspane").children(".eventscontainer").each(function(a){var b=
$(this).children(".eventname"),c=$(this).attr("class").split(" ")[0];a=b.nextAll(".eventtype:first");var d=a.nextAll(".eventtrigger:first"),l=$(this).attr("id"),b={eventtype:c,id:l,name:b.html(),action:a.find("div input").val(),setting:a.find(".setting").val(),trigger:d.find("input").val()};a.material_select("destroy");e.push(b)});e.sort(function(a,b){return parseFloat(a.trigger)-parseFloat(b.trigger)});a.data("events",e);a.data("eventspane",$("#eventspane").html());$("#pagecontainers").html("");
...
$(".playpage .handlecontainer").hide();$(".playpage").children().removeClass("resize-element");$(".playpage").children().children().removeClass("resize-element");$(".playpage").children().attr("contenteditable","false");$(".playpage").children().children().attr("contenteditable","false");for(f=0;f<e.length;f++);}
