...
function closeOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);b.hasClass("page")&&($("#pagecontainers .handle").remove(),$("#pagecontainers").children("div[id^='text-container']").each(function(a){var d=this.outerHTML;b.data("textcontainers")[a].html=d;console.log("Updating HTML for ",a)}),$("#pagecontainers").children("div[id^='decision-container']").each(function(a){var d=this.outerHTML;b.data("decisioncontainers")[a].html=d;console.log("save html for decision ",a)}));b.hasClass("control")&&
...
function populateEdgeOverlay(a){edgeOverlay_newOutcomeCount=edgeOverlay_newConditionCount=0;edgeOverlay_selectedEdge=a;for(var b=0;b<a.data.conditions.length;b++){var c=a.data.conditions[b];console.log(c);edgeOverlay_addExistingCondition("conditionsList",c)}console.log(a.data.outcomes.length);for(b=0;b<a.data.outcomes.length;b++)c=a.data.outcomes[b],edgeOverlay_addExistingOutcome("outcomesList",c)}
function saveEdge(a,b){for(var c=a.json(),d=0;d<c.data.conditions.length;d++){var e=c.data.conditions[d],f="exCondition_"+e.edge+"_"+e.id;e.stat=$("#"+f+"_statTypeList").val();e.comparison=$("#"+f+"_compList").val();e.value=$("#"+f+"_value").val()}for(d=0;d<c.data.outcomes.length;d++)e=c.data.outcomes[d],f="exOutcome_"+e.edge+"_"+e.id,e.stat=$("#"+f+"_statTypeList").val(),e.modifier=$("#"+f+"_modList").val(),e.value=$("#"+f+"_value").val();if("EDGE_OVERLAY"===b){for(d=0;d<edgeOverlay_newConditionCount;d++){var e=
$("#newCondition_statTypeList"+d).val(),g=$("#newCondition_compList"+d).val(),f=$("#newCondition_value"+d).val(),e=conditions_createCondition(c,e,g,f);console.log(e);void 0!==e&&c.data.conditions.push(e)}for(d=0;d<edgeOverlay_newOutcomeCount;d++)e=$("#newOutcome_statTypeList"+d).val(),g=$("#newOutcome_modList"+d).val(),f=$("#newOutcome_value"+d).val(),e=conditions_createOutcome(c,e,g,f),console.log(e),void 0!==e&&c.data.outcomes.push(e)}}
function populateStatsDropDownMenu(a){a="#"+a;for(var b=0;b<project_project.statTypes.length;b++)$(a).append('<option value="'+project_project.statTypes[b].type+'">'+project_project.statTypes[b].type+"</option>"),$(a).material_select()}
...
function removeOutcome(a){console.log("Removing outcome "+a);$("#"+a).hide(300,function(){$("#"+a).remove()});var b=a.split("_");if("newOutcome"!==b[0])if("exOutcome"===b[0])for(var c=cy.edges("[id='"+b[1]+"']").json(),d=0;d<c.data.outcomes.length;d++)d===parseInt(b[2])&&c.data.outcomes.splice(d,1);else console.log("removeOutcome(): error in id formatting")};var controlOverlay={};
function populateControlOverlay(a){a=a.connectedEdges().jsons();for(var b=0;b<a.length;b++){var c=a[b],d="<li id=connectedEdge_"+c.data.id+'><div class="collapsible-header">Edge '+c.data.name+'</div><div class="collapsible-body"><ul id = "'+c.data.id+'_collapsible" class="collapsible" data-collapsible="expandable"><li><div class="collapsible-header">Conditions for '+c.data.name+'</div><div class="collapsible-body"><ul id = "controlConditions_'+c.data.id+'" class="list-unstyled" ></ul></div></li><li><div class="collapsible-header">Outcomes for '+c.data.name+
'</div><div class="collapsible-body"><ul id = "controlOutcomes_'+c.data.id+'" class="list-unstyled" ></ul></div></li></ul></div></li>';$("#connectedEdgesList").append(d);$("#"+c.data.id+"_collapsible").collapsible({accordion:!1});for(d=0;d<c.data.conditions.length;d++)edgeOverlay_addExistingCondition("controlConditions_"+c.data.id,c.data.conditions[d]);for(d=0;d<c.data.outcomes.length;d++)edgeOverlay_addExistingOutcome("controlOutcomes_"+c.data.id,c.data.outcomes[d])}}
function saveControl(a){a.connectedEdges().forEach(function(a){saveEdge(a,"CONTROL_OVERLAY")})};var dragDrop={};grid_snapping_mode=!1;var grid_targets=interact.createSnapGrid({x:10,y:10,range:10,offset:{x:-2,y:2}});function initSnapOptions(){return grid_snapping_mode?{targets:[grid_targets],relativePoints:[{x:1,y:1}],endOnly:!1}:{targets:[{x:0,y:0,range:0}]}}var snap_options=initSnapOptions();
function setInteractions(){interact(".drag-element:not([locked])").draggable({snap:snap_options,inertia:!1,restrict:{restriction:"parent",endOnly:!1,elementRect:{top:0,left:0,bottom:1,right:1}},autoScroll:!0,onmove:dragMoveListener,onend:function(a){console.log(a)}}).allowFrom(".handle");interact(".resize-element").resizable({snap:snap_options,edges:{left:!1,right:!0,bottom:!0,top:!1}}).on("resizemove",function(a){var b=a.target;b.style.width=checkBounds(b.parentNode.getAttribute("data-x"),a.rect.width,
$("#pagecontainers").width())+"px";b.style.height=checkBounds(b.parentNode.getAttribute("data-y"),a.rect.height,$("#pagecontainers").height()-$(".handle").height())+"px"})}setInteractions();function toggleGridMode(){grid_snapping_mode=!grid_snapping_mode;snap_options=initSnapOptions();setInteractions()}function checkBounds(a,b,c){a=parseFloat(a)||0;console.log("Check "+a+" + "+b);a+b>c?(console.log("Greater than "+c),b=c-a):console.log("Less than "+c);return b}
