...
function openEditPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null===a&&overlayToolbar(b);if(b.hasClass("page")){a=b.data("outputcontainer");$("#pagecontainers").append(a);$("#pagecontainers div.output-container:last").prepend(genHandleHTML("output",0));var c=b.data("textcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div.text-container:last").prepend(genHandleHTML("text",c[a].name));c=b.data("imgcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),
$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",c[a].name));outgoingEdges=b.outgoers().edges();for(var c=b.data("decisioncontainers"),d=0;d<outgoingEdges.size();d++){var e=!1;for(a=0;a<c.length;a++)outgoingEdges[d].data("name")==c[a].name&&(e=!0);e||addDecisionContainer(b,d,outgoingEdges.eq(d).data("text"),outgoingEdges[d].data("name"))}for(a=0;a<c.length;a++){e=!1;for(d=0;d<outgoingEdges.size();d++)c[a].name==outgoingEdges[d].data("name")&&($("#pagecontainers").append(c[a].html),
$("#pagecontainers div.decision-container:last").prepend(genHandleHTML("decision",c[a].name)),e=!0);e||c.splice(a,1)}show_handles||$(".handle").hide()}}function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}function openEditControlOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$("#controlcontainers #controltext").val(escapeHtml(b.data("text")));populateControlOverlay(b)}
...
function closeOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null!=b&&(b.hasClass("page")&&($("#pagecontainers .handle").remove(),b.data("textcontainers",[]),b.data("imgcontainers",[]),b.data("outputcontainer",""),$("#pagecontainers").children("div[class^='text-container']").each(function(a){var d=this.outerHTML;console.log("Updating HTML for ",a);var e=b.data("textcontainers");e.push({name:a+1,html:d});b.data("textcontainers",e)}),$("#pagecontainers").children("div[class^='decision-container']").each(function(a){var d=
this.outerHTML;b.data("decisioncontainers")[a].html=d;console.log("Save HTML for decision ",a)}),$("#pagecontainers").children("div[class^='img-container']").each(function(a){var d=this.outerHTML;console.log("Save HTML for img ",a);var e=b.data("imgcontainers");e.push({name:a+1,html:d});b.data("imgcontainers",e)}),$("#pagecontainers").children("div[class^='output-container']").each(function(a){b.data("outputcontainer",this.outerHTML);console.log("Save HTML for output ")}),$("#pagecontainers").html("")),
b.hasClass("control")&&(saveControl(b),$("#connectedEdgesList").children().remove()),b.isEdge()&&(saveEdge(b,"EDGE_OVERLAY"),$("#conditionsList").children().remove(),$("#outcomesList").children().remove()))}
...
$("#"+b).css("transition",""))})};var contextMenu={};function changeCSSinMenu(a,b,c,d,e){c.$trigger.parent().siblings(a).css(d,e);return!1}var primary_colour="rgba(255,255,255,1.0)",secondary_colour="rgba(255,255,255,1.0)";$("#ColourPicker1").spectrum({color:primary_colour,change:function(a){primary_colour=a.toRgbString();$("#ColourPicker1").css("background-color",primary_colour)}});$("#ColourPicker2").spectrum({color:secondary_colour,change:function(a){secondary_colour=a.toRgbString();$("#ColourPicker2").css("background-color",secondary_colour)}});
var template_ID=0,text_template_menu_list={},decision_template_menu_list={},image_template_menu_list={};
function generateContextMenu(a,b){if("text"==a||"output"==a)var c=".editdiv";else"decision"==a?c=".editdec":"img"==a&&(c=".editdiv");if("text"==a||"decision"==a||"output"==a)return{items:{font:{name:"Font",items:{Alignment:{name:"Alignment",items:{Left:{name:"Left",callback:function(a,b){return changeCSSinMenu(c,a,b,"text-align","left")}},Right:{name:"Right",callback:function(a,b){return changeCSSinMenu(c,a,b,"text-align","Right")}},Center:{name:"Center",callback:function(a,b){return changeCSSinMenu(c,
a,b,"text-align","Center")}},Justify:{name:"Justify",callback:function(a,b){return changeCSSinMenu(c,a,b,"text-align","Justify")}}}},"font-family":{name:"Style",items:{"fold2-key1":{name:"Courier New",callback:function(a,b){return changeCSSinMenu(c,a,b,"font-family",'"Courier New", Courier, monospace')}},"fold2-key2":{name:"Lucinda Console",callback:function(a,b){return changeCSSinMenu(c,a,b,"font-family",'"Lucida Console", Monaco, monospace')}},"fold2-key3":{name:"Times New Roman",callback:function(a,
b){return changeCSSinMenu(c,a,b,"font-family",'"Times New Roman", Times, serif')}},"fold2-key4":{name:"Arial",callback:function(a,b){return changeCSSinMenu(c,a,b,"font-family",'"Arial", Helvetica, sans-serif')}},"fold2-key5":{name:"Tahoma",callback:function(a,b){return changeCSSinMenu(c,a,b,"font-family","Tahoma, Geneva, sans-serif")}},"fold2-key6":{name:"Comic Sans",callback:function(a,b){return changeCSSinMenu(c,a,b,"font-family",'"Comic Sans MS", cursive, sans-serif')}}}},"font-size":{name:"Size",
callback:function(a,b){var f=prompt("Enter font size","14");return 50>=f&&0<=f?changeCSSinMenu(c,a,b,"font-size",f+"px"):!1}},"font-color":{name:"Colour",items:{Red:{name:"Black",callback:function(a,b){return changeCSSinMenu(c,a,b,"color","Black")}},Gray:{name:"Gray",callback:function(a,b){return changeCSSinMenu(c,a,b,"color","Gray")}},White:{name:"White",callback:function(a,b){return changeCSSinMenu(c,a,b,"color","White")}},Primary:{name:"Primary",callback:function(a,b){return changeCSSinMenu(c,
a,b,"color",primary_colour)}},Secondary:{name:"Secondary",callback:function(a,b){return changeCSSinMenu(c,a,b,"color",secondary_colour)}}}}}},background:{name:"Background",items:{Colour:{name:"Background Colour",items:{Gray:{name:"Gray",callback:function(a,b){return changeCSSinMenu(c,a,b,"background-color","Gray")}},White:{name:"White",callback:function(a,b){return changeCSSinMenu(c,a,b,"background-color","White")}},Primary:{name:"Primary",callback:function(a,b){return changeCSSinMenu(c,a,b,"background-color",
primary_colour)}},Secondary:{name:"Secondary",callback:function(a,b){return changeCSSinMenu(c,a,b,"background-color",secondary_colour)}}}},Opacity:{name:"Opacity",callback:function(a,b){var f=prompt("Enter an opacity from 0-100","100");if(100>=f&&0<=f){var f=f/100,g=b.$trigger.parent().siblings(c).css("background-color"),g=-1==g.indexOf("a")?g.replace(")",", "+f+")").replace("rgb","rgba"):g.substr(0,g.lastIndexOf(",")).concat(", "+f+")");console.log("Current colour: ",g);b.$trigger.parent().siblings(c).css("background-color",
g)}return!1}}}},border:{name:"Border",items:{Style:{name:"Style",items:{None:{name:"None",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","none")}},Solid:{name:"Solid",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","solid")}},Double:{name:"Double",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","double")}},Dashed:{name:"Dashed",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","dashed")}},Dotted:{name:"Dotted",callback:function(a,
b){return changeCSSinMenu(c,a,b,"border-style","dotted")}},Button:{name:"Button",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","outset")}},Ridge:{name:"Ridge",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","ridge")}}}},Colour:{name:"Colour",items:{Red:{name:"Black",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-color","Black")}},Gray:{name:"Gray",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-color","Gray")}},Primary:{name:"Primary",
callback:function(a,b){return changeCSSinMenu(c,a,b,"border-color",primary_colour)}},Secondary:{name:"Secondary",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-color",secondary_colour)}}}},Corners:{name:"Rounded Corners",callback:function(a,b){var f=prompt("Enter a number from 0 (square)  to 12 (fully rounded)","5");return 12>=f&&0<=f?changeCSSinMenu(c,a,b,"border-radius",f+"px"):!1}},Thickness:{name:"Border Width",callback:function(a,b){var f=prompt("Enter a number from 0 up","2");return 100>=
f&&0<=f?changeCSSinMenu(c,a,b,"border-width",f+"px"):!1}}}},position:zIndex_menu_entry(key,options),sep2:"---------",clone:{name:"Clone",callback:function(a,b){}},template:{name:"Templates",items:{save:{name:"Save As",callback:function(a,e){for(var f=0;void 0!==b["template"+template_ID]&&100>f;)template_ID++,f++,console.log("conflict with template IDs");100>f&&(f=prompt("Enter name for template"),b["template"+template_ID]={name:f,savedhtml:e.$trigger.parent().parent().children(c)[0].outerHTML,callback:function(a,
d){if("Load"==d.$selected.parent().siblings("span").html()){console.log("Loading: "+a);var e=d.$trigger.parent().parent().children(c),f=e.html();e[0].outerHTML=b[a].savedhtml;d.$trigger.parent().parent().children(c).html(f)}else"Delete"==d.$selected.parent().siblings("span").html()&&delete b[a]}},template_ID++)}},load:{name:"Load",items:b},"delete":{name:"Delete",items:b}}},"delete":{name:"Delete",icon:"delete",callback:function(b,c){if(confirm("Are you sure you want to delete this container?")){var f=
c.$trigger.parent().attr("id").split(a)[1];removeContainer(a,f)}}}}};if("img"==a)return{items:{URL:{name:"Change URL",callback:function(a,b){var f=prompt("Enter new URL for image/video","http://"),g=checkImageURL(f,"");$.ajax({url:f,success:function(a){a=$(c).parent();$(c).remove();a.append(g)},error:function(a){alert("URL: "+f+" does not exist")}})}},position:zIndex_menu_entry(key,options)}}}
function bringEleToFront(a){var b=0;$("#pagecontainers").children("div").each(function(){var a=$(this).css("zIndex");a>b&&(b=a)});b++;console.log("Set zIndex to ",b);a.css("zIndex",b)}
function zIndex_menu_entry(a,b){return{name:"Position",items:{Front:{name:"Bring to Front",callback:function(a,b){var e=0;$("#pagecontainers").children("div").each(function(){var a=$(this).css("zIndex");a>e&&(e=a)});e++;console.log("Set zIndex to ",e);b.$trigger.parent().parent().css("zIndex",e)}},Back:{name:"Send to Back",callback:function(a,b){var e=1E4;$("#pagecontainers").children("div").each(function(){var a=$(this).css("zIndex");a<e&&(e=a)});e--;0>e&&(e=0);console.log("Set zIndex to ",e);b.$trigger.parent().parent().css("zIndex",
e)}}}}}
$(function(){$.contextMenu({selector:".textmenu",trigger:"left",build:function(a,b){return generateContextMenu("text",text_template_menu_list)}});$.contextMenu({selector:".decmenu",trigger:"left",build:function(a,b){return generateContextMenu("decision",decision_template_menu_list)}});$.contextMenu({selector:".controlmenu",trigger:"left",build:function(a,b){return generateContextMenu("output",text_template_menu_list)}});$.contextMenu({selector:".imgmenu",trigger:"left",build:function(a,b){return generateContextMenu("img",image_template_menu_list)}})});var dragDrop={};grid_snapping_mode=!1;var grid_targets=interact.createSnapGrid({x:10,y:10,range:10,offset:{x:-2,y:2}});function initSnapOptions(){return grid_snapping_mode?{targets:[grid_targets],relativePoints:[{x:1,y:1}],endOnly:!1}:{targets:[],relativePoints:[{x:1,y:1}],endOnly:!1}}var snap_options=initSnapOptions();
...
a.rect.width,$("#pagecontainers").width())+"px";b.style.height=checkBounds(b.parentNode.getAttribute("data-y"),a.rect.height,$("#pagecontainers").height())+"px"})}setInteractions();function toggleGridMode(){(grid_snapping_mode=!grid_snapping_mode)?$(".gridmode").html("Grid: Enabled"):$(".gridmode").html("Grid: Disabled");$(".gridmode").toggleClass("activebutton");snap_options=initSnapOptions();setInteractions()}
function checkBounds(a,b,c){a=parseFloat(a)||0;console.log("Check "+a+" + "+b);a+b>c?(console.log("Greater than "+c),a=c-b):console.log("Less than "+c);return a}function dragMoveListener(a){var b=a.target,c=(parseFloat(b.getAttribute("data-x"))||0)+a.dx;a=(parseFloat(b.getAttribute("data-y"))||0)+a.dy;b.style.webkitTransform=b.style.transform="translate("+c+"px, "+a+"px)";b.setAttribute("data-x",c);b.setAttribute("data-y",a)}window.dragMoveListener=dragMoveListener;var doit;
window.onresize=function(){clearTimeout(doit);doit=setTimeout(modalBounds,300)};function modalBounds(){$("#page-modal").hasClass("open")&&$(".drag-element").each(function(){$(this)})};var pageOverlay={},show_handles=!0;function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}function toggleHandles(){show_handles=!show_handles;$(".handlemode").toggleClass("activebutton");show_handles?($(".handle").show(),$(".handlemode").html("Handles: Shown")):($(".handle").hide(),$(".handlemode").html("Handles: Hidden"))}
...
$(".handle").hide()}
function addImageContainer(){var a,b=prompt("Enter image url","http://"),c=genPageCenterHTML(300,220);null!=b&&(a="<div class='img-container drag-element' style='position:absolute; "+c+"'>",a=checkImageURL(b,a),a+="</div>",$.ajax({url:b,success:function(b){b=$(".img-container").length;var c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",b+1));bringContainerToFront($("pagecontainers div.img-container:last"));show_handles||$(".handle").hide()},
error:function(a){alert("URL: "+b+" does not exist")}}))}
function checkImageURL(a,b){if(null!=a.match(/\.(jpeg|jpg|gif|png)$/))b+="<img class='editdiv resize-element' src="+a+"></img>";else if(null!=a.match(/\.(webm)$/))b=b+"<video preload='auto' autoplay='autoplay' loop='loop' class='editdiv resize-element'>"+('<source src= "'+a+"\"type='video/webm'></source>"),b+="</video>";else if(null!=a.match(/\.(gifv|mp4)$/)){var c=a.lastIndexOf(".gifv");-1!=c&&(a=a.substr(0,c)+".mp4",console.log("Regexed to: ",a));b+="<video preload='auto' autoplay='autoplay' loop='loop' class='editdiv resize-element'>";
...
function genHandleHTML(a,b){var c;if("img"==a)c="<div id = 'img"+b+"'class = 'handle'>Image Container "+b+'<a style="float:right" class="imgmenu btn-floating btn waves-effect waves-light red"><i class="material-icons">settings</i></a>';else if("text"==a)c="<div id = 'text"+b+"'class = 'handle'>Text Container "+b+'<a style="float:right" class="textmenu btn-floating btn waves-effect waves-light red"><i class="material-icons">settings</i></a>',c+="</div>";else if("decision"==a)c="<div id = 'decision"+
b+"'class = 'handle'>Link "+b+'<a style="float:right" class="decmenu btn-floating btn waves-effect waves-light red"><i class="material-icons">settings</i></a>',c+="</div>";else if("output"==a)c="<div id = 'output"+b+'\'class = \'handle\'>Control Output<a style="float:right" class="controlmenu btn-floating btn waves-effect waves-light red"><i class="material-icons">settings</i></a>',c+="</div>";else return console.log("Unknown container type when generating handle for HTML"),null;return c}
...
function layout_driver(a){if("none"!=a){var b=null;"Tree"==a?b=breadth_first_layout:"Grid"==a?b=grid_layout:"Circle"==a?b=concentric_layout:"Spread"==a&&(b=cose_layout);null!==b?change_layout(b):console.log("No layout selected")}}function change_layout(a){"breadthfirst"==a.name&&(console.log("Set root"),a.roots=cy.$(".start"));cy.layout(a)};var states={DEFAULT:0,CONNECTING:1,NEWPAGE:2,NEWCONTROL:3};current_state=states.DEFAULT;function defaultState(){exitStates();current_state=states.DEFAULT}function exitStates(){current_state==states.CONNECTING&&(cy.boxSelectionEnabled(!0),null!==source_node&&(source_node.removeClass("source_node"),source_node=null));$("#sidebar .btn").removeClass("activebutton");$(".pagemode").html("Page Node")}
...
;var ui={};function removeElement(){element=cy.$(":selected");element.empty()||(element.isNode()&&(0<element.connectedEdges().size()&&element.connectedEdges().forEach(function(a){a.hasClass("controledge")&&removeEdgeFromPriorityList(a)}),cleanup_node_labels(element)),element.isEdge()&&(element.hasClass("controledge")&&removeEdgeFromPriorityList(element),cleanup_edge_labels(element),style_end_nodes()));$(".editbutton").hide();$(".selectionbutton").hide()}
...
(b+="pageedge");c=String.fromCharCode("A".charCodeAt()+source_node.edgesTo("*").size());c=cy.add({data:{name:c,source:source_node.data("id"),target:a.data("id"),text:"<Decision text to display)>",conditions:[],outcomes:[]},classes:b,group:"edges"});c.on("tap",function(a){this.select()});source_node.hasClass("control")&&(source_node.json().data.priorityList.push(c.json().data.id),console.log("control nodes defaultFailEdge: "+source_node.json().data.defaultFailEdge),"none"===source_node.json().data.defaultFailEdge&&
(console.log("setting initial default fail edge to "+c.json().data.id),source_node.json({data:{defaultFailEdge:c.json().data.id}}),console.log("default fail edge is "+source_node.json().data.defaultFailEdge)));console.log(cy.elements().jsons());source_node.removeClass("source_node");source_node=null;a.unselect();style_end_nodes()}}
...
var observer=new MutationObserver(function(a){a.forEach(function(a){(a.oldvalue="block"==a.target.style.display)&&cy.resize()})}),cytabNode=$("#cyTab")[0],observerConfig={attributes:!0,childList:!1,characterData:!1,attributeOldValue:!0,attributeFilter:["style"]};observer.observe(cytabNode,observerConfig);var pageTemplates={},page_templates={Default:{outputcontainer:"",imgcontainers:[],audio:"none",textcontainers:[],decisioncontainers:[]}},selected_page_template=page_templates.Default;function createPageTemplate(){var a=prompt("Enter name for page template");if("Default"===a)alert("Can't override Default");else{var b=cy.$(":selected")[0].data();page_templates[a]=b;$("#pagetemplates").append("<li><a onclick=\"chooseNodeTemplate('"+a+"');\">"+a+"</a></li>")}}
function chooseNodeTemplate(a){selected_page_template=page_templates[a];$(".pagemode").html("Page Node ("+a+")");console.log("Select template: "+a)};var canvasEvents={};console.log("Enter canvasEvents.js");source_node=null;
cy.on("tap",function(a){var b=a.cyTarget;b===cy&&cy.$(":selected").unselect();current_state!==states.CONNECTING||b!==source_node&&b!==cy||null===source_node||(source_node.removeClass("source_node"),source_node=null);current_state===states.NEWPAGE?(console.log("Add a node"),b===cy&&console.log("Really Add a node"+cy.add({data:{name:cy.nodes().size()+1,outputcontainer:selected_page_template.outputcontainer,imgcontainers:selected_page_template.imgcontainers,audio:selected_page_template.audio,textcontainers:selected_page_template.textcontainers,
decisioncontainers:[]},classes:"page",group:"nodes",renderedPosition:a.cyRenderedPosition}).data()),1===cy.elements().size()&&cy.$("node").first().addClass("start")):current_state===states.NEWCONTROL&&b===cy&&cy.add({data:{name:cy.nodes().size()+1,text:"control node text",priorityList:[],defaultFailEdge:"none"},classes:"control",group:"nodes",renderedPosition:a.cyRenderedPosition})});
...
cy.on("unselect",function(a){a.cyTarget.isNode()&&a.cyTarget.outgoers().removeClass("parent-selected");console.log("Unselect event fired ",a.cyTarget.data("id"));0===cy.$(":selected").size()?($(".editbutton").hide(),$(".selectionbutton").hide()):showOverlayLinks(cy.$(":selected")[0])});var playGame={};outgoingEdges=currentNode=null;function prepareForGame(){outgoingEdges=currentNode=null;$(".playpage").html("");for(var a=cy.elements(),b=0;b<a.length;b++)updatePageStyle(a[b])}function wipeGame(){$(".playpage").html("")}function parseNode(){console.log("Parse node ",currentNode.data("id"));outgoingEdges=currentNode.outgoers().edges();currentNode.hasClass("page")?parsePage(outgoingEdges):currentNode.hasClass("control")&&parseControl(currentNode,outgoingEdges)}
function parsePage(a){stylePage()}
function stylePage(){$(".playpage").html("");for(var a=currentNode.data("textcontainers"),b=currentNode.data("decisioncontainers"),c=currentNode.data("imgcontainers"),d=currentNode.data("outputcontainer"),e=0;e<a.length;e++)$(".playpage").append(a[e].html);for(e=0;e<c.length;e++)$(".playpage").append(c[e].html);for(e=0;e<b.length;e++)$(".playpage").append(b[e].html);$(".playpage").append(d);$(".playpage").children("div[class^='decision-container']").each(function(a){$(this).click(function(){progressStory(a)})});$(".playpage .handle").hide();
$(".playpage").children().removeClass("resize-element");$(".playpage").children().children().removeClass("resize-element");$(".playpage").children().attr("contenteditable","false");$(".playpage").children().children().attr("contenteditable","false")}
