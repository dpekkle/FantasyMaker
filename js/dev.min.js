...
removeElement(),current_state=states.DEFAULT,$(a).removeClass("activebutton"))};var pageOverlay={};function prepareForGame(){for(var a=cy.elements(),b=0;b<a.length;b++)showPageOverlay(a[b]),closeOverlay(a[b])}function addEdgeToPageOverlay(a){$("#pagecontainers").append("<button class = 'decisionbutton drag-element' id='decision"+i+"'>"+escapeHtml(outgoingEdges.eq(i).data("text"))+"</button><br>")}
function showPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$(".toolbar").hide();$("#modalcontainers").children().hide();overlayToolbar(b);if(b.hasClass("page")){$("#pagecontainers").show();$("#pagecontainers").html("");for(var c=cy.$(":selected")[0].data("textcontainers"),d=0;d<c.length;d++)$("#pagecontainers").append(c[d].html),$("#text-area"+d).val(escapeHtml(c[d].contents)),$("#text-area"+d).on("input",function(a){a=this.value;console.log("Text: ",a);cy.$(":selected").data("textcontainers")[d].contents=
a}),console.log("Display:",escapeHtml(c[d].contents));outgoingEdges=b.outgoers().edges();for(d=0;d<outgoingEdges.size();d++)$("#pagecontainers").append("<button class = 'decisionbutton drag-element' id='decision"+d+"'>"+escapeHtml(outgoingEdges.eq(d).data("text"))+"</button><br>")}b.isEdge()&&($("#connectioncontainers").show(),$("#connectioncontainers #decisiontext").val(escapeHtml(b.data("text"))));b.hasClass("control")&&($("#controlcontainers").show(),$("#controlcontainers #controltext").val(escapeHtml(b.data("text"))));
