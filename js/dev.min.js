...
b,c,d,e){var f=$("#prompt-modal .modal-content");this.verifyFunction=void 0!==e?e:null;this.callbackFunction=d;$("#prompt-modal").openModal({dismissible:!1,ready:function(){f.html("");f.append('<h3 style="text-align:center;">'+a+"</h3>");f.append("<p>"+b+"<p>");for(var d=0;d<c.length;d++)f.append('<label for="'+d+'"> '+c[d].name+"</label>"),"number"==c[d].type?f.append('<input id= "'+d+'"type="'+c[d].type+'" value = "'+parseInt(c[d]["default"])+'" min ="'+c[d].min+'" max ="'+c[d].max+'">'):f.append('<input id= "'+
...
function getNextOutcomeID(a){for(var b=-1,c=0;c<a.data.outcomes.length;c++)a.data.outcomes[c].id>b&&(b=a.data.outcomes[c].id);return++b};var attributesOverlay={},aattributeCount=0,aattID=0;function aattributesOverlay_openAttributesOverlay(){}
function aattributesOverlay_addAttribute(a){var b='<li id="att-'+attID+'">  <div class="collapsible-header" id="att-'+attID+'-header">Attribute Name</div> <div class="collapsible-body"> <br/> <div class="row"> <div class="input-field col s4"> <input id="att-'+attID+'-name" type="text" class="validate"> <label for="att-'+attID+'-name">Name</label> </div> <div class="input-field col s2"> <input id="att-max-val" type="number"/> <label for="att-max-val">Max Value</label> </div> <div class="input-field col s2"> <input id="att-init-value" type="number"/> <label for="att-init-value">Initial Value</label></div>  <button class="btn btn-default" id="att-'+attID+
'" onclick="attributeOverlay_updateAttribute(this.id)">Update</button> </div> </div> </li>';$("#"+a).append(b);attributeCount++;attID++}function aattributeOverlay_updateAttribute(a){var b=$("#"+a+"-name").val();$("#"+a+"-header").html(b)};var events={},selected_event=null;function deleteEvent(){null!=selected_event&&$(selected_event).remove()}
