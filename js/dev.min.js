...
"\"type='video/webm'></source>",a+="</video></div>";else{alert("Not a valid url");return}var b=cy.$(":selected")[0].data("imgcontainers").length,c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div#img-container:last").prepend("<div class='handle'>Image Container "+(b+1)+"</div>");c=cy.$(":selected")[0].data("imgcontainers");c.push({name:b+1,html:a});cy.$(":selected")[0].data("imgcontainers",c)}}function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}
...
c[a].name+"</div>"),e=!0);e||c.splice(a,1)}}}function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}function openEditControlOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$("#controlcontainers #controltext").val(escapeHtml(b.data("text")))}
...
function closeOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);b.hasClass("page")&&($("#pagecontainers .handle").remove(),$("#pagecontainers").children("div[id^='text-container']").each(function(a){var d=this.outerHTML;b.data("textcontainers")[a].html=d;console.log("Updating HTML for ",a)}),$("#pagecontainers").children("div[id^='decision-container']").each(function(a){var d=this.outerHTML;b.data("decisioncontainers")[a].html=d;console.log("save html for decision ",a)}),$("#pagecontainers").children("div[id^='img-container']").each(function(a){var d=
this.outerHTML;b.data("imgcontainers")[a].html=d;console.log("save html for img ",a)}),$("#pagecontainers").html(""));b.isEdge()&&(saveEdge(),$("#conditionsList").children().remove(),$("#outcomeList").children().remove())}
...
function modalBounds(){$("#page-modal").hasClass("open")&&$(".resize-element").each(function(){console.log("Test resize");var a=$(this);$(this).width(checkBounds(a.parent().attr("data-x"),a.width(),$("#pagecontainers").width()));$(this).height(checkBounds(a.parent().attr("data-y"),a.height(),$("#pagecontainers").height()-$(".handle").height()))})};var edgeOverlay={},edgeOverlay_conditionCount=0,edgeOverlay_outcomeCount=0,edgeOverlay_selectedEdge;
...
$(".playpage").children().children().removeClass("resize-element");$(".playpage").children().attr("contenteditable","false");$(".playpage").children().children().attr("contenteditable","false")}function parseControl(a){}function progressStory(a){null===currentNode?(currentNode=cy.$(".start")[0],parseNode()):0<currentNode.outgoers().size()?(currentNode=outgoingEdges.eq(a).target(),console.log("Now on node ",currentNode.data("id")),parseNode()):(currentNode=null,$(".playpage").html(""))}
function stripDraggable(a){return a.replace(/drag-element/g,"")};console.log("Enter main.js version 0.5.0");$("select").material_select();var testgraph={};function checkValidGraph(){var a="";cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
