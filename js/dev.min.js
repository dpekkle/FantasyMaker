...
function closeOverlay(a){null===a&&document.getElementById("Modal").close();var b=a;null===a&&(b=cy.$(":selected")[0]);b.hasClass("page")&&$("#pagecontainers").children("div[id^='text-container']").each(function(a){var d=this.outerHTML;b.data("textcontainers")[a].html=d;console.log("Updating HTML for ",a)});b.isEdge()&&(saveEdge(),$("#conditionsList").children().remove(),$("#outcomeList").children().remove())}
function escapeHtml(a){var b=document.createElement("div");b.appendChild(document.createTextNode(a));return b.innerHTML};var ui={};function removeElement(){element=cy.$(":selected");element.empty()||(cy.remove(element),total_pages--,cleanup_titles(),cy.$("node").first().addClass("start"))}
