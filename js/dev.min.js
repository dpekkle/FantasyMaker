...
removeElement(),current_state=states.DEFAULT,$(a).removeClass("activebutton"))};var pageOverlay={};function showPageOverlay(){var a=cy.$(":selected")[0];$(".toolbar").hide();$("#modalcontainers").children().hide();overlayToolbar(a);if(a.hasClass("page"))for($("#pagecontainers").show(),outgoingEdges=a.outgoers().edges(),a=0;a<outgoingEdges.size();a++)$("#pagecontainers").append("<button class = 'decisionbutton' id='decision"+a+"'>"+escapeHtml(outgoingEdges.eq(a).data("text"))+"</button>");document.getElementById("Modal").showModal()}
function overlayToolbar(a){1===cy.$(":selected").size()&&(a.hasClass("control")?($("#controltoolbar").show(),$("#controlname").text("control "+a.data("id")),document.getElementById("controltext").value=a.data("text")):a.hasClass("page")?($("#pagetoolbar").show(),$("#pagename").text("Page "+a.data("id")),document.getElementById("pagetext").value=a.data("text"),changeImage(),changeAudio()):a.isEdge()&&($("#connectiontoolbar").show(),$("#connectionname").text("Connection"),document.getElementById("connectiontext").value=
a.data("text")))}function closeOverlay(){document.getElementById("Modal").close();var a=cy.$(":selected")[0];a.hasClass("page")&&(a.data("styleHTML",$("#pagecontainers").html()),console.log(a.data("styleHTML")),$(".decisionbutton").remove())}function escapeHtml(a){var b=document.createElement("div");b.appendChild(document.createTextNode(a));return b.innerHTML};var ui={};$(".textarea").on("input",function(a){a=this.value;cy.$(":selected").data("text",a)});function removeElement(){element=cy.$(":selected");!element.empty()&&confirm("Are you sure you want to delete this element and all it's links?")&&(cy.remove(element),hideEditPanes(),cy.$("node").first().addClass("start"),http_delete(element.connectedEdges().jsons()),http_delete(element.jsons()),cy.remove(element),hideEditPanes());cy.$("node").first().addClass("start")}
...
function stylePage(){var a=currentNode.data("styleHTML");$(".playpage").html(a);a.includes("pagetext")&&($(".playpage #pagetext").html(escapeHtml(currentNode.data("text"))),console.log(currentNode.data("text")));if(a.includes("decisiontext"))for(a=0;a<outgoingEdges.size();a++)$(".playpage #decisiontext").html(escapeHtml(outgoingEdges.eq(a).data("text"))),console.log("Decision 1: ",outgoingEdges.eq(a).data("text"))}function parseControl(a){}
function progressStory(){null===currentNode?(currentNode=cy.$(".start")[0],parseNode()):0<currentNode.outgoers().size()&&(currentNode=outgoingEdges.eq(0).target(),console.log("Now on node ",currentNode.data("id")),parseNode())};console.log("Enter main.js version 0.5.0");var dialog=document.querySelector("dialog");dialogPolyfill.registerDialog(dialog);var testgraph={};function checkValidGraph(){var a="";cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
