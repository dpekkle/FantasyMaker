...
;var project={};console.log("Entering project.js");var project_project=project_createNewProject();project_updateProject();defaultState();project_updateProject();
function project_createNewProject(){return{project_templates:{Default:{pagestyle:"width:800px; height:600px; \tborder: 3px solid #BBBBBB",outputcontainer:"",imgcontainers:[],textcontainers:[],decisioncontainers:[]}},template_menus:new templateMenuObj,audio:new audioObj,projectOwner:"none",projectName:"none",graph:[],gameAttributes:{},attributesHTML:"",resolution:{x:1E3,y:800}}}
...
"' class='audioevent eventscontainer'><span class = 'eventname flex-center-vertically'>"+this.name+"</span><span class = 'eventtype flex-center-vertically'><select><option value='Play'>Play</option><option value='Stop'>Stop</option></select></span><span class = 'eventtrigger'><input type='number' min = 0 value = 0></span></div>");$(".eventscontainer").last().on("click",".eventname",function(a){a.preventDefault();$(".eventscontainer").removeClass("highlighted");$(this).parent().toggleClass("highlighted");
selected_event=$(this).parent();console.log(selected_event)});$(".eventtype select").material_select()}}
function audioObj(){this.selected_audio=null;this.unique_id=0;this.assets=[];this.getAssetAsModalList=function(a){var b="";if(void 0!=a)a=this.assets[a],b+="<li id='"+a.id+"''>Name: "+a.name+"<br>Link: "+a.link+"</li>";else for(var c=0;c<this.assets.length;c++)a=this.assets[c],b+="<li id='"+a.id+"''>Name: "+a.name+"<br>Link: "+a.link+"</li>";return b};this.getAssetAsMenu=function(){assetlist={};console.log("Generate audio menu of length: ",this.assets.length);for(var a=0;a<this.assets.length;a++)assetlist[a]=
{name:this.assets[a].name,id:this.assets[a].id,callback:function(a,c){project_project.audio.assets[a].addEvent()}};return assetlist};this.getAsset=function(a){return void 0!==a?0<=a?this.assets.find(function(b){return b.id===a}):!1:this.assets};this.removeAsset=function(){if(null!=this.selected_audio){var a=this.getAsset(parseInt(this.selected_audio));this.assets.splice(this.assets.indexOf(a),1);$("#audiolist").find("#"+this.selected_audio).remove()}};this.addAssetButton=function(){var a=prompt("Enter youtube link");
if(null==a)console.log("Link cannot be empty");else{var b=prompt("Give audio a name");null==b?console.log("Name cannot be empty"):(this.addAsset(b,a),$("#audiolist li").on("click",function(a){a.preventDefault();$("#audiolist li").removeClass("highlighted");$(this).toggleClass("highlighted");project_project.audio.selected_audio=$(this).attr("id")}))}};this.addAsset=function(a,b){var c=this.parseType(b);"error"!=c?(this.assets[this.assets.length]=new audioAsset(a,this.unique_id,b,c),$("#audiolist").html(""),
$("#audiolist").append(this.getAssetAsModalList()),this.unique_id++):console.log("Failed to add asset")};this.parseType=function(a){return a.match(/^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/)&&RegExp.$1?"youtube":"error"};this.addAudioEvent=function(a){this.assets[a].addEvent()}}
function onYouTubePlayerAPIReady(){$(document).ready(function(){audioplayer=new YT.Player("audioplayer",{origin:"https://www.youtube.com",height:"100",width:"100",events:{onReady:onPlayerReady,onStateChange:onPlayerStateChange,onError:onPlayerError}})})}function onPlayerReady(a){alert("started video")}function onPlayerStateChange(a){-1===a.data&&alert("youtube video not started");5===a.data&&alert("youtube video queued");1===a.data&&alert("now playing");0===a.data&&alert("done")}
...
function openEditPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null===a&&overlayToolbar(b);if(b.hasClass("page")){$("#pagename").html("Design Page: "+b.data("name"));$("#pagecontainers").append('<a style="float:right" class="pagemenu btn-floating waves-effect waves-light gray"><i class="material-icons">settings</i></a>');a=b.data("pagestyle");$("#pagecontainers").attr("style",a);a=b.data("outputcontainer");$("#pagecontainers").append(a);$("#pagecontainers div.output-container:last").prepend(genHandleHTML("output",
...
a=b.data("events");for(c=0;c<a.length;c++)project_project.audio.getAsset(parseInt(a[c].id))&&project_project.audio.getAsset(parseInt(a[c].id)).addEvent(),$(".eventtrigger input").last().val(a[c].trigger),$(".eventtype select").last().val(a[c].action),$("#eventspane select").last().material_select();outgoingEdges=b.outgoers().edges();for(var d=b.data("decisioncontainers"),c=0;c<outgoingEdges.size();c++){var e=!1;for(a=0;a<d.length;a++)outgoingEdges[c].data("name")==d[a].name&&(e=!0);e||addDecisionContainer(b,
c,outgoingEdges.eq(c).data("text"),outgoingEdges[c].data("name"))}for(a=0;a<d.length;a++){e=!1;for(c=0;c<outgoingEdges.size();c++)d[a].name==outgoingEdges[c].data("name")&&($("#pagecontainers").append(d[a].html),$("#pagecontainers div.decision-container:last").prepend(genHandleHTML("decision",d[a].name)),e=!0);e||d.splice(a,1)}show_handles||$(".handle").hide();bindHandleSelection()}}function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}
...
(saveControl(b),$("#connectedEdgesList").children().remove());b.isEdge()&&(saveEdge(b,"EDGE_OVERLAY"),$("#conditionsList").children().remove(),$("#outcomesList").children().remove())}}
...
function templateMenuObj(){this.template_ID=0;this.text_template_menu_list={};this.decision_template_menu_list={};this.image_template_menu_list={};this.page_template_menu_list={}}
...
$(c).css("border-style",this.data().Style);null!=this.data().resolution&&($(c).css("flex","0 0 "+this.data().resolution.split("a")[1]),$(c).css("height",this.data().resolution.split("a")[2]))}}}}
...
function genHandleHTML(a,b){var c;c="<div class = 'handlecontainer'>";if("img"==a)c=c+'<a class="btn-floating waves-effect waves-light green lefticon"><i class="fa fa-picture-o"></i></a>'+("<div id = 'img"+b+"'class = 'handle img-handle'>Image"),c+="</div>",c+='<a class="imgmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else if("text"==a)c+='<a class="btn-floating waves-effect waves-light grey lefticon"><i class="material-icons">comment</i></a>',
c+="<div id = 'text"+b+"'class = 'handle text-handle'>Text",c+="</div>",c+='<a class="textmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else if("decision"==a)c+='<a class="btn-floating waves-effect waves-light blue lighten-3 lefticon">'+b+"</a>",c+="<div id = 'decision"+b+"'class = 'handle link-handle'>Decision",c+="</div>",c+='<a class="decmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';
else if("output"==a)c+='<a class="btn-floating waves-effect waves-light indigo lighten-1 lefticon"><i class="fa fa-terminal"></i></a>',c+="<div id = 'output"+b+"'class = 'handle control-handle'>Output",c+="</div>",c+='<a class="controlmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else return console.log("Unknown container type when generating handle for HTML"),null;c+="</div>";console.log(c);return c}
...
var k=$("#temp_name_edit_input"),p=$("#temp_name_edit_input_label"),h=$("#temp_init_val_edit_input"),q=$("#temp_init_val_edit_input_label"),l=$("#temp_max_val_edit_input"),r=$("#temp_max_val_edit_input_label"),m=$("#temp_update_button"),n=$("#temp_delete_button");console.log(e.name);f.text(e.name);f.attr("id",""+e+"-display-name");g.text(e.value);g.id=e+"-display-init-val";p.attr("for",""+e+"-edit-name-input");k.data("path",e.path);k.id=e+"-edit-name-input";q.attr("for",""+e+"-edit-init-input");h.data("path",
e.path);h.id=e+"-edit-init-input";r.attr("for",""+e+"-edit-max-input");l.data("path",e.path);l.id=e+"-edit-max-input";m.data("path",e.path);m.id=e+"-update";n.data("path",e.path);n.id=e+"-delete"}}}function generateID(){for(var a="",b=0;4>b;b++)a+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(62*Math.random()));return a}function saveListHTML(){project_project.attributeHTML=$("#attributes-list").html()}
...
$(".playpage .handlecontainer").hide();$(".playpage").children().removeClass("resize-element");$(".playpage").children().children().removeClass("resize-element");$(".playpage").children().attr("contenteditable","false");$(".playpage").children().children().attr("contenteditable","false");for(f=0;f<e.length;f++);}function parseControl(a,b){progressStory(0)}function getIndexFromOutgoingEdges(a,b){for(var c=b.jsons(),d=0;d<c.length;d++)if(c[d].data.id===a)return d;return-1}
