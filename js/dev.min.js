...
function project_createNewProject(){return{project_templates:{Default:{pagestyle:"width:800px; height:600px; \tborder: 3px solid #BBBBBB",outputcontainer:"",imgcontainers:[],textcontainers:[],decisioncontainers:[]}},audio:new audioObj,projectOwner:"none",projectName:"none",graph:[],gameAttributes:{},attributesHTML:"",resolution:{x:1E3,y:800}}}
...
if(null==a)console.log("Link cannot be empty");else{var b=prompt("Give audio a name");null==b?console.log("Name cannot be empty"):(this.addAsset(b,a),$("#audiolist li").on("click",function(a){a.preventDefault();$("#audiolist li").removeClass("highlighted");$(this).toggleClass("highlighted");project_project.audio.selected_audio=$(this).attr("id")}))}};this.addAsset=function(a,b){var c=this.parseType(b);"error"!=c?(this.assets[this.size]=new audioAsset(a,this.size,b,c),this.size++,$("#audiolist").html(""),
$("#audiolist").append(this.getAssetAsModalList())):console.log("Failed to add asset")};this.parseType=function(a){return a.match(/^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/)&&RegExp.$1?"youtube":"error"};this.addAudioEvent=function(a){this.assets[a].addEvent()}}testAudioObjects();
function testAudioObjects(){project_project.audio.addAsset("Reggae Rap","https://www.youtube.com/watch?v=4vKmEmY8ZD8");project_project.audio.addAsset("Fantasy RPG","https://www.youtube.com/watch?v=Pq824AM9ZHQ");project_project.audio.addAsset("Missing You","https://www.youtube.com/watch?v=Ct_t92NloA8");project_project.audio.addAsset("Electronic","https://www.youtube.com/watch?v=MiRcFl8iafg");console.log(project_project.audio.getAsset(0));console.log(project_project.audio.getAsset(1));console.log(project_project.audio.getAsset());
console.log(project_project.audio.getAsset("a"));console.log(project_project.audio.getAsset(-1));console.log(project_project.audio.getAsset(2))}function onYouTubePlayerAPIReady(){$(document).ready(function(){audioplayer=new YT.Player("audioplayer",{origin:"dpekkle.github.io/FantasyMaker/create.html",height:"100",width:"100",events:{onReady:onPlayerReady,onStateChange:onPlayerStateChange,onError:onPlayerError}})})}function onPlayerReady(a){alert("started video")}
function onPlayerStateChange(a){-1===a.data&&alert("youtube video not started");5===a.data&&alert("youtube video queued");1===a.data&&alert("now playing");0===a.data&&alert("done")}function onPlayerError(a){alert("Youtube audio playback encountered error: ",a.data,". Embedding of this link may be disabled by the owner.");console.log(a.data)};var httpRequests={};console.log("Enter httpRequests.js");function http_save(){console.log("save");project_updateProject();$.ajax({type:"POST",url:"/saveProject",data:{save:JSON.stringify(project_project)},success:function(a){alert("Graph Saved")},contenttype:"application/json"})}
