...
function removeElement(){element=cy.$(":selected");if(!element.empty()&&confirm("Are you sure you want to delete this element and all it's links?")){hideEditPanes();cy.$("node").first().addClass("start");for(var a=0;a<element.connectedEdges().jsons().length;a++)http_modStack.push(element.connectedEdges().jsons()[a]);for(a=0;a<element.jsons().length;a++)http_modStack.push(element.jsons()[a]);cy.remove(element);hideEditPanes()}}
function createConnection(a){if(current_state==states.CONNECTING&&a.isNode())if(null==source_node)source_node=a,source_node.addClass("source_node"),console.log("Source node assigned as node",a.data("id"));else if(console.log("Creating link from ",source_node.data("id")," to ",a.data("id")),source_node.data("id")!=a.data("id")){var b="",d=!0;if(source_node.hasClass("control")){var c=source_node.edgesTo("*");console.log("Size from control: ",c.size());0===c.size()?b="success-edge":1===c.size()?b=c.first().hasClass("success-edge")?
"fail-edge":"success-edge":d=!1}d&&cy.add({data:{source:source_node.data("id"),target:a.data("id"),text:"<Decision text to display)>"},classes:b,group:"edges"});source_node.removeClass("source_node");source_node=null}}function resizeCanvas(){var a=$(window).width(),b=$(window).height(),d=$("#tabheadings").height();$("#cy").css("width",9*a/12-30);$("#cy").css("height",b-d);$("#sidebar").css("height",b-d);console.log("We resized, width: "+a+" height: "+b);cy.resize()}$(window).resize(resizeCanvas);
