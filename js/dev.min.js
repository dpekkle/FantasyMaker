...
function removeEdgeFromPriorityList(a){var b=cy.nodes("[id='"+a.json().data.source+"']");if(void 0!==b){for(var c=0;c<b.json().data.priorityList.length;c++)b.json().data.priorityList[c]===a.json().data.id&&b.json().data.priorityList.splice(c,1);b.json().data.defaultFailEdge===a.json().data.id&&(0<b.json().data.priorityList.length?b.json({data:{defaultFailEdge:b.json().data.priorityList[0]}}):b.json({data:{defaultFailEdge:"none"}}))}}
...
function retrieveHandleHTML(a,b){var c;if("img"==a)c="<div id = 'img"+b+"'class = 'handle'>Image Container "+b;else if("text"==a)c="<div id = 'text"+b+"'class = 'handle'>Text Container "+b;else{if("decision"==a)return"<div id = 'decision"+b+"'class = 'handle'>Link "+b+"</div>";console.log("Unknown container type when generating handle for HTML");return null}c+='<a style="float:right" class="btn-floating btn waves-effect waves-light red" onclick="removeContainer(\''+a+"','"+b+'\')"><i class="material-icons">delete</i></a>';
return c+="</div>"}function removeContainer(a,b){alert("Are you sure you want to delete"+a+"container"+b+"?");$("#"+a+b).parent().remove()}function addDecisionContainer(a,b,c,d){b="<div class = 'decision-container drag-element' style='position:absolute;'>"+("<div id = 'editdec' class = 'decisionbutton drag-element resize-element' contenteditable=true>"+escapeHtml(c)+"</div>");b+="</div>";c=a.data("decisioncontainers");c.push({name:d,html:b});a.data("decisioncontainers",c)}
function addTextContainer(){var a;a="<div class='text-container drag-element' style='position:absolute;'><div id = 'editdiv' class='resize-element' contenteditable=true ></div></div>";var b=$(".text-container").length;a=htmlToElements(a);$("#pagecontainers").append(a);$("#pagecontainers div.text-container:last").prepend(retrieveHandleHTML("text",b+1))}
function addImageContainer(){var a,b=prompt("Enter image url","http://");if(null!=b){if(null!=b.match(/\.(jpeg|jpg|gif|png)$/))a="<div class='img-container drag-element' style='position:absolute;'>",a+="<img id = 'editdiv' class='resize-element' src="+b+"></img>",a+="</div>";else if(null!=b.match(/\.(webm)$/))a="<div class='img-container drag-element' style='position:absolute;'>",a+="<video preload='auto' autoplay='autoplay' loop='loop' id = 'editdiv' class='resize-element'>",a+='<source src= "'+
b+"\"type='video/webm'></source>",a+="</video></div>";else if(null!=b.match(/\.(gifv|mp4)$/)){var c=b.lastIndexOf(".gifv");-1!=c&&(b=b.substr(0,c)+".mp4",console.log("Regexed to: ",b));a="<div class='img-container drag-element' style='position:absolute;'>";a+="<video preload='auto' autoplay='autoplay' loop='loop' id = 'editdiv' class='resize-element'>";a+='<source src= "'+b+"\"type='video/mp4'></source>";a+="</video></div>"}else{alert("Not a valid url");return}$.ajax({url:b,success:function(b){b=
$(".img-container").length;var c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div.img-container:last").prepend(retrieveHandleHTML("img",b+1))},error:function(a){alert("file does not exist")}})}};var testgraph={};function checkValidGraph(){var a="";cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
