...
function openEditPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null===a&&overlayToolbar(b);if(b.hasClass("page")){var c=b.data("textcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div.text-container:last").prepend(genHandleHTML("text",c[a].name));c=b.data("imgcontainers");for(a=0;a<c.length;a++)$("#pagecontainers").append(c[a].html),$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",c[a].name));outgoingEdges=b.outgoers().edges();
for(var c=b.data("decisioncontainers"),d=0;d<outgoingEdges.size();d++){var e=!1;for(a=0;a<c.length;a++)outgoingEdges[d].data("name")==c[a].name&&(e=!0);e||addDecisionContainer(b,d,outgoingEdges.eq(d).data("text"),outgoingEdges[d].data("name"))}for(a=0;a<c.length;a++){e=!1;for(d=0;d<outgoingEdges.size();d++)c[a].name==outgoingEdges[d].data("name")&&($("#pagecontainers").append(c[a].html),$("#pagecontainers div.decision-container:last").prepend(genHandleHTML("decision",c[a].name)),e=!0);e||c.splice(a,
1)}}}function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}function openEditControlOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$("#controlcontainers #controltext").val(escapeHtml(b.data("text")));populateControlOverlay(b)}
...
function closeOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null!=b&&(b.hasClass("page")&&($("#pagecontainers .handle").remove(),b.data("textcontainers",[]),b.data("imgcontainers",[]),$("#pagecontainers").children("div[class^='text-container']").each(function(a){var d=this.outerHTML;console.log("Updating HTML for ",a);var e=b.data("textcontainers");e.push({name:a+1,html:d});b.data("textcontainers",e)}),$("#pagecontainers").children("div[class^='decision-container']").each(function(a){var d=
this.outerHTML;b.data("decisioncontainers")[a].html=d;console.log("Save HTML for decision ",a)}),$("#pagecontainers").children("div[class^='img-container']").each(function(a){var d=this.outerHTML;console.log("Save HTML for img ",a);var e=b.data("imgcontainers");e.push({name:a+1,html:d});cy.$(":selected")[0].data("imgcontainers",e)}),$("#pagecontainers").html("")),b.hasClass("control")&&(saveControl(b),$("#connectedEdgesList").children().remove()),b.isEdge()&&(saveEdge(b,"EDGE_OVERLAY"),$("#conditionsList").children().remove(),
$("#outcomesList").children().remove()))}function showOverlayLinks(a){$(".editbutton").hide();a.hasClass("page")?$('a[href="#page-modal"]').show():a.hasClass("control")?$('a[href="#control-modal"]').show():a.hasClass("pageedge")?$('a[href="#connection-modal"]').show():a.hasClass("controledge")&&$('a[href="#connection-modal"]').show()}function escapeHtml(a){var b=document.createElement("div");b.appendChild(document.createTextNode(a));return b.innerHTML}
...
$("#"+b).css("transition",""))})};var contextMenu={},picked_colour="#FFF";function changeCSSinMenu(a,b,c,d,e){c.$trigger.parent().siblings(a).css(d,e);return!1}var picked_colour="rgba(255,255,255,1.0)";$("#ColourPicker").spectrum({color:picked_colour,change:function(a){picked_colour=a.toRgbString();$("#ColourPicker").css("background-color",picked_colour)}});var template_ID=0,text_template_menu_list={},decision_template_menu_list={},image_template_menu_list={};
function generateContextMenu(a,b){if("Text"==a||"Control"==a)var c=".editdiv";else"Decision"==a?c=".editdec":"Image"==a&&(c=".editdiv");if("Text"==a||"Decision"==a||"Control"==a)return{items:{font:{name:"Text",items:{Alignment:{name:"Alignment",items:{Left:{name:"Left",callback:function(a,b){return changeCSSinMenu(c,a,b,"text-align","left")}},Right:{name:"Right",callback:function(a,b){return changeCSSinMenu(c,a,b,"text-align","Right")}},Center:{name:"Center",callback:function(a,b){return changeCSSinMenu(c,
...
callback:function(a,b){var f=prompt("Enter font size","14");return 50>=f&&0<=f?changeCSSinMenu(c,a,b,"font-size",f+"px"):!1}},"font-color":{name:"Colour",items:{Red:{name:"Black",callback:function(a,b){return changeCSSinMenu(c,a,b,"color","Black")}},Gray:{name:"Gray",callback:function(a,b){return changeCSSinMenu(c,a,b,"color","Gray")}},White:{name:"White",callback:function(a,b){return changeCSSinMenu(c,a,b,"color","White")}},Custom:{name:"Custom",callback:function(a,b){return changeCSSinMenu(c,a,
b,"color",picked_colour)}}}}}},background:{name:"Background",items:{Colour:{name:"Background Colour",items:{Gray:{name:"Gray",callback:function(a,b){return changeCSSinMenu(c,a,b,"background-color","Gray")}},White:{name:"White",callback:function(a,b){return changeCSSinMenu(c,a,b,"background-color","White")}},Custom:{name:"Custom",callback:function(a,b){return changeCSSinMenu(c,a,b,"background-color",picked_colour)}}}},Opacity:{name:"Opacity",callback:function(a,b){var f=prompt("Enter an opacity from 0-100",
"100");if(100>=f&&0<=f){var f=f/100,g=b.$trigger.parent().siblings(c).css("background-color"),g=-1==g.indexOf("a")?g.replace(")",", "+f+")").replace("rgb","rgba"):g.substr(0,g.lastIndexOf(",")).concat(", "+f+")");console.log("Current colour: ",g);b.$trigger.parent().siblings(c).css("background-color",g)}return!1}}}},border:{name:"Border",items:{Style:{name:"Style",items:{None:{name:"None",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","none")}},Solid:{name:"Solid",callback:function(a,
b){return changeCSSinMenu(c,a,b,"border-style","solid")}},Double:{name:"Double",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","double")}},Dashed:{name:"Dashed",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","dashed")}},Dotted:{name:"Dotted",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","dotted")}},Button:{name:"Button",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-style","outset")}},Ridge:{name:"Ridge",callback:function(a,
b){return changeCSSinMenu(c,a,b,"border-style","ridge")}}}},Colour:{name:"Colour",items:{Red:{name:"Black",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-color","Black")}},Gray:{name:"Gray",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-color","Gray")}},Custom:{name:"Custom",callback:function(a,b){return changeCSSinMenu(c,a,b,"border-color",picked_colour)}}}},Corners:{name:"Rounded Corners",callback:function(a,b){var f=prompt("Enter a number from 0 (square)  to 12 (fully rounded)",
"5");return 12>=f&&0<=f?changeCSSinMenu(c,a,b,"border-radius",f+"px"):!1}},Thickness:{name:"Border Width",callback:function(a,b){var f=prompt("Enter a number from 0 up","2");return 100>=f&&0<=f?changeCSSinMenu(c,a,b,"border-width",f+"px"):!1}}}},sep2:"---------",clone:{name:"Clone",callback:function(a,b){}},template:{name:"Templates",items:{save:{name:"Save As",callback:function(a,e){for(var f=0;void 0!==b["template"+template_ID]&&100>f;)template_ID++,f++,console.log("conflict with template IDs");
100>f&&(f=prompt("Enter name for template"),b["template"+template_ID]={name:f,savedhtml:e.$trigger.parent().parent().children(c)[0].outerHTML,callback:function(a,d){if("Load"==d.$selected.parent().siblings("span").html()){console.log("Loading: "+a);var e=d.$trigger.parent().parent().children(c),f=e.html();e[0].outerHTML=b[a].savedhtml;d.$trigger.parent().parent().children(c).html(f)}else"Delete"==d.$selected.parent().siblings("span").html()&&delete b[a]}},template_ID++)}},load:{name:"Load",items:b},
"delete":{name:"Delete",items:b}}},"delete":{name:"Delete",icon:"delete",callback:function(a,b){confirm("Are you sure you want to delete this container?")&&b.$trigger.parent().parent().remove()}}}};if("Image"==a)return{items:{URL:{name:"Change URL",callback:function(a,b){var f=prompt("Enter new URL for image/video","http://"),g=checkImageURL(f,"");$.ajax({url:f,success:function(a){a=$(c).parent();$(c).remove();a.append(g)},error:function(a){alert("URL: "+f+" does not exist")}})}}}}}
$(function(){$.contextMenu({selector:".textmenu",trigger:"left",build:function(a,b){return generateContextMenu("Text",text_template_menu_list)}});$.contextMenu({selector:".decmenu",trigger:"left",build:function(a,b){return generateContextMenu("Decision",decision_template_menu_list)}});$.contextMenu({selector:".controlmenu",trigger:"left",build:function(a,b){return generateContextMenu("Control",text_template_menu_list)}});$.contextMenu({selector:".imgmenu",trigger:"left",build:function(a,b){return generateContextMenu("Image",
...
a.rect.width,$("#pagecontainers").width())+"px";b.style.height=checkBounds(b.parentNode.getAttribute("data-y"),a.rect.height,$("#pagecontainers").height())+"px"})}setInteractions();function toggleGridMode(){grid_snapping_mode=!grid_snapping_mode;$(".gridmode").toggleClass("activebutton");snap_options=initSnapOptions();setInteractions()}function checkBounds(a,b,c){a=parseFloat(a)||0;console.log("Check "+a+" + "+b);a+b>c?(console.log("Greater than "+c),b=c-a):console.log("Less than "+c);return b}
function dragMoveListener(a){var b=a.target,c=(parseFloat(b.getAttribute("data-x"))||0)+a.dx;a=(parseFloat(b.getAttribute("data-y"))||0)+a.dy;b.style.webkitTransform=b.style.transform="translate("+c+"px, "+a+"px)";b.setAttribute("data-x",c);b.setAttribute("data-y",a)}window.dragMoveListener=dragMoveListener;var doit;window.onresize=function(){clearTimeout(doit);doit=setTimeout(modalBounds,300)};
function modalBounds(){$("#page-modal").hasClass("open")&&$(".resize-element").each(function(){console.log("Test resize");var a=$(this);a.width(checkBounds(a.parent().attr("data-x"),a.width(),$("#pagecontainers").width()));a.height(checkBounds(a.parent().attr("data-y"),a.height(),$("#pagecontainers").height()))})};var pageOverlay={};function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}
...
b+"'class = 'handle'>Link "+b+'<a style="float:right" class="decmenu btn-floating btn waves-effect waves-light red"><i class="material-icons">settings</i></a>',c+="</div>";else return console.log("Unknown container type when generating handle for HTML"),null;return c}
function genPageCenterHTMLString(a,b,c){a=($("#pagecontainers").width()-a)/2;b=($("#pagecontainers").height()-b)/2;0<=c&&(b+=30*c);b>$("#pagecontainers").height()&&(b=$("#pagecontainers").height());return"transform: translate("+a+"px, "+b+"px);' data-x='"+a+"' data-y='"+b}function removeContainer(a,b){alert("Are you sure you want to delete"+a+"container"+b+"?");$("#"+a+b).parent().remove()}
...
function addTextContainer(){var a="<div class='text-container drag-element' style='position:absolute; "+genPageCenterHTMLString(300,220)+"'>",a=a+"<div class='editdiv resize-element' contenteditable=true ></div></div>",b=$(".text-container").length,a=htmlToElements(a);$("#pagecontainers").append(a);$("#pagecontainers div.text-container:last").prepend(genHandleHTML("text",b+1))}
...
1===cy.elements().size()&&cy.$("node").first().addClass("start")):current_state===states.NEWCONTROL&&b===cy&&cy.add({data:{name:cy.nodes().size()+1,text:"control node text",priorityList:[],defaultFailEdge:"none"},classes:"control",group:"nodes",renderedPosition:a.cyRenderedPosition})});
...
function stylePage(){$(".playpage").html("");for(var a=currentNode.data("textcontainers"),b=currentNode.data("decisioncontainers"),c=currentNode.data("imgcontainers"),d=0;d<a.length;d++)$(".playpage").append(a[d].html);for(d=0;d<c.length;d++)$(".playpage").append(c[d].html);for(d=0;d<b.length;d++)$(".playpage").append(b[d].html);$(".playpage").children("div[class^='decision-container']").each(function(a){$(this).click(function(){progressStory(a)})});$(".playpage .handle").hide();$(".playpage").children().removeClass("resize-element");
