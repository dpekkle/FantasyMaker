...
function audioObj(){this.selected_audio=null;this.size=0;this.assets=[];this.getAssetAsModalList=function(a){var b="";if(void 0!=a)a=this.getAsset(a),b+="<li id='"+a.id+"''>Name: "+a.name+"<br>Link: "+a.link+"</li>";else for(var c=0;c<this.assets.length;c++)a=this.getAsset(c),b+="<li id='"+a.id+"''>Name: "+a.name+"<br>Link: "+a.link+"</li>";return b};this.getAssetAsMenu=function(){assetlist={};for(var a=0;a<this.assets.length;a++)console.log("Generate audio menu"),console.log(this.assets[a]),assetlist[a]=
{name:this.assets[a].name,id:this.assets[a].id,callback:function(a,c){console.log("Key: ",a);audio.getAsset(a).addEvent();console.log(c)}};console.log(assetlist);return assetlist};this.getAsset=function(a){return void 0!==a?0<=a&&a<this.size?this.assets[a]:"No such asset":this.assets};this.removeAsset=function(){null!=this.selected_audio&&(this.assets.splice(this.selected_audio,1),$("#audiolist").find("#"+this.selected_audio).remove())};this.addAssetButton=function(){var a=prompt("Enter youtube link");
if(null==a)console.log("Link cannot be empty");else{var b=prompt("Give audio a name");null==b?console.log("Name cannot be empty"):this.addAsset(b,a)}};this.addAsset=function(a,b){var c=this.parseType(b);"error"!=c?(this.assets[this.size]=new audioAsset(a,this.size,b,c),this.size++,$("#audiolist").html(""),$("#audiolist").append(this.getAssetAsModalList())):console.log("Failed to add asset")};this.parseType=function(a){return a.match(/^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/)&&
RegExp.$1?"youtube":"error"};this.addAudioEvent=function(a){this.assets[a].addEvent()}}var audio=new audioObj;testAudioObjects();
function testAudioObjects(){audio.addAsset("RAP BEAT - REGGAE STYLE","https://www.youtube.com/watch?v=4vKmEmY8ZD8");audio.addAsset("Fantasy RPG","https://www.youtube.com/watch?v=Pq824AM9ZHQ");console.log(audio.getAsset(0));console.log(audio.getAsset(1));console.log(audio.getAsset());console.log(audio.getAsset("a"));console.log(audio.getAsset(-1));console.log(audio.getAsset(2))}
...
a=b.data("events");for(c=0;c<a.length;c++)audio.getAsset(a[c].id).addEvent(),$(".eventtrigger input").last().val(a[c].trigger),$(".eventtype select").last().val(a[c].action),$("#eventspane select").last().material_select();outgoingEdges=b.outgoers().edges();for(var d=b.data("decisioncontainers"),c=0;c<outgoingEdges.size();c++){var e=!1;for(a=0;a<d.length;a++)outgoingEdges[c].data("name")==d[a].name&&(e=!0);e||addDecisionContainer(b,c,outgoingEdges.eq(c).data("text"),outgoingEdges[c].data("name"))}for(a=
0;a<d.length;a++){e=!1;for(c=0;c<outgoingEdges.size();c++)d[a].name==outgoingEdges[c].data("name")&&($("#pagecontainers").append(d[a].html),$("#pagecontainers div.decision-container:last").prepend(genHandleHTML("decision",d[a].name)),e=!0);e||d.splice(a,1)}show_handles||$(".handle").hide();bindHandleSelection()}}function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}
...
$(".playpage .handle").hide();$(".playpage").children().removeClass("resize-element");$(".playpage").children().children().removeClass("resize-element");$(".playpage").children().attr("contenteditable","false");$(".playpage").children().children().attr("contenteditable","false");for(f=0;f<e.length;f++);}
