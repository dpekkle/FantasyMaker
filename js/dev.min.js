...
f&&0<=f?changeCSSinMenu(c,a,b,"border-width",f+"px"):!1}}}},sep2:"---------",clone:{name:"Clone",callback:function(a,b){}},template:{name:"Templates",items:{save:{name:"Save As",callback:function(a,e){for(var f=0;void 0!==b["template"+template_ID]&&100>f;)template_ID++,f++,console.log("conflict with template IDs");100>f&&(f=prompt("Enter name for template"),b["template"+template_ID]={name:f,savedhtml:e.$trigger.parent().parent().children(c)[0].outerHTML,callback:function(a,d){if("Load"==d.$selected.parent().siblings("span").html()){console.log("Loading: "+
a);var e=d.$trigger.parent().parent().children(c),f=e.html();e[0].outerHTML=b[a].savedhtml;d.$trigger.parent().parent().children(c).html(f)}else"Delete"==d.$selected.parent().siblings("span").html()&&delete b[a]}},template_ID++)}},load:{name:"Load",items:b},"delete":{name:"Delete",items:b}}},"delete":{name:"Delete",icon:"delete",callback:function(b,c){if(confirm("Are you sure you want to delete this container?")){var f=c.$trigger.parent().attr("id").split(a)[1];removeContainer(a,f)}}}}};if("img"==
a)return{items:{URL:{name:"Change URL",callback:function(a,b){var f=prompt("Enter new URL for image/video","http://"),g=checkImageURL(f,"");$.ajax({url:f,success:function(a){a=$(c).parent();$(c).remove();a.append(g)},error:function(a){alert("URL: "+f+" does not exist")}})}}}}}
...
image_template_menu_list)}})});var dragDrop={};grid_snapping_mode=!1;var grid_targets=interact.createSnapGrid({x:10,y:10,range:10,offset:{x:-2,y:2}});function initSnapOptions(){return grid_snapping_mode?{targets:[grid_targets],relativePoints:[{x:1,y:1}],endOnly:!1}:{targets:[],relativePoints:[{x:1,y:1}],endOnly:!1}}var snap_options=initSnapOptions();
...
function genPageCenterHTMLString(a,b,c){a=($("#pagecontainers").width()-a)/2;b=($("#pagecontainers").height()-b)/2;0<=c&&(b+=30*c);b>$("#pagecontainers").height()&&(b=$("#pagecontainers").height());return"transform: translate("+a+"px, "+b+"px);' data-x='"+a+"' data-y='"+b}
...
function addDecisionContainer(a,b,c,d){b="<div class = 'decision-container drag-element' style='position:absolute; "+genPageCenterHTMLString(300,220,cy.$(":selected")[0].data("decisioncontainers").length)+"'>";b+="<div class = 'editdec decisionbutton drag-element resize-element' contenteditable=true>"+escapeHtml(c)+"</div>";b+="</div>";c=a.data("decisioncontainers");c.push({name:d,html:b});a.data("decisioncontainers",c)}
function addOutputContainer(){if($("#pagecontainers div.output-container:last").length)alert("You may only have one control output container per page");else{var a="<div class='output-container drag-element' style='position:absolute; "+genPageCenterHTMLString(300,220)+"'>",a=a+"<div class='editdiv resize-element' contenteditable=false ></div></div>",a=htmlToElements(a);$("#pagecontainers").append(a);$("#pagecontainers div.output-container:last").prepend(genHandleHTML("output",0));show_handles||$(".handle").hide()}}
function addTextContainer(){var a="<div class='text-container drag-element' style='position:absolute; "+genPageCenterHTMLString(300,220)+"'>",a=a+"<div class='editdiv resize-element' contenteditable=true ></div></div>",b=$(".text-container").length,a=htmlToElements(a);$("#pagecontainers").append(a);$("#pagecontainers div.text-container:last").prepend(genHandleHTML("text",b+1));show_handles||$(".handle").hide()}
...
function addImageContainer(){var a,b=prompt("Enter image url","http://"),c=genPageCenterHTMLString(300,220);null!=b&&(a="<div class='img-container drag-element' style='position:absolute; "+c+"'>",a=checkImageURL(b,a),a+="</div>",$.ajax({url:b,success:function(b){b=$(".img-container").length;var c=htmlToElements(a);$("#pagecontainers").append(c);$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",b+1));show_handles||$(".handle").hide()},error:function(a){alert("URL: "+b+" does not exist")}}))}
...
decisioncontainers:selected_page_template.decisioncontainers},classes:"page",group:"nodes",renderedPosition:a.cyRenderedPosition})),1===cy.elements().size()&&cy.$("node").first().addClass("start")):current_state===states.NEWCONTROL&&b===cy&&cy.add({data:{name:cy.nodes().size()+1,text:"control node text",priorityList:[],defaultFailEdge:"none"},classes:"control",group:"nodes",renderedPosition:a.cyRenderedPosition})});
...
$(".playpage .handle").hide();$(".playpage").children().removeClass("resize-element");$(".playpage").children().children().removeClass("resize-element");$(".playpage").children().attr("contenteditable","false");$(".playpage").children().children().attr("contenteditable","false")}
