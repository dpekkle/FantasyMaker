...
cy.on("unselect",function(a){console.log("Unselect event fired ",a.cyTarget.data("id"));1!==cy.$(":selected").size()?hideEditPanes():(console.log("Only one selected"),hideEditPanes(),updateEditPane(cy.$(":selected")))});var playGame={};currentPage=null;outgoingEdges;function parsePage(){console.log("Parse page ",currentPage.data("id"));outgoingEdges=currentPage.outgoers().edges();currentPage.hasClass("page")&&($("#PageText").html(escapeHtml(currentPage.data("text"))),console.log(currentPage.data("text")),0<outgoingEdges.size()&&($("#Decision").html(escapeHtml(outgoingEdges.eq(0).data("text"))),console.log(outgoingEdges.eq(0).data("text"))))}
function progressStory(){null===currentPage?(currentPage=cy.$(".start")[0],parsePage()):0<currentPage.outgoers().size()&&(currentPage=outgoingEdges.eq(0).target(),console.log("Now on page ",currentPage.data("id")),parsePage())}function escapeHtml(a){var b=document.createElement("div");b.appendChild(document.createTextNode(a));return b.innerHTML};var pageOverlay={};function showPageOverlay(){document.getElementById("Modal").showModal()}function closeOverlay(){document.getElementById("Modal").close()};console.log("Enter main.js version 0.5.0");var dialog=document.querySelector("dialog");dialogPolyfill.registerDialog(dialog);var testgraph={};function checkValidGraph(){var a="";cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
