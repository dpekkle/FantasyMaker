...
function populateControlOverlay(a){currentDraggedItem=resetCurentDraggedItem();currentHoverOver=null;for(var b=0;b<a.json().data.priorityList.length;b++){var c=cy.edges("[id='"+a.json().data.priorityList[b]+"']").json(),d="<li id=connectedEdge_"+c.data.id+' ><div id="header_'+c.data.id+'" draggable="true" ondragend=handleDragEnd(event) ondragstart=handleDrag(event,\''+c.data.name+'\') ondragenter=handleDragEnter(event) class="collapsible-header">Edge '+c.data.name+'</div><div class="collapsible-body"><ul id = "'+
...
return c+="</div>"}function findCytoscapeObjectIndexByKey(a,b,c){for(var d=0;d<a.length;d++)if(a[d].hasOwnProperty(b)&&a[d][b]===c)return d;return-1}function removeContainer(a,b){alert("Are you sure you want to delete"+a+"container"+b+"?");$("#"+a+b).parent().remove()}
function addDecisionContainer(a,b,c,d){b="<div class = 'decision-container drag-element' style='position:absolute;'>"+("<div id = 'editdec' class = 'decisionbutton drag-element resize-element' contenteditable=true>"+escapeHtml(c)+"</div>");b+="</div>";c=a.data("decisioncontainers");c.push({name:d,html:b});a.data("decisioncontainers",c)}
