...
"fail-edge":"success-edge":c=!0;b+=" controledge"}else source_node.hasClass("page")&&(b+="pageedge");c&&(c=String.fromCharCode("A".charCodeAt()+source_node.edgesTo("*").size()),b=cy.add({data:{name:c,source:source_node.data("id"),target:a.data("id"),text:"<Decision text to display)>",conditions:[],outcomes:[]},classes:b,group:"edges"}),b.on("tap",function(a){this.select()}),source_node.hasClass("control")&&(source_node.json().data.priorityList.push(b.json().data.id),console.log("control nodes defaultFailEdge: "+
source_node.json().data.defaultFailEdge),"none"===source_node.json().data.defaultFailEdge&&(console.log("setting initial default fail edge to "+b.json().data.id),source_node.json({data:{defaultFailEdge:b.json().data.id}}),console.log("default fail edge is "+source_node.json().data.defaultFailEdge))),console.log(cy.elements().jsons()));source_node.removeClass("source_node");source_node=null;a.unselect();style_end_nodes()}}
