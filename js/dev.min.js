...
removeElement(),current_state=states.DEFAULT,$(a).removeClass("activebutton"))};var pageOverlay={};function htmlToElements(a){var b=document.createElement("template");b.innerHTML=a;return b.content.childNodes}
function addDecisionContainer(a,b,c){a="<div id = 'decision-container"+a+"' class='drag-element' style='position:absolute;'>"+("<div class = 'handle'>Link "+(a+1)+"</div>");a+="<div id = 'editdec' class = 'decisionbutton drag-element' contenteditable=true style='border:1px solid #F00;'>"+escapeHtml(b)+"</div>";a+="</div>";new_container=htmlToElements(a);b=cy.$(":selected").data("decisioncontainers");b.push({edgeid:c,html:a});cy.$(":selected").data("decisioncontainers",b)}
function addTextContainer(){var a=cy.$(":selected").data("textcontainers").length,b;b="<div id = 'text-container"+a+"' class='drag-element' style='position:absolute;'>"+("<div class = 'handle'>Text "+(a+1)+"</div>");b+="<div id = 'editdiv' contenteditable=true style='border:1px solid #F00; width:200px; height:200px;position:relative; max-height: 200px; overflow-x:hidden; overflow-y:auto;'></div>";b+="</div>";new_container=htmlToElements(b);$("#pagecontainers").append(new_container);$("#text-container"+
a+" #editdiv").trigger("focus");b=cy.$(":selected").data("textcontainers");a={html:$("#text-container"+a)[0].outerHTML};b.push(a);cy.$(":selected").data("textcontainers",b)}
function showPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$(".toolbar").hide();$("#modalcontainers").children().hide();overlayToolbar(b);if(b.hasClass("page")){$("#pagecontainers").show();$("#pagecontainers").html("");for(var c=cy.$(":selected")[0].data("textcontainers"),d=0;d<c.length;d++)$("#pagecontainers").append(c[d].html);outgoingEdges=b.outgoers().edges();for(var c=cy.$(":selected")[0].data("decisioncontainers"),e=0;e<outgoingEdges.size();e++){for(var f=!1,d=0;d<c.length;d++)outgoingEdges[e].data("target")==
c[d].edgeid&&(f=!0);f||addDecisionContainer(e,outgoingEdges.eq(e).data("text"),outgoingEdges[e].data("target"))}for(d=0;d<c.length;d++){f=!1;for(e=0;e<outgoingEdges.size();e++)c[d].edgeid==outgoingEdges[e].data("target")&&($("#pagecontainers").append(c[d].html),f=!0);f||c.splice(d,1)}}b.isEdge()&&($("#connectioncontainers").show(),populateEdgeOverlay(b.json()));b.hasClass("control")&&($("#controlcontainers").show(),$("#controlcontainers #controltext").val(escapeHtml(b.data("text"))));null===a&&document.getElementById("Modal").showModal()}
function overlayToolbar(a){a.hasClass("control")?($("#controltoolbar").show(),$("#controlname").text("control "+a.data("id"))):a.hasClass("page")?($("#pagetoolbar").show(),$("#pagename").text("Page "+a.data("id")),changeImage(a),changeAudio(a)):a.isEdge()&&($("#connectiontoolbar").show(),$("#connectionname").text("Connection"))}
function closeOverlay(a){null===a&&document.getElementById("Modal").close();var b=a;null===a&&(b=cy.$(":selected")[0]);b.hasClass("page")&&($("#pagecontainers").children("div[id^='text-container']").each(function(a){var d=this.outerHTML;b.data("textcontainers")[a].html=d;console.log("Updating HTML for ",a)}),$("#pagecontainers").children("div[id^='decision-container']").each(function(a){var d=this.outerHTML;b.data("decisioncontainers")[a].html=d;console.log("save html for decision ",a)}));b.isEdge()&&
(saveEdge(),$("#conditionsList").children().remove(),$("#outcomeList").children().remove())}function escapeHtml(a){var b=document.createElement("div");b.appendChild(document.createTextNode(a));return b.innerHTML};var ui={};function removeElement(){element=cy.$(":selected");element.empty()||(cy.remove(element),total_pages--,cleanup_titles(),cy.$("node").first().addClass("start"))}
...
"fail-edge":"success-edge":c=!1}if(c)cy.add({data:{source:source_node.data("id"),target:a.data("id"),text:"<Decision text to display)>",conditions:[],outcomes:[]},classes:b,group:"edges"}).on("tap",function(a){this.select()});source_node.removeClass("source_node");source_node=null;a.unselect()}}
