...
function browser_httpRequests_handleAuth(a){console.log("server returned"+a);return"EXPIRED"===a||"NO_TOKEN"===a?("EXPIRED"===a?console.log("http_handleAuth(): Token is expired"):console.log("http_handleAuth(): No Token"),users_flushToken(),browser_httpRequests_redirectHome(),!1):!0}function browser_httpRequests_redirectHome(){window.location="http://localhost:3000/create.html"};var gameBrowser={},gameBrowser_allProjects;gameBrowser_loadUsers();function gameBrowser_loadUsers(){var a={data:{}};+$.when(browser_httpRequests_getProjectsForBrowser(a,!0)).done(function(){gameBrowser_allProjects=a.data;console.log(gameBrowser_allProjects);gameBrowser_displayAllGames(gameBrowser_allProjects);$(".loading-content").hide();$("#main-content").show()})}
function gameBrowser_displayAllGames(a){$("#recommended-games-list").empty();a=$("#all-games-list");a.empty();if(gameBrowser_allProjects!={})for(var b=0;b<gameBrowser_allProjects.length;b++)gameBrowser_displayGame(gameBrowser_allProjects[b],b);else $("#main-content").innerHTML="<h1>There are no games to show</h1>";""==a.html&&a.append("<h4 style='color: #fff; margin-left: 10px;'>Be the first to publish a game!</h4> <a class='btn btn-medium waves-effect waves-light grey' href='create.html'>Create a Game</a>")}
function gameBrowser_filterByUsername(a,b){var c=$("#all-games-list");c.empty();c.append("<h4 style='color: #fff; margin-left: 10px;'>No games found with a username matching <span style='color: #00FEBC'>"+b+"</span></h4>")}
