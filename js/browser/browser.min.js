var host={},host_location="http://localhost:3000/";function host_create(){return host_location+"create.html"}function host_browse(){return host_location+"browse.html"}function host_play(){return host_location+"play/"}function host_index(){return host_location+"index.html"};var play_httpRequests={};
...
function authenticate(a){window.localStorage.setItem("play-token","");return $.ajax({url:"/playAuth",data:{uname:"guest"},cache:!1,type:"POST",success:function(a){console.log("LOGIN RES:");console.log(a);window.localStorage.setItem("play-token",JSON.stringify(a))},contenttype:"application/json"})}function addTokenToHeader(){var a=window.localStorage.getItem("play-token");a&&(a=JSON.parse(a).token,$.ajaxSetup({headers:{"x-access-token":a}}))};var users={};function users_generateNewUser(){return{username:"none",password:"none"}}function users_getUsername(){return window.localStorage.getItem("token")?JSON.parse(window.localStorage.getItem("token")).user:"INVALID"}function users_setToken(a){$.when(window.localStorage.setItem("token",JSON.stringify(a))).done()}function users_flushToken(){window.localStorage.setItem("token","")}
