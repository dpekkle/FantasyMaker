...
$("#"+project_project.projectName+"_pub").replaceWith(pubHTML);$(".pubTT").tooltip({delay:50})}function project_generateID(){for(var a="",b=0;20>b;b++)a+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(62*Math.random()));return a}var conditions={};console.log("Enter conditions.js");function conditions_createCondition(a,b){return{edge:a.data.id,id:getNextConditionID(a),html:b}}
function conditions_createOutcome(a,b){return{edge:a.data.id,id:getNextOutcomeID(a),html:b}}function getNextConditionID(a){for(var b=-1,c=0;c<a.data.conditions.length;c++)a.data.conditions[c].id>b&&(b=a.data.conditions[c].id);return++b}function getNextOutcomeID(a){for(var b=-1,c=0;c<a.data.outcomes.length;c++)a.data.outcomes[c].id>b&&(b=a.data.outcomes[c].id);return++b}var edgeOverlay={},edgeOverlay_newConditionCount=0,edgeOverlay_newOutcomeCount=0,edgeOverlay_selectedEdge;
...
getAttributeText(a[0].childNodes[1].childNodes[0])+"("+b+") "+f+" "+getAttributeText(a[0].childNodes[3].childNodes[0])+"("+d+") "+c+" "+getAttributeText(a[0].childNodes[5].childNodes[0])+"("+g+") is "+boolToString(e)+"<br>");return e}if("3"===b){var b=getAttributeValue(a[0].childNodes[1].childNodes[0]),f=a[0].childNodes[2].childNodes[0].data,d=getAttributeValue(a[0].childNodes[3].childNodes[0]),h=getAttributeValue(a[0].childNodes[5].childNodes[0]),c=a[0].childNodes[4].childNodes[0].data,l=a[0].childNodes[6].childNodes[0].data,
k=getAttributeValue(a[0].childNodes[7].childNodes[0]);if(void 0===b||void 0===d||void 0===h||void 0===k)return console.log("assessCondition() type3: attButton1, attButton2,attButton3, or attButton4 is undefined"),!1;e=assessModification(b,f,d);g=assessModification(h,l,k);e=assessComparison(e,c,g);logger.log("Condition: "+getAttributeText(a[0].childNodes[1].childNodes[0])+"("+b+") "+f+" "+getAttributeText(a[0].childNodes[3].childNodes[0])+"("+d+") "+c+" "+getAttributeText(a[0].childNodes[5].childNodes[0])+
"("+h+")"+l+" "+getAttributeText(a[0].childNodes[7].childNodes[0])+"("+k+") is "+boolToString(e)+"<br>");return e}if("4"===b){b=getInventoryItemFromHtml(a[0].childNodes[1].childNodes[0]);a=a[0].childNodes[2].childNodes[0].firstChild.attributes.state.value;if(void 0===b||void 0===a)return console.log("assessCondition() type4: item or exists is undefined"),!1;if(isNaN(parseInt(b.playCount)))return console.log("assessCondition() type4: item.playCount is not a number"),!1;if("true"===a||!0===a){if(console.log("dose exist?"),
...
function test_Edge_Labels(){var a=cy.nodes(".start").outgoers().edges().map(function(a,b){return a.data("name")+" with "+a.source().data("decisioncontainers")[b].name});console.info(a);for(var a=[],b=cy.nodes(".start").data("decisioncontainers"),c=0;c<b.length;c++)a.push(b[c].name);console.info(a)};var play_httpRequests={};function load(a,b){addTokenToHeader();return $.ajax({url:"/getProject",data:{projectOwner:a,projectName:b},cache:!1,type:"GET",success:function(c){"INVALID"===c?(console.log('No project found called "'+b+'" made by "'+a+'"'),$("#data").text('No project found called "'+b+'" made by "'+a+'"')):"EXPIRED"===c?(console.log("token expired"),$("#data").text("token expired")):(delete c[0]._id,console.log(c),project_project=c[0])},contenttype:"application/json"})}
function authenticate(a){window.localStorage.setItem("play-token","");return $.ajax({url:"/playAuth",data:{uname:"guest"},cache:!1,type:"POST",success:function(a){console.log("LOGIN RES:");console.log(a);window.localStorage.setItem("play-token",JSON.stringify(a))},contenttype:"application/json"})}function addTokenToHeader(){var a=window.localStorage.getItem("play-token");a&&(a=JSON.parse(a).token,$.ajaxSetup({headers:{"x-access-token":a}}))};var show={};$(document).ready();console.log($.cookie("FM_playUsername"));console.log($.cookie("FM_playProject"));$.when(getProject()).done(function(){prepareForGame();$("#mainContent").show();$("#loader").hide();$(".progressbutton").show()});function getProject(){var a=$.cookie("FM_playUsername"),b=$.cookie("FM_playProject");$.when(authenticate()).done(function(){$.when(load(a,b)).done(function(){http_setupCy()})})};
