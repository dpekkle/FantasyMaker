...
function Logger(){this.data=[];this.player=[];this.flush=function(){this.data=[];this.player=[]};this.log=function(a){this.data.push(a)};this.makerOutput=function(){return this.data};this.outputAsArray=function(){return this.data};this.playerLog=function(a){this.player.push(a)};this.outputPlayerAsArray=function(){return this.player};this.playerOutput=function(){var a="";for(idx in this.player)a+=this.player[idx]+"</br>";return a}}var logger=new Logger,loginSignup={};
...
c=getAttributeValue(b[0].childNodes[2].childNodes[0]),b=getTextButtonValue(b[0].childNodes[3].childNodes[0]),void 0===c&&(console.log("executeOutcomes() attribute modification: attButton1 or attButton2 is undefined"),c=" <Error: No attribute selected> "),logger.playerLog(d+c+b);else if("Text"===c)d=getTextButtonValue(b[0].childNodes[1].childNodes[0]),logger.playerLog(d);else if("inventory_addRemove"===c){console.log(b);d=getAddRemoveButtonState(b[0].childNodes[1].childNodes[0]);c=getAttributeValue(b[0].childNodes[2].childNodes[0]);
b=getInventoryItemFromHtml(b[0].childNodes[3].childNodes[0]);if(void 0===b){console.log("executeOutcomes() inventory add/remove: item is undefined.");break}else if(isNaN(c)){console.log("executeOutcomes() inventory add/remove: inputFieldValue is not a number.");break}"add"===d?(d=parseInt(b.playCount),c=d+c,b.playCount=c):"remove"===d&&(d=parseInt(b.playCount),c=d-c,b.playCount=0>c?0:c)}}else logger.log("Edge "+a.json().data.name+" has no outcomes.<br>")}
