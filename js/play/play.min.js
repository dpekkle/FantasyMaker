...
function populateEdgeOverlay(a){var b=cy.nodes("[id='"+a.data.source+"']").json().data.name,c=cy.nodes("[id='"+a.data.target+"']").json().data.name;$("#edge_name").text("Edge: "+a.data.name);$("#edge_details").text("Source: "+b+" -  Target: "+c);edgeOverlay_newOutcomeCount=edgeOverlay_newConditionCount=0;edgeOverlay_selectedEdge=a;for(b=0;b<a.data.conditions.length;b++)c=a.data.conditions[b],console.log(c),edgeOverlay_addExistingCondition("conditionsList",c);for(b=0;b<a.data.outcomes.length;b++)c=
...
this.data());changeCSSinMenu(c,a,"border-style",this.data().Style);changeCSSinMenu(c,a,"text-align",this.data().Alignment);changeCSSinMenu(c,a,"font-family",this.data().fontfamily)}}};if("img"==a)return{items:{border:border_menu_entry(c),position:zIndex_menu_entry(),sep3:"---------",URL:{name:"Change URL",callback:function(b,e){var f=e.$trigger.parent().siblings(c);myModal.prompt("Change URL","",[{name:"Enter image url","default":"http://",type:"text"}],function(a){var b=a[0];$.ajax({url:b,success:function(a){a=
f.parent();f.remove();a.append(html_string)},error:function(a){alert("URL: "+b+" does not exist")}})},function(b){return checkImageURL(b[0],a)?!0:b[0]+"not a valid URL"})}},"delete":{name:"Delete",icon:"delete",callback:function(b,c){if(confirm("Are you sure you want to delete this container?")){var f=c.$trigger.siblings(".handle").attr("id").split(a)[1];removeContainer(a,f)}}}},events:{show:function(a){$.contextMenu.setInputValues(a,this.data())},hide:function(a){$.contextMenu.getInputValues(a,this.data());
changeCSSinMenu(c,a,"border-style",this.data().Style)}}};if("vid"==a)return{items:{border:border_menu_entry(c),position:zIndex_menu_entry(),sep3:"---------",URL:{name:"Change URL",callback:function(b,e){var f=e.$trigger.parent().siblings(c);myModal.prompt("Change URL","",[{name:"Enter image url","default":"http://",type:"text"}],function(b){var c=b[0];(html_string=checkImageURL(c,a))&&$.ajax({url:c,success:function(a){a=f.parent();f.remove();a.append(html_string)},error:function(a){alert("URL: "+
c+" does not exist")}})},function(b){return checkImageURL(b[0],a)?!0:b[0]+"not a valid URL"})}},"delete":{name:"Delete",icon:"delete",callback:function(b,c){if(confirm("Are you sure you want to delete this container?")){var f=c.$trigger.siblings(".handle").attr("id").split(a)[1];removeContainer(a,f)}}}},events:{show:function(a){$.contextMenu.setInputValues(a,this.data())},hide:function(a){$.contextMenu.getInputValues(a,this.data());changeCSSinMenu(c,a,"border-style",this.data().Style)}}};if("page"==
a)return{items:{border:border_menu_entry(c),background:{type:"color",customName:"Background Colour",className:"background-color "+c}},events:{show:function(a){$.contextMenu.setInputValues(a,this.data())},hide:function(a){$.contextMenu.getInputValues(a,this.data());$(c).css("border-style",this.data().Style)}}}}
...
$(document).ready(function(){var a=!1;$("html").on("keyup",function(b){if(17==b.which||91==b.which)a=!1}).on("keydown",function(b){if(17==b.which||91==b.which)a=!0;if($(".modal").hasClass("open"))$("#prompt-modal").hasClass("open")&&(13==b.which?myModal.evaluateModal(!0):27==b.which&&myModal.evaluateModal(!1));else if(console.log(b.which),a&&67==b.which)0!=cy.$(":selected").length&&0==cy.$(":selected:parent").length&&(console.log("Copy"),nodes_to_clone=cy.$(":selected").nodes().jsons(),console.log(nodes_to_clone));
else if(a&&86==b.which){if(console.log(nodes_to_clone),null!==nodes_to_clone){console.log("Paste");cy.$(":selected").unselect();for(b=0;b<nodes_to_clone.length;b++){var c={x:nodes_to_clone[b].position.x+10,y:nodes_to_clone[b].position.y+=10};nodes_to_clone[b].data.name=parseInt(cy.nodes().size()+1);nodes_to_clone[b].classes=nodes_to_clone[b].classes.replace("start","");delete nodes_to_clone[b].data.id;cy.add({data:nodes_to_clone[b].data,classes:nodes_to_clone[b].classes,group:nodes_to_clone[b].group,
position:c}).select()}nodes_to_clone=cy.$(":selected").nodes().jsons()}}else if(27==b.which)console.log("Press escape!"),defaultState(),cy.$(":selected").unselect();else if(49<=b.which&&58>=b.which)0==$(".dropdown-content.active").length?49==b.which?$(".pagemode").trigger("click"):50==b.which?$(".controlmode").trigger("click"):51==b.which?$(".jumpmode").trigger("click"):52==b.which?$(".connectionmode").trigger("click"):53==b.which&&$(".prebuilt").trigger("click"):$(".dropdown-content.active li a").eq(b.which-
49).trigger("click");else if(0!=cy.$(":selected").length)if(8==b.which||46==b.which)console.log("Press delete"),$(".deletebutton")[0].click();else if(13==b.which||32==b.which)event.preventDefault(),$(".openoverlay").each(function(a){"none"!=$(this).css("display")&&(console.log("Enter press on ",$(this)),$(this).trigger("click"))})})});var http_loginSignup={};
