...
function http_redirectHome(){window.location=host_create()}function http_save(a,b){http_addTokenToHeader();return $.ajax({type:"POST",url:"/saveProject",data:{save:JSON.stringify(a)},success:function(a){http_handleAuth(a)?b.data=!0:(b.data=!1,console.log(a))},contenttype:"application/json"})}
...
function saveProject(){for(var a=cy.elements(".page"),b=0;b<a.length;b++)updatePageStyle(a[b]);project_saveProject()}function setStart(){cy.$(".start").removeClass("start");cy.$(":selected")[0].addClass("start");cy.$(":selected")[0].removeClass("leaf")}function removeParent(){cy.$(":selected")[0].move({parent:null})}
...
var canvasEvents={};console.log("Enter canvasEvents.js");compound_group_num=0;held_node=source_node=null;cy.on("tapstart","node",function(a){held_node=a.cyTarget});
