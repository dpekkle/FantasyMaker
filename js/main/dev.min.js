...
c+="</div>",c+='<a class="controlmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else if("inventory"==a)c+='<a class="btn-floating waves-effect waves-light purple lighten-2 lefticon"><i class="fa fa-briefcase"></i></a>',c+="<div id = 'inventory' class = 'handle control-handle'>Inventory",c+="</div>",c+='<a class="inventorymenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else if("character"==
a)c+='<a class="btn-floating waves-effect waves-light gray lighten-1 lefticon"><i class="fa fa-female"></i></a>',c+="<div id = 'character' class = 'handle control-handle'>Character",c+="</div>",c+='<a class="charactermenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else if("jump"==a)c+='<a class="btn-floating waves-effect waves-light orange lighten-1 lefticon"><i class="fa fa-caret-up"></i></a>',c+="<div id = 'jump' class = 'handle control-handle'>"+
b,c+="</div>",c+='<a class="jumpmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else if("jumpback"==a)c+='<a class="btn-floating waves-effect waves-light orange lighten-1 lefticon"><i class="fa fa-caret-down"></i></a>',c+="<div id = 'jumpback' class = 'handle control-handle'>Jump Back",c+="</div>",c+='<a class="jumpbackmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else return console.log("Unknown container type when generating handle for HTML"),
null;return c+="</div>"}function genPageCenterHTML(a,b,c){a=(parseFloat(project_project.resolution.x)-a)/2;b=(parseFloat(project_project.resolution.y)-b)/2;0<=c&&(b+=30*c);b>project_project.resolution.y&&(b=project_project.resolution.y);return"transform: translate("+a+"px, "+b+"px);' data-x='"+a+"' data-y='"+b};var contextMenu={};function changeCSSinMenu(a,b,c,d){console.log("Inside change CSS");"#pagecontainers"==a?$(a).css(c,d):b.$trigger.parent().siblings(a).css(c,d)}function bringEleToFront(a){var b=0;$("#pagecontainers").children("div").each(function(){var a=$(this).css("zIndex");a>b&&(b=a)});b++;console.log("Set zIndex to ",b);a.css("zIndex",b)}var primary_colour="rgba(255,255,255,1.0)",secondary_colour="rgba(255,255,255,1.0)";
...
function generateContextMenu(a,b){if("text"==a||"output"==a||"img"==a||"vid"==a)var c=".editdiv";else"decision"==a||"jump"==a||"jumpback"==a||"inventory"==a||"character"==a?c=".editdec":"page"==a&&(c="#pagecontainers");if("text"==a||"decision"==a||"output"==a||"jump"==a||"jumpback"==a||"inventory"==a||"character"==a)return{items:{font:{name:"Font",items:{Alignment:{name:"Alignment",type:"select",options:{left:"left",right:"right",Center:"Center",Justify:"Justify"}},fontfamily:{name:"Style",type:"select",
options:{'"Courier New", Courier, monospace':"Courier New",'"Lucida Console", Monaco, monospace':"Lucinda",'"Times New Roman", Times, serif':"Times New Roman",'"Arial", Helvetica, sans-serif':"Arial","Tahoma, Geneva, sans-serif":"Tahoma",'"Comic Sans MS", cursive, sans-serif':"Comic Sans"}},"font-size":{name:"Size",callback:function(a,b){var f=b.$trigger.parent().siblings(c);myModal.prompt("Set font size","Between 1 and 40",[{name:"Font-size","default":f.css("font-size"),min:"1",max:"40",type:"number"}],
function(a){f.css("font-size",a[0])},function(a){return 40>=a[0]&&1<=a[0]?!0:"Ensure number is between 1 and 40"})}},"font-color":{type:"color",customName:"Font Colour",className:"color "+c}}},background:{name:"Background",items:{Colour:{type:"color",customName:"Background Colour",className:"background-color "+c},Opacity:{name:"Opacity",callback:function(a,b){var f=b.$trigger.parent().siblings(c),g=f.css("background-color"),g=g.match(/rgba/)?g.replace(/^.*,(.+)\)/,"$1"):1;myModal.prompt("Set Opacity",
...
sep2:"---------",clone:{name:"Clone",callback:function(a,b){}},template:{name:"Templates",items:{save:{name:"Save As",callback:function(a,e){var f=e.$trigger.parent().parent().children(c)[0].outerHTML;myModal.prompt("Create Container Template","Saves the style of this container (e.g. background colour), allowing you to load them into other containers",[{name:"Template","default":"",type:"text"}],function(a){a=a[0];b[a]={name:a,savedhtml:f,callback:generateTemplateCallback(c,b)}},function(a){return""==
a[0]?"Can't be an empty name":"Default"==a[0]?"Can't override Default":void 0!==b[a[0]]?"A template with that name already exists":!0})}},load:{name:"Load",items:b},"delete":{name:"Delete",items:b}}},"delete":{name:"Delete",icon:"delete",callback:function(b,c){if(confirm("Are you sure you want to delete this container?")){var f=c.$trigger.siblings(".handle").attr("id").split(a)[1];removeContainer(a,f)}}}},events:{show:function(a){$.contextMenu.setInputValues(a,this.data())},hide:function(a){$.contextMenu.getInputValues(a,
...
b=0;b<c.length;b++)$("#pagecontainers").append(c[b].html),$("#pagecontainers div.text-container:last").prepend(genHandleHTML("text",c[b].name));c=a.data("imgcontainers");for(b=0;b<c.length;b++)$("#pagecontainers").append(c[b].html),$("#pagecontainers div.img-container:last").prepend(genHandleHTML("img",c[b].name));c=a.data("vidcontainers");for(b=0;b<c.length;b++)$("#pagecontainers").append(c[b].html),$("#pagecontainers div.vid-container:last").prepend(genHandleHTML("vid",c[b].name));c=a.data("specialbuttons");
for(b=0;b<c.length;b++)$("#pagecontainers").append(c[b].html),$("#pagecontainers div.button-container:last").prepend(genHandleHTML(c[b].type,c[b].jumpnode));$("#eventspane").append('<span class="eventspanetitle eventname" style="text-align:center; font-size: 16px;">Asset</span><span class="eventspanetitle eventtype" style="text-align:center; font-size: 16px;">Event</span><span class="eventspanetitle eventtrigger" style="text-align:center; font-size: 16px;">Time</span>');b=a.data("events");for(c=0;c<
b.length;c++)project_project.audio.getAsset(b[c].id)&&project_project.audio.getAsset(b[c].id).addEvent(),$(".eventtrigger input").last().val(b[c].trigger),$(".eventtype select").last().val(b[c].action),$(".eventtype .setting").last().val(b[c].setting),"Volume"==b[c].action&&$(".eventtype .setting").last().show(),$(".eventscontainer").last().find("select").material_select();outgoingEdges=a.outgoers().edges();for(var d=a.data("decisioncontainers"),c=0;c<outgoingEdges.size();c++){for(var e=!1,b=0;b<
d.length;b++)outgoingEdges[c].data("name")==d[b].name&&(e=!0);e||addDecisionContainer(a,c,outgoingEdges.eq(c).data("text"),outgoingEdges[c].data("name"))}for(b=0;b<d.length;b++){e=!1;for(c=0;c<outgoingEdges.size();c++)d[b].name==outgoingEdges[c].data("name")&&($("#pagecontainers").append(d[b].html),$("#pagecontainers div.decision-container:last").prepend(genHandleHTML("decision",d[b].name)),e=!0);e||d.splice(b,1)}show_handles||$(".handle").hide();$(".handle").on("mousedown touch",function(a){$(this).siblings().trigger("focus");
...
cy.nodes(":parent").size()+1,name:"Group"},classes:"expanded",group:"nodes"},{data:{displacement:{x:0,y:0},parent:"Emptyparent"+cy.nodes(":parent").size()+1,name:cy.nodes(".page, .control").size()+1,pagestyle:selected_page_template.data.pagestyle,outputcontainer:selected_page_template.data.outputcontainer,imgcontainers:selected_page_template.data.imgcontainers,vidcontainers:selected_page_template.data.vidcontainers,textcontainers:selected_page_template.data.textcontainers,decisioncontainers:[],events:[],
eventspane:'<div class= "eventscontainer"><span class="eventspanetitle eventname" style="text-align:center; font-size: 16px;">Asset</span><span class="eventspanetitle eventtype" style="text-align:center; font-size: 16px;">Event</span><span class="eventspanetitle eventtrigger" style="text-align:center; font-size: 16px;">Trigger</span></div>'},classes:"page",group:"nodes",renderedPosition:a.cyRenderedPosition}])))});
