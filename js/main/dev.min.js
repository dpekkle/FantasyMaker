...
$(".deletebutton")[0].click()):13!=b.which&&32!=b.which||$(".openoverlay").each(function(a){"none"!=$(this).css("display")&&(console.log("Enter press on ",$(this)),$(this).trigger("click"))}))})});var jumpOverlay={};function populateJumpOverlay(a){a=a.data("triggerType");"attribute"==a?loadJumpConditions():"button"==a&&loadJumpButton();setJumpNodeType(a)}function saveJump(){cy.$(":selected")[0].data("triggerType",$("#choosejumptrigger :checked").attr("value"));saveJumpConditions();saveJumpButton();setJumpNodeType(null);$("#jumpConditionList").html("");$("#jumpButtonList").html("")}
...
function loadJumpConditions(){for(var a=cy.$(":selected")[0].data("conditions"),b=0;b<a.length;b++)$("#jumpConditionList").append(a[b]),$("#row_"+b+"_attbutton1_specValue_inputField").val($("#row_"+b+"_attbutton1_specValue_inputField").attr("value")),$("#row_"+b+"_attbutton2_specValue_inputField").val($("#row_"+b+"_attbutton2_specValue_inputField").attr("value")),$("#row_"+b+"_attButton1").tooltip({delay:50}),$("#row_"+b+"_attButton2").tooltip({delay:50})}
function saveJumpConditions(){for(var a=[],b=$("#jumpConditionList li"),c=0;c<b.size();c++)$("#row_"+c+"_attbutton1_specValue_inputField").attr("value",$("#row_"+c+"_attbutton1_specValue_inputField").val()),$("#row_"+c+"_attbutton2_specValue_inputField").attr("value",$("#row_"+c+"_attbutton2_specValue_inputField").val()),a.push(b[c].outerHTML);cy.$(":selected")[0].data("conditions",a)}
function addJumpCondition(){var a=parseInt($("#jumpConditionList li").size());generic_addRow("row_"+a+"_","jump-settings-menu","jumpConditionList")}function loadJumpButton(){}function saveJumpButton(){}function createNewJumpButton(){}$("#choosejumptrigger").on("change",function(a){a=$("#choosejumptrigger :checked").attr("value");setJumpNodeType(a)});function Logger(){this.data=[];this.player=[];this.flush=function(){this.data=[];this.player=[]};this.log=function(a){this.data.push(a)};this.makerOutput=function(){return this.data};this.outputAsArray=function(){return this.data};this.playerLog=function(a){this.player.push(a)};this.outputPlayerAsArray=function(){return this.player};this.playerOutput=function(){var a="";for(idx in this.player)a+=this.player[idx]+"</br>";return a}}var logger=new Logger;var playConditions={};
...
function getAttributeValue(a){if("specValue"===a.id.split("_")[3])return parseFloat(a.childNodes[0].value);if("randValue"===a.id.split("_")[3]){var b=parseFloat(a.attributes.min.value);a=parseFloat(a.attributes.max.value);return Math.floor(Math.random()*(a-b+1))+b}b=a.attributes.path.nodeValue;console.log("Path is: ",b);return gameAttributes_find(b).value}
...
function assessComparison(a,b,c){a=parseFloat(a);c=parseFloat(c);switch(b){case "=":return a===c?!0:!1;case "!=":return a!==c?!0:!1;case ">":return a>c?!0:!1;case "<":return a<c?!0:!1;case ">=":return a>=c?!0:!1;case "<=":return a<=c?!0:!1}}function assessModification(a,b,c){a=parseInt(a);c=parseInt(c);switch(b){case "=":return c;case "+":return a+c;case "-":return a-c;case "*":return a*c;case "/":return a/c}};var playJump={};function initJumpNodes(){conditional_jump_nodes=cy.nodes('.jump[triggerType = "attribute"]');jump_context_stack=[]}function checkConditionalJumps(){var a=!1;conditional_jump_nodes.forEach(function(b){if(evaluateConditionalJump(b))return a=b,!1});return a}
function evaluateConditionalJump(a){console.log("Call evaluateConditionalJump for ",a.data("name"));var b=a.json().data.conditions;if(0<b.length){a=!0;for(var c=0;c<b.length;c++)!1===assessJumpCondition(b[c])&&(a=!1);return a}console.log("Jump node"+a.data("name")+" has no conditions");return!0}
function assessJumpCondition(a){console.log("Assess jump node condition");var b=$.parseHTML(a);a=getAttributeValue(b[0].childNodes[0].childNodes[1].childNodes[0]);var c=b[0].childNodes[0].childNodes[2].childNodes[0].data,b=getAttributeValue(b[0].childNodes[0].childNodes[3].childNodes[0]),d=assessComparison(a,c,b);console.log(a,c,b,"is: ",d);return d}function runJumpNode(a){jump_context_stack.push(a);console.log("The jump node "+a.data("name")+" is now officially jumped to")};var playGame={};function loadingScreen(a){a?($("#loader").show(),$(".progressbutton").hide()):($("#loader").hide(),$(".progressbutton").show())}
...
function wipeGame(){project_project.audio.loaded=0;cy.$(".jump").each(function(a,c){c.data("origin",null)});for(var a=0;a<project_project.audio.getAsset().length;a++)project_project.audio.assets[a].setVolume(100),project_project.audio.assets[a].stopAudio();$(".playpage").html("");$(".playpage").attr("style","");event_manager.clearTimer()}
function parseNode(){console.log("Parse node ",currentNode.data("id"));outgoingEdges=currentNode.outgoers().edges();currentNode.hasClass("page")?parsePage(outgoingEdges):currentNode.hasClass("control")&&parseControl(currentNode,outgoingEdges)}function parsePage(a){event_manager.newPage(currentNode.data("events"));stylePage()}
...
function progressStory(a){null===currentNode?1>cy.$("*").length?alert("Your graph is empty"):(currentNode=cy.$(".start")[0],parseNode()):0<currentNode.outgoers().size()?(currentNode=outgoingEdges.eq(a).target(),executeOutcomes(outgoingEdges.eq(a)),console.log("Now on node ",currentNode.data("id")),(a=checkConditionalJumps())?(a.data("origin",currentNode),runJumpNode(a)):parseNode()):(currentNode=null,console.log("Reached the end"),$(".playpage").html("<h1>Fin!</h1>"),wipeGame())}
function stripDraggable(a){return a.replace(/drag-element/g,"")}
...
a+")"});$(".playpage").css({"-webkit-transform":"scale("+a+")"})}$(window).resize(resizePlayPage);console.log("Enter main.js version 0.5.0");$(document).ready();$("select").material_select();var testgraph={};function testJumpNodes(){var a=cy.collection();cy.$(".jumpend").each(function(b){b.predecessors("node").roots().each(function(c){c.hasClass("jump")||(a.add(c),a.add(b))})});return 0!=a.size()?(a.addClass("jumpenderror"),setTimeout(function(){a.removeClass("jumpenderror")},3E3),!1):!0}
function checkValidGraph(){var a="";1>cy.$("*").length&&(a=a.concat("The graph is empty\n"));cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));testJumpNodes||(a=a.concat("Jump End nodes can be reached without arriving from a jump start!"));""==a?alert("All tests passed"):alert(a)}
function testConnectivity(){var a=!1,b=cy.$(".start, .jump"),c=cy.$(".page, .control, .jumpend");console.log("Root nodes: "+b);var d=cy.collection(),d=d.add(b);c.union(cy.$("edge")).dfs({roots:b,visit:function(a,b,c){d=d.add(c);console.log("visit "+this.id()+" total "+a)},directed:!0});console.log("Total nodes:"+c.size()+" Connected_nodes: "+d.size());if(c.size()==d.size())a=!0;else{var e=c.diff(d).left;console.log("Disconnected:",e.size());console.log("left: "+e.map(function(a){return a.id()}).join(", "));
