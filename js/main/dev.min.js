...
$.contextMenu({selector:".special-buttons",trigger:"left",build:function(a,b){return{items:{Jump:{name:"Jump Buttons",items:project_project.button_list},"Jump Back":{name:"Jump Back",callback:function(a,b){console.log("Create a jump back button on the page overlay")}},Attributes:{name:"Show Attributes Panel",callback:function(a,b){console.log("Create an attributes panel button")}},Inventory:{name:"Show Inventory",callback:function(a,b){console.log("Create an inventory button")}}}}}});
...
46==b.which)console.log("Press delete"),$(".deletebutton")[0].click();else if(13==b.which||32==b.which)event.preventDefault(),$(".openoverlay").each(function(a){"none"!=$(this).css("display")&&(console.log("Enter press on ",$(this)),$(this).trigger("click"))})})});var jumpOverlay={};function populateJumpOverlay(a){a=a.data("triggerType");loadJumpConditions();loadJumpButton();setJumpNodeType(a);$('#choosejumprepeat input[value="'+cy.$(":selected")[0].data("repeat")+'"]').prop("checked",!0);$("#choosejumptrigger :checked").prop("checked","")}
...
function createNewJumpButton(){myModal.prompt("Create New Button","After creation you can add this button to page nodes via the edit page overlay. Once you've done this the player can press the button to be taken to this jump node. It's a good idea for each jump node to have it's own button.",[{name:"Enter name","default":"",type:"text"}],function(a){a=a[0];cy.$(":selected")[0].data("button");project_project.button_list[a]={name:a,callback:function(a,c){console.log("Create a jump button on the page overlay")}};
$("#jumpbuttons").append("<li><a onclick=\"chooseJumpButton('"+a+"');\">"+a+"</a></li>")},function(a){return""==a[0]?"Can't be an empty name":"Default"==a[0]?"Can't override Default":void 0!==project_project.button_list[a[0]]?"A button with that name already exists":!0})}function chooseJumpButton(a){cy.$(":selected")[0].data("button",project_project.button_list[a]);$(".choose-button").html(a)}$("#choosejumptrigger").on("change",function(a){a=$("#choosejumptrigger :checked").attr("value");setJumpNodeType(a)});
