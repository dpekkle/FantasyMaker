...
for(var b=0;b<a[0].graph.length;b++)"edges"!==a[0].graph[b].group&&cy.add(a[0].graph[b]);for(b=0;b<a[0].graph.length;b++)if("edges"==a[0].graph[b].group)cy.add(a[0].graph[b]).on("tap",function(a){this.select()});Materialize.toast("Project '"+project_project.projectName+"' Loaded",3E3,"rounded");resizeCanvas();defaultState()}},contenttype:"application/json"})}
...
function projectSettings_deleteProject(a,b,c){myModal.prompt("Delete Project '"+b+"'.","Are you sure you wish to delete this project? This cannot be undone.",[],function(d){projectSettings_activePage=1;b===c&&$("#currentProject").hide();$.when(http_deleteProject(a,b)).done(function(){projectSettings_userProjects={projects:[]};$.when(http_getUsersProjects(a,projectSettings_userProjects)).done(function(){projectSettings_populateProjectsList(a,1);Materialize.toast("Project '"+b+"' Deleted",3E3,"rounded")})})},
...
function project_createNewProject(){myModal.prompt("Create New Project","Enter a unique ID for this project, this cannot be changed. If you wish to change the project title that players will see when you publish your game you can edit the title in the 'Project' menu.",[{name:"Project ID","default":"",type:"text"}],function(a){},function(a){var b=/^[a-zA-Z0-9_-]+$/;if(""==a[0])myModal.warning("Project ID cannot be empty");else if(!1===b.test(a[0]))myModal.warning("Project ID can only contain letters, numbers, underscores or hyphens");
else{var c={};$.when(http_getUsersProjects(users_getUsername(),c)).done(function(){console.log("RET");console.log(c);if(c.hasOwnProperty("projects"))for(var b=0;b<c.projects.length;b++)if(a[0].trim()===c.projects[b].projName){myModal.warning("You already have a project with the ID "+c.projects[b].projName+". Please choose something else.");return}project_project=initEmptyProject(users_getUsername(),a[0].trim());cy.elements().remove();var e={data:[]};$.when(http_save(project_project,e)).done(function(){!0===
e.data?(Materialize.toast("Project '"+project_project.projectName+"' created!",3E3,"rounded"),$("#prompt-modal").closeModal(),$("#UI_projName").text("Project: "+project_project.projectName),nav_toMain(),http_getUsersProjects(users_getUsername(),projectSettings_userProjects)):Materialize.toast("Failed to create project. Please log in again.",3E3,"rounded")})})}})}
...
function createFight(a){console.log(a.cyRenderedPosition);var b=!1;0===cy.elements(".page").size()&&(b=!0);var c=cy.add([{data:{id:"fightparent"+cy.nodes(":parent").size()+1,name:"Fight"},classes:"expanded",group:"nodes",style:{"z-index":cy.nodes(":parent").size()+1}},{data:{displacement:{x:0,y:0},parent:"fightparent"+cy.nodes(":parent").size()+1,name:cy.nodes(".page, .control").size()+1,pagestyle:selected_page_template.data.pagestyle,outputcontainer:selected_page_template.data.outputcontainer,imgcontainers:selected_page_template.data.imgcontainers,
vidcontainers:selected_page_template.data.vidcontainers,textcontainers:selected_page_template.data.textcontainers,decisioncontainers:[],events:[],eventspane:'<div class= "eventscontainer"><span class="eventspanetitle eventname" style="text-align:center; font-size: 16px;">Asset</span><span class="eventspanetitle eventtype" style="text-align:center; font-size: 16px;">Event</span><span class="eventspanetitle eventtrigger" style="text-align:center; font-size: 16px;">Trigger</span></div>'},classes:"page",
group:"nodes",renderedPosition:a.cyRenderedPosition},{data:{displacement:{x:-40,y:100},parent:"fightparent"+cy.nodes(":parent").size()+1,name:cy.nodes(".page, .control").size()+2,pagestyle:selected_page_template.data.pagestyle,outputcontainer:selected_page_template.data.outputcontainer,imgcontainers:selected_page_template.data.imgcontainers,vidcontainers:selected_page_template.data.vidcontainers,textcontainers:selected_page_template.data.textcontainers,decisioncontainers:[],events:[],eventspane:'<div class= "eventscontainer"><span class="eventspanetitle eventname" style="text-align:center; font-size: 16px;">Asset</span><span class="eventspanetitle eventtype" style="text-align:center; font-size: 16px;">Event</span><span class="eventspanetitle eventtrigger" style="text-align:center; font-size: 16px;">Trigger</span></div>'},
classes:"page",group:"nodes",renderedPosition:a.cyRenderedPosition},{data:{displacement:{x:40,y:100},parent:"fightparent"+cy.nodes(":parent").size()+1,name:cy.nodes(".page, .control").size()+3,pagestyle:selected_page_template.data.pagestyle,outputcontainer:selected_page_template.data.outputcontainer,imgcontainers:selected_page_template.data.imgcontainers,vidcontainers:selected_page_template.data.vidcontainers,textcontainers:selected_page_template.data.textcontainers,decisioncontainers:[],events:[],
eventspane:'<div class= "eventscontainer"><span class="eventspanetitle eventname" style="text-align:center; font-size: 16px;">Asset</span><span class="eventspanetitle eventtype" style="text-align:center; font-size: 16px;">Event</span><span class="eventspanetitle eventtrigger" style="text-align:center; font-size: 16px;">Trigger</span></div>'},classes:"page",group:"nodes",renderedPosition:a.cyRenderedPosition}]);b&&c.eq(1).addClass("start");current_state=states.CONNECTING;source_node=null;cy.$(":selected").unselect();
createConnection(c.eq(1));createConnection(c.eq(2));createConnection(c.eq(1));createConnection(c.eq(3));$.each(c,function(b,e){if(0<b){var f=this.data("displacement").x,g=this.data("displacement").y;console.log(f);console.log(g);c[b].stop(!1,!0);c[b].animate({position:{x:a.cyPosition.x+f,y:a.cyPosition.y+g}},{duration:500})}});current_state=states.NEWFIGHT;cy.$("#fightparent"+cy.nodes(":parent").size()+1).select()}
...
b){var b=getAttributeValue(a[0].childNodes[1].childNodes[0]),f=a[0].childNodes[2].childNodes[0].data,d=getAttributeValue(a[0].childNodes[3].childNodes[0]),e=assessModification(b,f,d),c=a[0].childNodes[4].childNodes[0].data,h=getAttributeValue(a[0].childNodes[5].childNodes[0]),k=a[0].childNodes[6].childNodes[0].data,l=getAttributeValue(a[0].childNodes[7].childNodes[0]),g=assessModification(h,k,l),e=assessComparison(e,c,g);logger.log("Condition: "+getAttributeText(a[0].childNodes[1].childNodes[0])+
"("+b+") "+f+" "+getAttributeText(a[0].childNodes[3].childNodes[0])+"("+d+") "+c+" "+getAttributeText(a[0].childNodes[5].childNodes[0])+"("+h+")"+k+" "+getAttributeText(a[0].childNodes[7].childNodes[0])+"("+l+") is "+boolToString(e)+"<br>");return e}if("4"===b){b=a[0].childNodes[1].childNodes[0].attributes.itemid.value;d=gameInventory_getItem(b);a=a[0].childNodes[2].childNodes[0].firstChild.attributes.state.value;console.log("EXISTS CONDITION:");console.log("ITEMID: "+b);console.log("ITEM: ");console.log(d);
console.log("EXISTS: "+a);if(void 0===b||void 0===d)return console.log("assessCondition() type4: item undefined"),!1;if(isNaN(parseInt(d.playCount)))return console.log("assessCondition() type4: item.playCount is not a number"),!1;if("true"===a||!0===a){if(console.log("dose exist?"),0<parseInt(d.playCount))return console.log("YES"),!0}else if(console.log("does not exist?"),0===parseInt(d.playCount))return console.log("YES"),!0;console.log("NO");return!1}if("5"===b)return console.log(a),b=a[0].childNodes[1].childNodes[0].attributes.itemid.value,
d=gameInventory_getItem(b),a=getAttributeValue(a[0].childNodes[2].childNodes[0]),void 0===b||void 0===d||isNaN(parseInt(d.playCount))?!1:parseInt(d.playCount)===a?!0:!1;if("6"===b)return console.log("Type 6 condition should not be used, use type 1 for random comparisons"),!1}
