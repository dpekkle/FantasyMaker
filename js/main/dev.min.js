...
function progressStory(a){null===currentNode?1>cy.$("*").length?alert("The project is empty"):($(".progressbutton").hide(),currentNode=cy.$(".start")[0],parseNode()):0<currentNode.outgoers().size()?(currentNode=outgoingEdges.eq(a).target(),executeOutcomes(outgoingEdges.eq(a)),console.log("Now on node ",currentNode.data("id")),(a=checkConditionalJumps())?(a.data("origin",currentNode),runJumpNode(a)):parseNode()):(currentNode=null,console.log("Reached the end"),$(".playpage").html("<h1>Fin!</h1>"),
wipeGame());logger.flush()}function stripDraggable(a){return a.replace(/drag-element/g,"")}
