...
function chooseNodeTemplate(a){$(".pagemode").addClass("activebutton");current_state=states.NEWPAGE;selected_page_template=project_project.project_templates[a];$(".pagemode").html("Page Node ("+a+")");console.log("Select template: "+a)};var canvasEvents={};console.log("Enter canvasEvents.js");held_node=source_node=null;cy.on("tapstart","node",function(a){held_node=a.cyTarget});
cy.on("tapend",function(a){if(null!=held_node){console.log("Check for dropping into parents:",cy.$(":parent").length);var b=cy.$(":parent:orphan");a=a.cyRenderedPosition;testobj={added:!1,parent:"none"};checkCompoundBounds(b,a,testobj);console.log("Finished: ",testobj);testobj.added&&testobj.parent!==held_node.parent().id()&&held_node.move({parent:testobj.parent});held_node=null}});
function checkCompoundBounds(a,b,c){a.forEach(function(a){var e=a.renderedBoundingBox();b.x>e.x1&&b.x<e.x2&&b.y>e.y1&&b.y<e.y2&&held_node!==a&&(c.added=!0,c.parent=a.id(),console.log("Move ",held_node.id()," into ",a.id()),console.log(c),checkCompoundBounds(a.children(":parent").difference(a),b,c))})}cy.on("taphold",":parent",function(a){parent.children().animated()});
cy.on("tap",":parent",function(a){cy.$(":selected").unselect();this.hasClass("collapsed")?(expand(this),this.descendants().outgoers("edge").removeClass("hidden")):this.hasClass("expanded")&&(this.descendants().each(function(a,c){if(c.parent().hasClass("expanded")){var d=c.position().x,e=c.position().y,d={x:d,y:e};console.log("Save position for: ",c.id()," at: ",d);c.data("displacement",d)}}),collapse(this),this.descendants().outgoers("edge").addClass("hidden"))});
...
for(f=0;f<b.length;f++)assessEdge(currentNode.outgoers("[name='"+b[f].name+"']").id())&&$(".playpage").append(b[f].html);$(".playpage").append(e);$(".output-container").children().append(logger.controlOutputHTML());$(".playpage").children("div[class^='decision-container']").each(function(a){$(this).click(function(){progressStory(a)})});$(".playpage .handlecontainer").hide();$(".playpage").children().removeClass("resize-element");$(".playpage").children().removeClass("resize-child");$(".playpage").children().children().removeClass("resize-element");
$(".playpage").children().attr("contenteditable","false");$(".playpage").children().children().attr("contenteditable","false")}
