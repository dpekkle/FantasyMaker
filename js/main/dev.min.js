...
function(){var a=this;myModal.prompt("Add audio assett","Supports youtube links that allow embedding",[{name:"Name","default":"",type:"text"},{name:"Link","default":"https://www.youtube.com/watch?v=4vKmEmY8ZD8",type:"text"}],function(a){$("#audiolist li").on("click",function(a){a.preventDefault();$("#audiolist li").removeClass("highlighted");$(this).toggleClass("highlighted");project_project.audio.selected_audio=$(this).attr("id")})},function(b){return""==b[0]||null==b[0]?!1:""==b[1]||null==b[1]?
!1:a.addAsset(b[0],b[1])?!0:!1})};this.addAsset=function(a,b){var c=this.parseType(b);if("error"!==c)return this.changed=!0,this.assets[this.assets.length]=new audioAsset(a,this.unique_id,b,c),$("#audiolist").html(""),$("#audiolist").append(this.getAssetAsModalList()),this.unique_id++,!0;console.log("Failed to add asset");return!1};this.parseType=function(a){return a.match(/^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/)&&RegExp.$1?
"youtube":"error"};this.addAudioEvent=function(a){this.assets[a].addEvent()}}function loadAudioObject(a){var b=new audioObj;b.selected_audio=a.selected_audio;b.unique_id=0;b.loaded=a.loaded;if(0==a.assets.length)b.changed=!1;else{b.changed=!0;for(var c=0;c<a.assets.length;c++)b.addAsset(a.assets[c].name,a.assets[c].link)}return b};var users={};function users_generateNewUser(){return{username:"none",password:"none"}}function users_getUsername(){return window.localStorage.getItem("token")?JSON.parse(window.localStorage.getItem("token")).user:"INVALID"}function users_setToken(a){$.when(window.localStorage.setItem("token",JSON.stringify(a))).done()}function users_flushToken(){window.localStorage.setItem("token","")}
...
function nav_navBarLoggedOut(){$("#profile_button").text("");$("#project_button").hide();$("#profile_button").hide();$("#settings_button").hide();$("#login_button").show();$("#signup_button").show()};var initCanvas={};console.log("Enter initCanvas.js");states={DEFAULT:0,CONNECTING:1,NEWPAGE:2,NEWCONTROL:3};current_state=states.DEFAULT;
...
function(a){this.select()});else cy.add(a[0].graph[b]);Materialize.toast("Project '"+project_project.projectName+"' Loaded",3E3,"rounded");resizeCanvas()}},contenttype:"application/json"})}function http_setupCy(){for(var a=0;a<project_project.graph.length;a++)if("edges"==project_project.graph[a].group)cy.add(project_project.graph[a]).on("tap",function(a){this.select()});else cy.add(project_project.graph[a])}
...
function http_login(a,b,c){return $.ajax({url:"/login",data:{uname:a,pwd:b},cache:!1,type:"POST",success:function(a){c.data=a},contenttype:"application/json"})}function http_handleAuth(a){return"EXPIRED"===a||"NO_TOKEN"===a?("EXPIRED"===a?console.log("http_handleAuth(): Token is expired"):console.log("http_handleAuth(): No Token"),users_flushToken(),http_redirectHome(),!1):!0}
function http_getProjectsForBrowser(a){http_addTokenToHeader();return $.ajax({url:"/getAllUsersProjects",cache:!1,type:"GET",success:function(a){http_handleAuth(a)},contenttype:"application/json"})}function http_addTokenToHeader(){var a=window.localStorage.getItem("token");a&&(a=JSON.parse(a).token,$.ajaxSetup({headers:{"x-access-token":a}}))};var generalOverlay={};$(document).ready(function(){$(".modal-trigger").leanModal({dismissible:!0,ready:function(){var a=cy.$(":selected")[0];null!=a&&(a.hasClass("page")?(openEditPageOverlay(a),console.log("Opening page overlay")):a.isEdge()?(openEditConnectionOverlay(a),console.log("Opening Edge Overlay")):a.hasClass("control")&&(openEditControlOverlay(a),console.log("Opening Control Overlay")))},complete:function(){closeOverlay(null)}})});
function openEditPageOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);null===a&&overlayToolbar(b);b.hasClass("page")&&(populatePageOverlay(b),$("#pagecontainers").width(project_project.resolution.x),$("#pagecontainers").height(project_project.resolution.y),resizePageContainerDiv())}function openEditConnectionOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);populateEdgeOverlay(b.json())}
function openEditControlOverlay(a){var b=a;null===a&&(b=cy.$(":selected")[0]);$("#controlcontainers #controltext").val(escapeHtml(b.data("text")));populateControlOverlay(b)}function openAttributesOverlay(){$("#attributes-modal").openModal({dismissible:!0,ready:function(){gameAttributes_loadAttributes()},complete:function(){gameAttributes_saveAttributes()}})}
function openInventoryDefinitionOverlay(){$("#inventory-definition-modal").openModal({dismissible:!0,ready:function(){},complete:function(){}})}
...
function htmlToElements(a){var b=document.createElement("template");b.innerHTML=a;return b.content.childNodes}
...
'">');f.html("");f.append('<h3 style="text-align:center;">'+a+"</h3>");f.append('<p id="promptDesc">'+b+"<p>");for(d=0;d<c.length;d++)f.append('<label for="'+d+'"> '+c[d].name+"</label>"),"number"==c[d].type?f.append('<input id= "'+d+'"type="'+c[d].type+'" value = "'+parseFloat(c[d]["default"])+'" min ="'+c[d].min+'" max ="'+c[d].max+'">'):f.append('<input id= "'+d+'"type="'+c[d].type+'" value = "'+c[d]["default"]+'">');$("#prompt-modal").openModal({dismissible:!1,ready:function(){},complete:function(){this.confirm=
!1;this.validated=!0;this.callbackFunction=this.verifyFunction=null;f.html("")}})};this.warning=function(a){$("#promptDesc").text(a);$("#promptDesc").css("color","red")}}myModal=new myModal;myModal.init();var projectSettings={},projectSettings_activePage=1,projectSettings_amtPages=1,projectSettings_userProjects={projects:[]};
function projectSettings_prepThenNavToProjects(a){console.log(a);$("#currentProject_header").text("Current Project: "+a.projectName);$("#currentProject_title").text(a.title);$("#currentProject_author").text(a.author);$("#currentProject_desc").text(a.description);$("#currentProject_dateCreated").text(a.dateCreated);$("#currentProject_lastModified").text(a.lastModified);$("#currentProject_published").text(a.published);$("#currentProject_imageLink").text(a.imageLink);!0===a.published?($("#currentProject_gameLink").attr("href",
a.gameLink),$("#currentProject_gameLink").text(a.gameLink),$("#currentProject_noGameLink").hide(),$("#currentProject_gameLink").show(),$("#pubSwitch").prop("checked",!0)):($("#currentProject_gameLink").hide(),$("#currentProject_noGameLink").show(),$("#pubSwitch").prop("checked",!1));"none"!==a.projectName&&(nav_isProject=!0);$.when(projectSettings_prepareProjectsPage()).done(nav_toProjects())}
function projectSettings_prepThenNavToMain(a){$.when(http_load(a)).done(function(){nav_isProject=!0;resizeCanvas();nav_toMain();projectSettings_activePage=1})}
...
function projectSettings_deleteProject(a,b){myModal.prompt("Delete Project '"+b+"'.","Are you sure you wish to delete this project? This cannot be undone.",[],function(c){projectSettings_activePage=1;$.when(http_deleteProject(a,b)).done(function(){projectSettings_userProjects={projects:[]};$.when(http_getUsersProjects(a,projectSettings_userProjects)).done(function(){projectSettings_populateProjectsList(a,1);Materialize.toast("Project '"+b+"' Deleted",3E3,"rounded")})})},function(a){return!0})};var states={DEFAULT:0,CONNECTING:1,NEWPAGE:2,NEWCONTROL:3,NEWJUMP:4,NEWFIGHT:5,NEWSTORE:6};current_state=states.DEFAULT;function defaultState(){exitStates();current_state=states.DEFAULT}function exitStates(){current_state==states.CONNECTING&&(cy.boxSelectionEnabled(!0),null!==source_node&&(source_node.removeClass("source_node"),source_node=null));$("#sidebar .btn").removeClass("activebutton");$(".pagemode").html("Page Node");cy.$(":parent").selectify()}
function changeState(a){exitStates();$(a).addClass("activebutton");$(a).hasClass("connectionmode")&&current_state!=states.CONNECTING?(current_state=states.CONNECTING,cy.$(":selected").unselect(),cy.$(":parent").unselectify(),cy.boxSelectionEnabled(!1),source_node=null):$(a).hasClass("pagemode")&&current_state!=states.NEWPAGE?exitStates():$(a).hasClass("controlmode")&&current_state!=states.NEWCONTROL?current_state=states.NEWCONTROL:$(a).hasClass("jumpmode")&&current_state!=states.NEWJUMP?current_state=
states.NEWJUMP:$(a).hasClass("prebuilt")&&current_state!=states.NEWFIGHT&&current_state!=states.NEWSTORE?exitStates():($(a).hasClass("deletebutton")&&removeElement(),current_state=states.DEFAULT,$(a).removeClass("activebutton"))}function choosePrebuilt(a){$(".prebuilt").addClass("activebutton");"Fight"==a?current_state=states.NEWFIGHT:"Store"==a&&(current_state=states.NEWSTORE)};var project={};console.log("Entering project.js");var host="http://localhost:3000/",project_project=initEmptyProject("none","none");defaultState();
function initEmptyProject(a,b){return{project_templates:{Default:{pagestyle:"width: 800px; flex: 0 0 800px; height:600px; border: 3px solid black",outputcontainer:"",imgcontainers:[],vidcontainers:[],textcontainers:[],decisioncontainers:[]}},template_menus:new templateMenuObj,audio:new audioObj,projectOwner:a,projectName:b,dateCreated:project_getCurrentTime(),lastModified:project_getCurrentTime(),published:!1,title:b,author:a,description:"No description available.",imageLink:"No URL provided",gameLink:host+
"play/"+a+"/"+b,graph:[],gameAttributes:{},gameInventory:{},attributesHTML:"",resolution:{x:1E3,y:800}}}
function project_createNewProject(){myModal.prompt("Create New Project","",[{name:"Project Name","default":"",type:"text"}],function(a){},function(a){if(""==a[0])myModal.warning("Project name cannot be empty");else{var b={names:[]};$.when(http_getUsersProjects(users_getUsername(),b)).done(function(){for(var c=0;c<b.names.length;c++)if(a[0].trim()===b.names[c].name){myModal.warning("You already have a project named "+b.names[c].name+". Please choose another name.");return}project_project=initEmptyProject(users_getUsername(),
a[0].trim());cy.elements().remove();var d={data:[]};$.when(http_save(project_project,d)).done(function(){!0===d.data?(Materialize.toast("Project '"+project_project.projectName+"' created!",3E3,"rounded"),$("#prompt-modal").closeModal(),$("#UI_projName").text("Project: "+project_project.projectName),nav_toMain(),http_getUsersProjects(users_getUsername(),projectSettings_userProjects)):Materialize.toast("Failed to create project. Please log in again.",3E3,"rounded")})})}})}
...
function project_saveProject(){project_project.graph=cy.elements().jsons();project_project.lastModified=project_getCurrentTime();var a={data:!1};$.when(http_save(project_project,a)).done(function(){!0===a.data?Materialize.toast("Saved project",3E3,"rounded"):Materialize.toast("Failed to save project. Please log in again.",3E3,"rounded")})}
function project_login(){myModal.prompt("Log In","Log in and continue creating!",[{name:"Username","default":"",type:"text"},{name:"Password","default":"",type:"password"}],function(a){},function(a){var b={data:{}};$.when(http_login(a[0],a[1],b)).done(function(){"INVALID_USERNAME"!==b.data&&"INVALID_PASSWORD"!==b.data&&"SERVER_ERR"!==b.data?project_successfulLogin(b):myModal.warning("Login details were invalid. Please try again.")})})}
function project_successfulLogin(a){users_flushToken();window.localStorage.setItem("token",JSON.stringify(a.data));$("#prompt-modal").closeModal();Materialize.toast("Welcome back "+users_getUsername()+"!",3E3,"rounded");console.log(users_getUsername()+" logged in");$("#profile_button").text(users_getUsername());projectSettings_prepThenNavToProjects(project_project)}
function project_signUp(){myModal.prompt("Sign Up","Become a game creator with FantasyMaker!",[{name:"Username","default":"",type:"email"},{name:"Password","default":"",type:"password"},{name:"Confirm Password","default":"",type:"password"}],function(a){},function(a){if(""===a[0]||""===a[1]||""===a[2])return myModal.warning("All fields must be filed out."),!1;if(a[1]!==a[2])return myModal.warning("Your password and confirmation password do not match."),!1;var b=users_generateNewUser();b.username=
a[0];b.password=a[1];$.when(http_signUp(b,"")).done(function(b){"VALID"===b?(Materialize.toast("Welcome to FantasyMaker "+a[0]+".",3E3,"rounded"),$("#prompt-modal").closeModal()):myModal.warning("That username has already been taken. Please try another username")})})}
function project_logOut(){myModal.prompt("Log Out","Are you sure you wish to log out? unsaved progress may be lost.",[],function(a){console.log(users_getUsername()+" logged out");nav_toLogin();users_flushToken();project_project=initEmptyProject("none","none");http_redirectHome()},function(a){return!0})}function project_getCurrentTime(){var a=new Date,b=a.getDate(),c=a.getMonth()+1,a=a.getFullYear();return b+"-"+c+"-"+a}
...
function project_modifyAuthor(){myModal.prompt("Modify Project Author","Change the author of your project. This will appear as the name of your project in the game browser.",[{name:"Author","default":project_project.author,type:"text"}],function(a){project_project.author=a[0];$("#currentProject_author").text(project_project.author);project_saveProject()},function(a){return""===a[0]?(myModal.warning("Author field cannot be empty."),!1):!0})}
...
function getNextOutcomeID(a){for(var b=-1,c=0;c<a.data.outcomes.length;c++)a.data.outcomes[c].id>b&&(b=a.data.outcomes[c].id);return++b};var edgeOverlay={},edgeOverlay_newConditionCount=0,edgeOverlay_newOutcomeCount=0,edgeOverlay_selectedEdge;function populateEdgeOverlay(a){edgeOverlay_newOutcomeCount=edgeOverlay_newConditionCount=0;edgeOverlay_selectedEdge=a;for(var b=0;b<a.data.conditions.length;b++){var c=a.data.conditions[b];console.log(c);edgeOverlay_addExistingCondition("conditionsList",c)}for(b=0;b<a.data.outcomes.length;b++)c=a.data.outcomes[b],edgeOverlay_addExistingOutcome("outcomesList",c)}
...
a1024pxa768px:"Medium 1024x768",a1280pxa1024px:"Large 1280x1024"}}},events:{show:function(a){$.contextMenu.setInputValues(a,this.data())},hide:function(a){$.contextMenu.getInputValues(a,this.data());$(c).css("border-style",this.data().Style);if(null!=this.data().resolution){a=this.data().resolution.split("a")[1];var b=this.data().resolution.split("a")[2];$(c).css("width",a);$(c).css("height",b);project_project.resolution.x=a;project_project.resolution.y=b;resizePageContainerDiv()}}}}}
...
trigger:"left",build:function(a,b){return{items:{Audio:{name:"Audio",items:project_project.audio.getAssetAsMenu()}}}}})});var dragDrop={};grid_snapping_mode=!1;var grid_targets=interact.createSnapGrid({x:10,y:10,range:10,offset:{x:-2,y:2}});function initSnapOptions(){return grid_snapping_mode?{targets:[grid_targets],relativePoints:[{x:1,y:1}],endOnly:!1}:{targets:[],relativePoints:[{x:1,y:1}],endOnly:!1}}var snap_options=initSnapOptions();
function setInteractions(){interact(".drag-element:not([locked])").draggable({snap:snap_options,inertia:!1,restrict:{restriction:"parent",endOnly:!1,elementRect:{top:0,left:0,bottom:1,right:1}},onstart:function(a){console.log("Started dragging")},onmove:dragMoveListener,onend:function(a){console.log(a)}}).allowFrom(".handle");interact(".resize-element").resizable({snap:snap_options,edges:{left:!1,right:!0,bottom:!0,top:!1},onmove:resizeMoveListener})}setInteractions();
...
function chooseNodeTemplate(a){$(".pagemode").addClass("activebutton");current_state=states.NEWPAGE;selected_page_template=project_project.project_templates[a];$(".pagemode").html("Page Node ("+a+")");console.log("Select template: "+a)};var canvasEvents={};console.log("Enter canvasEvents.js");source_node=null;cy.on("tapend",":orphan",function(a){var b=a.cyTarget,c=a.cyRenderedPosition;cy.$(":parent").each(function(a,e){if(e!==b){var f=e.renderedBoundingBox();console.log(f);c.x>f.x1&&c.x<f.x2&&c.y>f.y1&&c.y<f.y2&&(console.log("Move ",b.id()," into ",e.id()),b.move({parent:e.id()}))}})});cy.on("taphold",":parent",function(a){parent.children().animated()});
cy.on("tap",":parent",function(a){cy.$(":selected").unselect();this.hasClass("collapsed")?(expand(this),this.descendants().outgoers("edge").removeClass("hidden")):this.hasClass("expanded")&&(collapse(this),this.descendants().outgoers("edge").addClass("hidden"))});
function expand(a){var b=a.position();a.hasClass("collapsed")&&(a.children().each(function(a,d){d.stop(!1,!0);var e=this.data("displacement").x,f=this.data("displacement").y;console.log(e);console.log(f);d.animate({position:{x:b.x+e,y:b.y+f},style:{opacity:1}},{duration:500});d.removeClass("hidden")}),a.removeClass("collapsed"),a.addClass("expanded"))}
function collapse(a){var b=a.position();a.hasClass("expanded")&&(a.children().each(function(a,d){d.stop(!1,!0);var e=d.relativePosition();d.data("displacement",e);d.animate({position:{x:b.x,y:b.y},style:{opacity:0},complete:function(){d.addClass("hidden")}},{duration:500})}),a.addClass("collapsed"),a.removeClass("expanded"))}cy.on("tap",":selected",function(a){console.log("Tapped on: ",cy.$(":selected").data("name"));current_state===states.CONNECTING&&createConnection(a.cyTarget)});
...
!0);fight_collection[b].animate({position:{x:a.cyPosition.x+e,y:a.cyPosition.y+f}},{duration:500})}});current_state=states.NEWFIGHT;cy.$("#fightparent"+fight).select()};var conditionsConextMenu={};
...
a[0]);var b=!0;""==a[0]&&(console.log("No Attribute name entered"),Materialize.toast("Please enter an attribute name",3E3,"rounded"),b=!1);if(a[1]<a[2]||a[1]>a[3]||a[2]>a[3]||""==(a[2]||a[3]))console.log("Illegal attribute values"),Materialize.toast("Conflicting Attribute Values",3E3,"rounded"),b=!1;return b})};var gameInventory={};function InventoryItem(a,b,c){this.itemID=a;this.name=b;this.description=c;this.itemCount=0}function gameInventory_defineItem(a,b){var c="item-"+generateID();project_project.gameInventory[c]=new InventoryItem(c,a,b);return project_project.gameInventory[c]}function gameInventory_editItem(a,b,c){a=project_project.gameInventory[a];a.name=b;a.description=c;return a}function gameInventory_deleteItem(a){project_project.gameInventory[a]=null;delete project_project.gameInventory[a]}
function gameInventory_getItem(a){return project_project.gameInventory[a]}
function gameInventory_openDefineItemOverlay(){myModal.prompt("Define a New Item","Define an item below",[{name:"Item Name","default":"",type:"text"},{name:"Item Description","default":"",type:"text"}],function(a){gameInventory_definitionModal_append(gameInventory_defineItem(a[0],a[1]))},function(a){console.log("Verifying ",a[0]);if(""==a[0])return Materialize.toast("Please enter an item name",3E3,"rounded"),!1;console.log("item name successfully verified");Materialize.toast("New Item Defined",3E3,
"rounded");return!0})}
function gameInventory_openEditItemOverlay(a){var b=gameInventory_getItem(a);myModal.prompt("Editing "+b.name,"Update item below",[{name:"Item Name","default":b.name,type:"text"},{name:"Item Description","default":b.description,type:"text"}],function(b){gameInventory_definitionModal_update(gameInventory_editItem(a,b[0],b[1]))},function(a){console.log("Verifying ",a[0]);if(""==a[0])return Materialize.toast("Please enter an item name",3E3,"rounded"),!1;console.log("item name successfully verified");Materialize.toast("Item Updated",
3E3,"rounded");return!0})}
function gameInventory_definitionModal_append(a){a='<li class="'+a.itemID+'-list-element"><div class="collapsible-header"><span class="'+a.itemID+'-name">'+a.name+'</span></div><div class="collapsible-body"><div class="row pos-relative "><div class="col m12"><h6>Description: </h6></div><div class="col m12"><p class="'+a.itemID+'-description">'+a.description+'</p><div class="col m3 offset-m9 attribute_float-bottom pos-absolute"><a class="btn-floating btn-small waves-effect waves-light blue edit-button" onclick="gameInventory_openEditItemOverlay(\''+a.itemID+
'\')"><i class="small material-icons">mode_edit</i></a> &nbsp;<a class="btn-floating btn-small waves-effect waves-light red delete-button"  onclick="gameInventory_definitionModal_delete(\''+a.itemID+'\')"><i class="small material-icons">delete</i></a></div></div></div></li>';$(".inventory-list").append(a)}function gameInventory_definitionModal_update(a){$("."+a.itemID+"-name").text(a.name);$("."+a.itemID+"-description").text(a.description)}
function gameInventory_definitionModal_delete(a){$("."+a+"-list-element").remove();gameInventory_deleteItem(a)};var hotkeys={},nodes_to_clone;
...
function progressStory(a){null===currentNode?1>cy.$("*").length?alert("Your graph is empty"):(currentNode=cy.$(".start")[0],parseNode()):0<currentNode.outgoers().size()?(currentNode=outgoingEdges.eq(a).target(),executeOutcomes(outgoingEdges.eq(a)),console.log("Now on node ",currentNode.data("id")),parseNode()):(currentNode=null,console.log("Reached the end"),$(".playpage").html("<h1>Fin!</h1>"))}function stripDraggable(a){return a.replace(/drag-element/g,"")}
function resizePlayPage(){$(".playpage").width(project_project.resolution.x);$(".playpage").height(project_project.resolution.y);var a;a=$(window).height();var b=$("#tabheadings").outerHeight()+$(".nav-wrapper").outerHeight()+$(".progressbutton").outerHeight();$("#playwindow").css("height",a-b);a=$("#playwindow").width();var b=$("#playwindow").height(),c=$(".playpage").width(),d=$(".playpage").outerHeight();a=Math.min(a/c,b/d);$(".playpage").css({transform:"scale("+a+")"});$(".playpage").css({"-ms-transform":"scale("+
a+")"});$(".playpage").css({"-webkit-transform":"scale("+a+")"})}$(window).resize(resizePlayPage);console.log("Enter main.js version 0.5.0");$(document).ready();$("select").material_select();var testgraph={};function checkValidGraph(){var a="";1>cy.$("*").length&&(a=a.concat("The graph is empty\n"));cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
