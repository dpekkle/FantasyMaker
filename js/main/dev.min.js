...
function showOverlayLinks(a){$(".editbutton").hide();a.isNode()?($(".editname").show(),a.isChild()&&$(".removeparent").show(),a.hasClass("page")?($('button[data-target="page-modal"]').show(),$(".setstart").show()):a.hasClass("control")&&$('button[data-target="control-modal"]').show()):a.hasClass("pageedge")?$('button[data-target="connection-modal"]').show():a.hasClass("controledge")&&$('button[data-target="connection-modal"]').show()}
...
held_node=null}});function checkCompoundBounds(a,b,c){a.forEach(function(a){var e=a.renderedBoundingBox();b.x>e.x1&&b.x<e.x2&&b.y>e.y1&&b.y<e.y2&&held_node!==a&&(c.added=!0,c.parent=a.id(),checkCompoundBounds(a.children(":parent.expanded").difference(a),b,c))})}cy.on("taphold",":parent",function(a){parent.children().animated()});
