...
function resizePlayPage(){$(".playpage").width(project_project.resolution.x);$(".playpage").height(project_project.resolution.y);var a;a=$(window).height();var b=$("#tabheadings").outerHeight()+$(".nav-wrapper").outerHeight()+$(".progressbutton").outerHeight()+$(".gobackbutton").outerHeight()+1;$("#playwindow").css("height",a-b);a=$("#playwindow").width();var b=$("#playwindow").height(),c=$(".playpage").width(),d=$(".playpage").outerHeight();a=Math.min(a/c,b/d);$(".playpage").css({transform:"scale("+
a+")"});$(".playpage").css({"-ms-transform":"scale("+a+")"});$(".playpage").css({"-webkit-transform":"scale("+a+")"})}$(window).resize(resizePlayPage);console.log("Enter main.js version 0.5.0");$(document).ready();$("select").material_select();var testgraph={};function testJumpNodes(){var a=!0;cy.$(".jumpend").each(function(b,c){var d=c.predecessors("node").roots();for(b=0;b<d.size();b++)d[b].hasClass("jump")||(console.log("bad one found",d[b].data("name")),a=!1,d[b].flashClass("jumpenderror",3E3),c.flashClass("jumpenderror",3E3))});return a}
