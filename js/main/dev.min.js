...
contenttype:"application/json"})}function http_addTokenToHeader(){var a=window.localStorage.getItem("token");a&&(a=JSON.parse(a).token,$.ajaxSetup({headers:{"x-access-token":a}}))};var generalOverlay={};$(document).ready(function(){$(".modal-trigger").leanModal({dismissible:!0,ready:function(){var a=cy.$(":selected")[0];null!=a&&(a.hasClass("page")?(openEditPageOverlay(a),console.log("Opening page overlay")):a.isEdge()?(openEditConnectionOverlay(a),console.log("Opening Edge Overlay")):a.hasClass("control")?(openEditControlOverlay(a),console.log("Opening Control Overlay")):a.hasClass("jump")&&openEditJumpOverlay(a))},complete:function(){closeOverlay(null)}})});
...
function assessJumpCondition(a){var b=$.parseHTML(a);a=getAttributeValue(b[0].childNodes[0].childNodes[1].childNodes[0]);var c=b[0].childNodes[0].childNodes[2].childNodes[0].data,b=getAttributeValue(b[0].childNodes[0].childNodes[3].childNodes[0]),d=assessComparison(a,c,b);console.log(a,c,b,"is: ",d);return d}
function runJumpNode(a){jump_context_stack.push(a);console.log("The jump node "+a.data("name")+" is now officially jumped to");a=a.outgoers()[0].target();console.log("Target node is",a);return a}function runJumpEnd(){var a=jump_context_stack.pop().data("origin");console.log("Reached jump end. Now Jump back to",a.data("name"));return a};var playGame={};function loadingScreen(a){a?($("#loader").show(),$(".progressbutton").hide()):($("#loader").hide(),$(".progressbutton").show())}
