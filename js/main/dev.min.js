...
!1;this.validated=!0;this.callbackFunction=this.verifyFunction=null;f.html("")}})};this.warning=function(a){$("#promptDesc").css("color","red");!1!==a&&$("#promptDesc").text(a)}}myModal=new myModal;myModal.init();$(document).ready(function(){$("html").on("keydown",function(a){$("#prompt-modal").hasClass("open")&&(13==a.which?myModal.evaluateModal(!0):27==a.which&&myModal.evaluateModal(!1))})});var projectSettings={},projectSettings_activePage=1,projectSettings_amtPages=1,projectSettings_userProjects={projects:[]};
...
function populateEdgeOverlay(a){$("#edge_name").text("Edge: "+a.data.name);var b=cy.nodes("[id='"+a.data.source+"']").json().data.name,c=cy.nodes("[id='"+a.data.target+"']").json().data.name;$("#edge_name").attr("data-tooltip","Origin of edge: page "+b+"<br> Target of edge: page"+c+"<br>");$("#edge_name").tooltip({delay:50});edgeOverlay_newOutcomeCount=edgeOverlay_newConditionCount=0;edgeOverlay_selectedEdge=a;for(b=0;b<a.data.conditions.length;b++)c=a.data.conditions[b],console.log(c),edgeOverlay_addExistingCondition("conditionsList",
...
function cleanup_edge_labels(a){var b=a.source();if(b.hasClass("page"))for(var c=0;c<b.data("decisioncontainers").length;c++)if(b.data("decisioncontainers")[c].name==a.data("name")){b.data("decisioncontainers").splice(c,1);break}cy.remove(a);a=b.edgesTo("*");updatePageStyle(b);for(c=0;c<a.size();c++){var d=String.fromCharCode("A".charCodeAt()+c);a[c].style("label",d);a[c].data("name",d);b.hasClass("page")&&(b.data("decisioncontainers")[c].name=d)}}
...
function gameAttribute_createModule_updateContextPane(){var a,b,c=$("#folder-container"),d=$("#values-container");c.empty();d.empty();for(var e=0;e<currentAttributeObj.childrenArray.length;e++)a=gameAttributes_find(currentAttributeObj.path+"_"+currentAttributeObj.childrenArray[e]),a.is_leaf?(b='<div id="'+a.id+'-value-card" class="value-card-panel col l3 m4 s6 card-panel hoverable"><i class="material-icons small">equalizer</i><span style="font-size: 19px; font-weight: bold">'+a.name+'</span><hr/><span style="font-size: 14px"> Initial Value: <span id="'+
a.id+'-value" style="font-size: 19px; font-weight: bold">'+a.value+'</span></span><br/><span style="font-size: 14px"> Value Range: <span id="'+a.id+'-min-value" style="font-size: 19px; font-weight: bold">'+a.minValue+'</span> to <span id="'+a.id+'-max-value" style="font-size: 19px; font-weight: bold">'+a.maxValue+'</span></span><br/><hr/><a class="btn-floating red right hoverable" onclick="gameAttributes_createModule_deleteAttribute(\''+a.path+'\')"><i class="material-icons small">delete</i></a><a class="btn-floating blue right hoverable"onclick="gameAttributes_createModule_openEditAttributeModal(\''+
