...
<<<<<<< HEAD
function nav_navBarLoggedOut(){$("#profile_button").text("");$("#project_button").hide();$("#profile_button").hide();$("#settings_button").hide();$("#login_button").show();$("#signup_button").show()};var initCanvas={};console.log("Enter initCanvas.js");
...
function showOverlayLinks(a){$(".editbutton").hide();a.isNode()&&$(".editname").show();a.hasClass("page")?($('button[data-target="page-modal"]').show(),$(".setstart").show()):a.hasClass("control")?$('button[data-target="control-modal"]').show():a.hasClass("pageedge")?$('button[data-target="connection-modal"]').show():a.hasClass("controledge")&&$('button[data-target="connection-modal"]').show()}
function nameNode(a){myModal.prompt("Name Node","Give a unique name to this entity",[{name:"Name","default":a.data("name"),type:"text"}],function(b){a.data("name",b[0]);a.addClass("named")},function(b){var c=!0;if(""==b[0])c=!1;else{var d;a.hasClass("jump")?d=".jump":a.hasClass("page")||a.hasClass("control")?d=".page, .control":a.isParent()&&(d=":parent");cy.nodes(d).each(function(d,f){a!==f&&(console.log("check ",b[0]+" against",f.data("name")),b[0]==f.data("name")&&(console.log("found duplicate"),
...
$("#prompt-modal").closeModal();else $("#prompt-modal").closeModal()};this.prompt=function(a,b,c,d,e){var f=$("#prompt-modal .modal-content");this.verifyFunction=void 0!==e?e:null;this.callbackFunction=d;f.html("");a=""+('<h3 style="text-align:center;">'+a+"</h3>")+('<p id="promptDesc">'+b+"<p>");for(b=0;b<c.length;b++)"number"==c[b].type?(a+='<div class="input-field">',a+='<label for="'+b+'"> '+c[b].name+'</label><input id= "'+b+'"type="'+c[b].type+'" value = "'+parseFloat(c[b]["default"])+'" min ="'+
c[b].min+'" max ="'+c[b].max+'">'):(a+='<div class="input-field">',a+='<label for="'+b+'"> '+c[b].name+'</label><input id= "'+b+'"type="'+c[b].type+'" value = "'+c[b]["default"]+'">'),a+="</div>";f.append(a);$("#prompt-modal .modal-content input").eq(0).prop("autofocus",!0);$("#prompt-modal").openModal({dismissible:!1,ready:function(){Materialize.updateTextFields()},complete:function(){this.confirm=!1;this.validated=!0;this.callbackFunction=this.verifyFunction=null;f.html("")}})};this.warning=function(a){$("#promptDesc").css("color",
"red");!1!==a&&$("#promptDesc").text(a)}}myModal=new myModal;myModal.init();var projectSettings={},projectSettings_activePage=1,projectSettings_amtPages=1,projectSettings_userProjects={projects:[]};
...
cy.on("tapend",function(a){if(null!=held_node){console.log("Check for dropping into parents:",cy.$(":parent").length);var b=cy.$(":parent:orphan.expanded");a=a.cyRenderedPosition;testobj={added:!1,parent:"none"};checkCompoundBounds(b,a,testobj);console.log("Finished: ",testobj);testobj.added&&testobj.parent!==held_node.parent().id()&&(console.log("Move ",held_node.id()," into ",testobj.parent),held_node.move({parent:testobj.parent}));held_node=null}});
function checkCompoundBounds(a,b,c){a.forEach(function(a){var e=a.renderedBoundingBox();b.x>e.x1&&b.x<e.x2&&b.y>e.y1&&b.y<e.y2&&held_node!==a&&(c.added=!0,c.parent=a.id(),checkCompoundBounds(a.children(":parent.expanded").difference(a),b,c))})}cy.on("taphold",":parent",function(a){parent.children().animated()});
...
cy.on("tap",function(a){var b=a.cyTarget;b===cy&&(cy.$(":selected").unselect(),current_state===states.CONNECTING&&b===cy&&null!==source_node&&(source_node.removeClass("source_node"),source_node=null),current_state===states.NEWPAGE?(console.log("Add a node"),b===cy&&console.log("Really Add a node"+cy.add({data:{name:cy.nodes(".page, .control").size()+1,pagestyle:selected_page_template.pagestyle,outputcontainer:selected_page_template.outputcontainer,imgcontainers:selected_page_template.imgcontainers,
...
current_state=states.NEWFIGHT;cy.$("#fightparent"+cy.nodes(":parent").size()+1).select()};var conditionsConextMenu={};
...
a+")"});$(".playpage").css({"-webkit-transform":"scale("+a+")"})}$(window).resize(resizePlayPage);console.log("Enter main.js version 0.5.0");$(document).ready();$("select").material_select();var testgraph={};function testJumpNodes(){}function checkValidGraph(){var a="";1>cy.$("*").length&&(a=a.concat("The graph is empty\n"));cy.$(".start").hasClass("page")||(a=a.concat("Starting class must be a page node\n"));testConnectivity()||(a=a.concat("Graph is NOT connected\n"));""==a?alert("All tests passed"):alert(a)}
function testConnectivity(){var a=!1,b=cy.$(".start"),c=cy.$(".page, .control");console.log("Root node: "+b.data("id"));var d=cy.collection(),d=d.add(b);c.union(cy.$("edge")).dfs({roots:b,visit:function(a,b,c){d=d.add(c);console.log("visit "+this.id()+" total "+a)},directed:!0});console.log("Total nodes:"+c.size()+" Connected_nodes: "+d.size());if(c.size()==d.size())a=!0;else{var e=c.diff(d).left;console.log("Disconnected:",e.size());console.log("left: "+e.map(function(a){return a.id()}).join(", "));
e.addClass("disconnected");setTimeout(function(){e.removeClass("disconnected")},3E3)}return a};var tests={};
