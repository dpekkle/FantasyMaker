...
function initEmptyProject(a,b){return{project_templates:{Default:{pagestyle:"width: 800px; flex: 0 0 800px; height:600px; border: 3px solid black",outputcontainer:"",imgcontainers:[],vidcontainers:[],textcontainers:[],decisioncontainers:[]}},template_menus:new templateMenuObj,audio:new audioObj,projectOwner:a,projectName:b,dateCreated:project_getCurrentTime(),lastModified:project_getCurrentTime(),published:!1,title:b,author:a,description:"No description available.",imageLink:"No URL provided",gameLink:host_play()+
a+"/"+b,graph:[],gameAttributes:{},gameInventory:{},attributesHTML:"",resolution:{x:1E3,y:800}}}
function project_createNewProject(){myModal.prompt("Create New Project","",[{name:"Project Name","default":"",type:"text"}],function(a){},function(a){if(""==a[0])myModal.warning("Project name cannot be empty");else{var b={};$.when(http_getUsersProjects(users_getUsername(),b)).done(function(){console.log("RET");console.log(b);if(b.hasOwnProperty("projects"))for(var c=0;c<b.projects.length;c++)if(a[0].trim()===b.projects[c].projName){myModal.warning("You already have a project named "+b.projects[c].projName+
". Please choose another name.");return}project_project=initEmptyProject(users_getUsername(),a[0].trim());cy.elements().remove();var d={data:[]};$.when(http_save(project_project,d)).done(function(){!0===d.data?(Materialize.toast("Project '"+project_project.projectName+"' created!",3E3,"rounded"),$("#prompt-modal").closeModal(),$("#UI_projName").text("Project: "+project_project.projectName),nav_toMain(),http_getUsersProjects(users_getUsername(),projectSettings_userProjects)):Materialize.toast("Failed to create project. Please log in again.",
3E3,"rounded")})})}})}function project_addTopGameAttributeFolder(a){var b=generateID();project_project.gameAttributes[b]=new GameAttribute(null,null,a,b,!1);a='<li class="'+b+'-list-element margin pointer"><a onclick="gameAttributes_display(\''+b+"')\">"+a+'</a><ul id="'+b+'-inner_list"></ul></li>';$("#attributes-list").append(a);console.log("new top level attribute added: "+project_project.gameAttributes[b].path)}
...
cy.on("tapend",function(a){if(null!=held_node){console.log("Check for dropping into parents:",cy.$(":parent").length);var b=cy.$(":parent:orphan");a=a.cyRenderedPosition;testobj={added:!1,parent:"none"};checkCompoundBounds(b,a,testobj);console.log("Finished: ",testobj);testobj.added&&testobj.parent!==held_node.parent().id()&&(console.log("Move ",held_node.id()," into ",ele.id()),held_node.move({parent:testobj.parent}));held_node=null}});
function checkCompoundBounds(a,b,c){a.forEach(function(a){var e=a.renderedBoundingBox();b.x>e.x1&&b.x<e.x2&&b.y>e.y1&&b.y<e.y2&&held_node!==a&&(c.added=!0,c.parent=a.id(),checkCompoundBounds(a.children(":parent").difference(a),b,c))})}cy.on("taphold",":parent",function(a){parent.children().animated()});
