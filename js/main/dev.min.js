...
function choosePrebuilt(a){$(".prebuilt").addClass("activebutton");"Empty"==a?current_state=states.NEWEMPTY:"Fight"==a&&(current_state=states.NEWFIGHT)}function chooseJump(a){$(".jumpmode").addClass("activebutton");"Start"==a?current_state=states.NEWJUMP:"End"==a&&(current_state=states.NEWJUMPEND)};var tutorial={},tutorial_graph=null,tutorial_objects=[{message:"First click Page Node and select 'Default'",css:function(){$(".pagemode").addClass("tutorial-button-highlight")},condition:function(){console.log("First function");return current_state==states.NEWPAGE?(console.log("First function done"),!0):!1}},{message:"Now tap on the screen on the left to create a page",css:function(){$(".pagemode").removeClass("tutorial-button-highlight");$("#cy").addClass("tutorial-button-highlight")},condition:function(){return 0!==
...
{message:"Click the edge button.",css:function(){$(".connectionmode").show()},condition:function(){return current_state==states.CONNECTING?!0:!1}},{message:"Now tap one of the pages you added.",condition:function(){return null!==source_node?!0:!1}},{message:"The blue border indicates the 'source' of the edge"},{message:"Now tap a different page node.",condition:function(){return 0!==cy.edges().size()?(cy.edges(),!0):!1}},{message:"Nice! You've now connected two pages"},{message:"Try making another edge",
condition:function(){return 1<cy.edges().size()?!0:!1}},{message:"The labels on each edge help you keep track of them"},{message:"Leave edge mode by pressing escape or the edge button again",condition:function(){return current_state!==states.CONNECTING?!0:!1}},{message:"Now lets see what that edge looks like in the page",css:function(){$(".connectionmode").removeClass("tutorial-button-highlight")}},{message:"Select a page with an edge leaving it",css:function(){},condition:function(){return 0<cy.$(":selected").outgoers().edges().size()?
!0:!1}},{message:"Now press 'Edit Page' to open up the Page Designer",css:function(){$('.editbutton[data-target="page-modal"').addClass("tutorial-button-highlight")},condition:function(){return"none"!==$("#page-modal").css("display")?!0:!1}},{message:"Here you can design the appearance of the page, you can see the edges that you just created",css:function(){$('.editbutton[data-target="page-modal"').removeClass("tutorial-button-highlight")}},{message:"You can position them however you want by dragging the handle on top of them."},
...
!1;this.validated=!0;this.callbackFunction=this.verifyFunction=null;f.html("")}})};this.warning=function(a){$("#promptDesc").css("color","red");!1!==a&&$("#promptDesc").text(a)}}myModal=new myModal;myModal.init();
$(document).ready(function(){$("html").on("keydown",function(a){$("#prompt-modal").hasClass("open")?13==a.which?myModal.evaluateModal(!0):27==a.which&&myModal.evaluateModal(!1):$("#define-attribute-modal").hasClass("open")&&(13==a.which?defineAttributeModal.evaluateModal(!0):27==a.which&&defineAttributeModal.evaluateModal(!1))})});var projectSettings={},projectSettings_activePage=1,projectSettings_amtPages=1,projectSettings_userProjects={projects:[]};
...
$("#"+b).css("transition",""))})};var pageContainerHelpers={};
...
"<div id = 'vid"+b+"'class = 'handle vid-handle'>Video",c+="</div>",c+='<a class="vidmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';else if("text"==a)c+='<a class="btn-floating waves-effect waves-light grey lefticon"><i class="material-icons">comment</i></a>',c+="<div id = 'text"+b+"'class = 'handle text-handle'>Text",c+="</div>",c+='<a class="textmenu btn-floating waves-effect waves-light red righticon"><i class="material-icons">settings</i></a>';
...
function dragMoveListener(a){var b=$("#pagecontainers")[0],c=b.getBoundingClientRect().width/b.offsetWidth,d=b.getBoundingClientRect().height/b.offsetHeight,b=a.target,c=(parseFloat(b.getAttribute("data-x"))||0)+a.dx/c;a=(parseFloat(b.getAttribute("data-y"))||0)+a.dy/d;b.style.webkitTransform=b.style.transform="translate("+c+"px, "+a+"px)";b.setAttribute("data-x",c);b.setAttribute("data-y",a)}window.dragMoveListener=dragMoveListener;var pageOverlay={},show_handles=!0;function updatePageStyle(a){openEditPageOverlay(a);closeOverlay(a)}function toggleHandles(){show_handles=!show_handles;$(".handlemode").toggleClass("activebutton");show_handles?($(".handlecontainer").show(),$(".handlemode").html("Handles: Shown"),$(".pagemenu").show()):($(".handlecontainer").hide(),$(".handlemode").html("Handles: Hidden"),$(".pagemenu").hide())}
...
function bindHandleSelection(){$(".handle").last().on("mousedown touch",function(a){$(this).siblings().trigger("focus");bringContainerToFront($(this).parent().parent());console.log("trigger handle")})}function bringContainerToFront(a){console.log("Bring to front: ",a);var b=50;$("#pagecontainers").children("div").each(function(){var a=$(this).css("zIndex");a>b&&(b=a)});b++;if("decision"==a)return b;console.log("Set zIndex to ",b);a.css("zIndex",b)}
...
d[b].name),e=!0);e||d.splice(b,1)}show_handles||$(".handle").hide();$(".handle").on("mousedown touch",function(a){$(this).siblings().trigger("focus");bringContainerToFront($(this).parent().parent());console.log("trigger handle")})}
...
$(document).ready(function(){var a=!1;$("html").on("keyup",function(b){if(17==b.which||91==b.which)a=!1}).on("keydown",function(b){if(17==b.which||91==b.which)a=!0;if("none"==$("#Play").css("display")&&!$(".modal").hasClass("open"))if(console.log(b.which),a&&67==b.which)0!=cy.$(":selected").length&&0==cy.$(":selected:parent").length&&(console.log("Copy"),nodes_to_clone=cy.$(":selected").nodes().jsons(),console.log(nodes_to_clone));else if(a&&86==b.which){if(console.log(nodes_to_clone),null!==nodes_to_clone){console.log("Paste");
cy.$(":selected").unselect();for(b=0;b<nodes_to_clone.length;b++){var c={x:nodes_to_clone[b].position.x+10,y:nodes_to_clone[b].position.y+=10};nodes_to_clone[b].data.name=parseInt(cy.nodes().size()+1);nodes_to_clone[b].classes=nodes_to_clone[b].classes.replace("start","");delete nodes_to_clone[b].data.id;cy.add({data:nodes_to_clone[b].data,classes:nodes_to_clone[b].classes,group:nodes_to_clone[b].group,position:c}).select()}nodes_to_clone=cy.$(":selected").nodes().jsons()}}else if(27==b.which)console.log("Press escape!"),
defaultState(),cy.$(":selected").unselect();else if(49<=b.which&&58>=b.which)0==$(".dropdown-content.active").length?49==b.which?$(".pagemode").trigger("click"):50==b.which?$(".controlmode").trigger("click"):51==b.which?$(".jumpmode").trigger("click"):52==b.which?$(".connectionmode").trigger("click"):53==b.which&&$(".prebuilt").trigger("click"):$(".dropdown-content.active li a").eq(b.which-49).trigger("click");else if(0!=cy.$(":selected").length)if(8==b.which||46==b.which)console.log("Press delete"),
$(".deletebutton")[0].click();else if(13==b.which||32==b.which)event.preventDefault(),$(".openoverlay").each(function(a){"none"!=$(this).css("display")&&(console.log("Enter press on ",$(this)),$(this).trigger("click"))})})});var http_loginSignup={};function http_signUp(a,b){return $.ajax({url:"/signUp",data:{data:a},cache:!1,type:"POST",success:function(a){http_handleAuth(a)},contenttype:"application/json"})}function http_login(a,b,c){return $.ajax({url:"/login",data:{uname:a,pwd:b},cache:!1,type:"POST",success:function(a){c.data=a},contenttype:"application/json"})}
