...
function assessCondition(a){a=$.parseHTML(a.html);console.log(a);var b=a[0].attributes[1].value;if("1"===b){var b=getAttributeValue(a[0].childNodes[1].childNodes[0]),c=a[0].childNodes[2].childNodes[0].data,d=getAttributeValue(a[0].childNodes[3].childNodes[0]),e=assessComparison(b,c,d);logger.log("Condition: "+getAttributeText(a[0].childNodes[1].childNodes[0])+"("+b+") "+c+" "+getAttributeText(a[0].childNodes[3].childNodes[0])+"("+d+") is "+boolToString(e)+"<br>");return e}if("2"===b){var b=getAttributeValue(a[0].childNodes[1].childNodes[0]),
...
function getAttributeValue(a){console.log("getting value for: "+a.id);if("input-field"===a.classList[0])return console.log("input field found"),console.log("input feild in html: "+a.attributes.value.value),parseFloat(a.attributes.value.value),console.log("input feild parsed: "+a.attributes.value.value),parseFloat(a.attributes.value.value);if("random"===a.classList[0]){console.log("random number found");var b=parseFloat(a.attributes.min.value);a=parseFloat(a.attributes.max.value);var c=Math.floor(Math.random()*
(a-b+1))+b;console.log("Generated number between "+b+" and "+a+" is: "+c);return c}b=a.attributes.path.nodeValue;console.log("Path is: ",b);return gameAttributes_find(b).value}function getAttributeText(a){if("input-field"===a.classList[0])return a.attributes.value.value;if("random"===a.classList[0])return"a random number";a=a.attributes.path.value.split("_");for(var b,c,d=0;d<a.length;d++)0===d?(c=a[d],b=gameAttributes_find(c).name):(c+="_"+a[d],b+=" : "+gameAttributes_find(c).name);return b}
function assessComparison(a,b,c){a=parseFloat(a);c=parseFloat(c);console.log("COMPARISON: LEFT: "+a+" RIGHT: "+c);switch(b){case "=":return a===c?!0:!1;case "!=":return a!==c?!0:!1;case ">":return a>c?!0:!1;case "<":return a<c?!0:!1;case ">=":return a>=c?!0:!1;case "<=":return a<=c?!0:!1}}
function assessModification(a,b,c){a=parseFloat(a);c=parseFloat(c);console.log("MODIFICATION: LEFT: "+a+" RIGHT: "+c);switch(b){case "=":return c;case "+":return a+c;case "-":return a-c;case "*":return a*c;case "/":return a/c}};var playJump={};function initJumpNodes(){conditional_jump_nodes=cy.nodes('.jump[triggerType = "attribute"]');jump_context_stack=[]}function checkConditionalJumps(){var a=!1;conditional_jump_nodes.forEach(function(b){if(evaluateConditionalJump(b))return a=b,!1});return a}
