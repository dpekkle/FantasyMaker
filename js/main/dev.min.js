...
b.items.textOutput.items.playerOutputAttribute={name:"Player Output (Text : Attribute : Text)",callback:function(b,c){swapOutcome(b,a.attr("id"))}},b.items.textOutput.items.playerOutputText={name:"Player Output (Text Only)",callback:function(b,c){swapOutcome(b,a.attr("id"))}},b.items.invOuts.items.inventory_addRemove={name:"Add or remove inventory items",callback:function(b,c){swapOutcome(b,a.attr("id"))}}):(b.items.attConds={name:"Attribute Conditions",items:{}},b.items.invConds={name:"Inventory Conditions",
items:{}},b.items.randConds={name:"Random Number Conditions",items:{}},b.items.attConds.items.type1={name:"Compare Two Attributes",callback:function(b,c){setComparisonType("type1",a.attr("id"))}},b.items.attConds.items.type2={name:"Compare Statement and Attribute",callback:function(b,c){setComparisonType("type2",a.attr("id"))}},b.items.attConds.items.type3={name:"Compare Two Statements",callback:function(b,c){setComparisonType("type3",a.attr("id"))}},b.items.invConds.items.type4={name:"Item In Inventory",
callback:function(b,c){setComparisonType("type4",a.attr("id"))}},b.items.invConds.items.type5={name:"Amount Of Item In Inventory",callback:function(b,c){setComparisonType("type5",a.attr("id"))}},b.items.randConds.items.type6={name:"Random Number Comparison",callback:function(b,c){setComparisonType("type6",a.attr("id"))}});b.items.sep={name:"----------------------------------------------",disabled:!0};var c=a.attr("id").split("_"),d;if("newCondition"===c[0]||"exCondition"===c[0])d="Remove Condition";
else if("newOutcome"===c[0]||"exOutcome"===c[0])d="Remove Outcome";b.items.remove={name:d,callback:function(a,b){"Remove Condition"===d?"newCondition"===c[0]?removeCondition(c[0]+"_"+c[1]):removeCondition(c[0]+"_"+c[1]+"_"+c[2]):"Remove Outcome"===d&&("newOutcome"===c[0]?removeOutcome(c[0]+"_"+c[1]):removeOutcome(c[0]+"_"+c[1]+"_"+c[2]))}};return b}});
...
function findFirstLeafPath(a){var b=gameAttributes_find(a);if(!1===b.is_leaf)for(var c=0;c<b.childrenArray.length;c++){var d=findFirstLeafPath(a+"_"+b.childrenArray[c]);if(void 0!==d)return d}else return a}
function initAttributeButton(a){$("#"+a).children().text("...........");$("#"+a).attr("path","");$("#"+a).attr("data-tooltip","There are no attributes in your project. <br> Add attributes via the 'Attributes' button. ");$("#"+a).tooltip({delay:50});if(0<Object.keys(project_project.gameAttributes).length)for(var b in project_project.gameAttributes){var c=findFirstLeafPath(b);if(void 0!==c){b=gameAttributes_find(c);$("#"+a).children().text(b.name);$("#"+a).attr("path",c);updateTooltip(a,c);break}}}
...
$("#"+d).replaceWith(c),$("#"+d+"_text_1").tooltip({delay:50})):"inventory_addRemove"===a&&(c=generateOutcome_inventoryAddRemove(d),$("#"+d).replaceWith(c),$("#"+d+"_invButton").tooltip({delay:50}))}
...
function gameAttributes_addAttributeFolder(a,b,c){b={name:b,is_leaf:!1,parentPath:a,id:c};c={};a?(b.path=a+"_"+b.id,c=gameAttributes_find(a),c.childrenArray.push(b.id)):(b.parentPath=null,c=project_project.gameAttributes,b.path=b.id);c[b.id]=new GameAttribute(b);gameAttributes_createModule_addAttributeListElement(b.path)}function gameAttributes_getAllParents(a){path=a.split("_");a=project_project.gameAttributes;for(var b=[],c=0;c<path.length;c++)a=a[path[c]],b.push(a);return b}
function gameAttributes_createModule_addAttributeValue(){var a=generateID();defineAttributeModal.prompt(a,!0,currentAttributeObj.path)}function gameAttributes_createModule_updateAttributeValue(a){defineAttributeModal.prompt(a,!1)}
function gameAttributes_createModule_addAttributeFolder(a){var b=generateID();myModal.prompt("Add A New Folder","",[{name:"Enter A Folder Name","default":"",type:"text"}],function(c){gameAttributes_addAttributeFolder(a,c[0],b);a||(currentAttributeObj=project_project.gameAttributes[b]);gameAttribute_createModule_updateContextPane()},function(a){return""==a[0]?!1:!0})}function gameAttributes_createModule_openEditAttributeModal(a){}function gameAttributes_createModule_deleteAttribute(a){}
function gameAttributes_createModule_displayFolder(a){currentAttributeObj=gameAttributes_find(a);gameAttribute_createModule_updateBreadcrumbs();gameAttribute_createModule_updateContextPane()}
...
a.id+'-value" style="font-size: 19px; font-weight: bold">'+a.value+'</span></span><br/><span style="font-size: 14px"> Value Range: <span id="'+a.id+'-min-value" style="font-size: 19px; font-weight: bold">'+a.minValue+'</span> to <span id="'+a.id+'-max-value" style="font-size: 19px; font-weight: bold">'+a.maxValue+'</span></span><br/><hr/><a class="btn-floating red right hoverable" onclick="gameAttributes_createModule_openEditAttributeModal(\''+a.path+'\')"><i class="material-icons small">delete</i></a><a class="btn-floating blue right hoverable"onclick="gameAttributes_createModule_deleteAttribute(\''+
a.path+'\')"><i class="material-icons small">mode_edit</i></a></div>',d.append(b),void 0==a.minValue&&$("#"+a.id+"-min-value").text("\u221e"),void 0==a.maxValue&&$("#"+a.id+"-max-value").text("\u221e")):(a='<div class="folder card-panel hoverable" onclick="gameAttributes_createModule_displayFolder(\''+a.path+'\')"><h6 class="folder-title ">'+a.name+"</h6></div>",c.append(a))}
...
function gameAttributes_createModule_addAttributeListElement(a){a=gameAttributes_find(a);var b;b=a.is_leaf?'<li id="'+a.id+'-list-element" class="lastChild">'+a.name+"</li>":'<li id="'+a.id+'-list-element" class="lastChild"><a class="'+a.id+'-name" onclick="gameAttributes_createModule_displayFolder(\''+a.path+"')\">"+a.name+"</a></li>";var c;null!=a.parentPath?(c=$("#"+gameAttributes_find(a.parentPath).id+"-inner-list"),$("#"+gameAttributes_find(a.parentPath).id+"-list-element li:last").removeClass("lastChild")):
(c=$("#all-attributes-inner-list"),$("#all-attributes-inner-list li:last").removeClass("lastChild"));c.append(b);a.is_leaf||(b=$("#"+a.id+"-list-element"),b.addClass("collapsibleListClosed"),b.append('<ul id="'+a.id+'-inner-list" class="collapsible-list"></ul>'))}
...
function gameAttributes_createModule_recursiveListDisplay(a){gameAttributes_createModule_addAttributeListElement(a);for(var b=gameAttributes_find(a),c=0;c<b.childrenArray.length;c++)gameAttributes_createModule_recursiveListDisplay(gameAttributes_find(a+"_"+b.childrenArray[c]).path)}var defineAttributeModal=new DefineAttributeModal;defineAttributeModal.init();
function DefineAttributeModal(){this.isNewValue=this.confirm=!1;this.init=function(){$("body").append('<div id="define-attribute-modal" class=" modal modal-fixed-footer"><div class="modal-content"><div class="row row-centered"><h3>Define Attribute</h3></div><div class="row row-centered"><div class="col m6"><div class="input-field col m10"><input id="attribute-name-input" class="validate"  type="text"/><label for="attribute-name-input" data-error="Required" class="active" >Attribute Name</label></div><div class="input-field col m10"><textarea id="attribute-detail-input" class="materialize-textarea"></textarea><label for="attribute-detail-input" class="active">Item Description</label></div></div><div class="col m6"><div class="input-field col m6 offset-m6"><input id="attribute-initValue-input" class="validate" value="0" type="number"/><label for="attribute-initValue-input" data-error="Required" class="active" >Initial Value</label></div><div class="switch col m4 right-align"><label>Enable Minimum<input id="attribute-minValue-toggle" type="checkbox"><span class="lever"></span></label></div><div class="input-field col m6 offset-m2"><input id="attribute-minValue-input" class="validate" value="0" type="number"/><label for="attribute-minValue-input" data-error="Required" class="active">Minimum Value</label></div><div class="switch col m4 right-align"><label >Enable Maximum<input id="attribute-maxValue-toggle" type="checkbox"><span class="lever"></span></label></div><div class="input-field col m6 offset-m2"><input id="attribute-maxValue-input" class="validate" value="0" type="number"/><label for="attribute-maxValue-input" data-error="Required" class="active" >Maximum Value</label></div></div></div></div><div class="modal-footer"><a href="#!" class=" modal-action modal-close waves-effect waves-red btn-flat" onclick="defineAttributeModal.evaluateModal(false)">Cancel</a><a href="#!" class=" modal-action waves-effect waves-green btn-flat" onclick="defineAttributeModal.evaluateModal(true)">Confirm</a></div></div>')};this.prompt=
function(a,b,c){this.nameInput=$("#attribute-name-input");this.detailInput=$("#attribute-detail-input");this.initValueInput=$("#attribute-initValue-input");this.minValueToggle=$("#attribute-minValue-toggle");this.minValueInput=$("#attribute-minValue-input");this.maxValueToggle=$("#attribute-maxValue-toggle");this.maxValueInput=$("#attribute-maxValue-input");this.currentAttributeID=a;this.currentPath=c;this.isNewValue=b;this.attributeObj={};this.isNewValue||(this.attributeObj=gameAttributes_find(c+
...
function(a){if(a){a=!1;var b=!0,c=!0;void 0!=this.nameInput.val()&&""!=this.nameInput.val()&&void 0!=this.initValueInput.val()&&""!=this.initValueInput.val()&&(a=!0);!this.maxValueToggle.prop("checked")||""!=this.maxValueInput.val()&&void 0!=this.maxValueInput.val()||(c=a=!1);!this.minValueToggle.prop("checked")||""!=this.minValueInput.val()&&void 0!=this.minValueInput.val()||(b=a=!1);if(a)void 0!=this.detailInput.val()&&""!=this.detailInput.val()||this.detailInput.val("No description provided"),
a={name:this.nameInput.val(),id:this.currentAttributeID,description:this.detailInput.val(),value:this.initValueInput.val(),parentPath:currentAttributeObj.path,is_leaf:!0},this.minValueToggle.prop("checked")?a.minValue=this.minValueInput.val():a.minValue=void 0,this.maxValueToggle.prop("checked")?a.maxValue=this.maxValueInput.val():a.maxValue=void 0,this.isNewValue?(currentAttributeObj[this.currentAttributeID]=new GameAttribute(a),currentAttributeObj.childrenArray.push(this.currentAttributeID),gameAttributes_createModule_recursiveListDisplayAll()):
currentAttributeObj[this.currentItemID]=new GameAttribute(a),gameAttributes_createModule_displayFolder(currentAttributeObj.path),$("#define-attribute-modal").closeModal(),this.nameInput.val(""),this.detailInput.val(""),this.initValueInput.val("0"),this.minValueInput.val("0"),this.maxValueInput.val("0");else{console.log("item definition validation failed");if(void 0==this.nameInput.val()||""==this.nameInput.val())Materialize.toast("Please provide an attribute name",5E3,"rounded","dismissible"),this.nameInput.addClass("invalid");
if(void 0==this.initValueInput.val()||""==this.initValueInput.val())Materialize.toast("Please provide an initial attribute value ",7E3,"rounded","dismissible"),this.initValueInput.addClass("invalid");b||(Materialize.toast("Please provide a minimum value",7E3,"rounded","dismissible"),this.minValueInput.addClass("invalid"));c||(Materialize.toast("Please provide a maximum value",7E3,"rounded","dismissible"),this.maxValueInput.addClass("invalid"))}}else $("#define-item-modal").closeModal(),this.nameInput.val(""),
this.detailInput.val(""),this.initValueInput.val("0"),this.maxValueInput.val("0"),this.minValueInput.val("0")}}function gameAttributes_find(a){path=a.split("_");a=project_project.gameAttributes;for(var b=0;b<path.length;b++)a=a[path[b]];return a}function generateID(){for(var a="",b=0;4>b;b++)a+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(62*Math.random()));return a};var gameInventory={};function InventoryItem(a){this.itemID=a.itemID;this.name=a.name;this.description=a.description;this.initCount=a.initCount;this.modifiers=a.modifiers;this.playCount=this.initCount}function gameInventory_defineItem(a,b){var c="item-"+generateID();project_project.gameInventory[c]=new InventoryItem(c,a,b)}function gameInventory_editItem(a,b,c){a=project_project.gameInventory[a];a.name=b;a.description=c;return a}
...
function project_logOut(){myModal.prompt("Log Out","Are you sure you wish to log out? Any unsaved progress will be lost.",[],function(a){console.log(users_getUsername()+" logged out");nav_toLogin();users_flushToken();project_project=initEmptyProject("none","none");http_redirectHome()},function(a){return!0})};var playConditions={};
...
b=getTextButtonValue(b[0].childNodes[3].childNodes[0]),void 0===c&&(console.log("executeOutcomes() attribute modification: attButton1 or attButton2 is undefined"),c=" <Error: No attribute selected> "),logger.playerLog(d+c+b);else if("Text"===c)d=getTextButtonValue(b[0].childNodes[1].childNodes[0]),logger.playerLog(d);else if("inventory_addRemove"===c){console.log(b);d=getAddRemoveButtonState(b[0].childNodes[1].childNodes[0]);c=getAttributeValue(b[0].childNodes[2].childNodes[0]);b=getInventoryItemFromHtml(b[0].childNodes[3].childNodes[0]);
if(void 0===b){console.log("executeOutcomes() inventory add/remove: item is undefined.");break}else if(isNaN(c)){console.log("executeOutcomes() inventory add/remove: inputFieldValue is not a number.");break}"add"===d?(d=parseInt(b.playCount),c=d+c,b.playCount=c):"remove"===d&&(d=parseInt(b.playCount),c=d-c,b.playCount=0>c?0:c)}}else logger.log("Edge "+a.json().data.name+" has no outcomes.<br>")}function getTextButtonValue(a){return a.attributes["data-tooltip"].value}
